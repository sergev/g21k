     ...
     rtx *loc;
{
  ...
  do_not_record = 0;
  hash_code = HASH (addr, Pmode1 (Extract_rtx_Pmode (addr)));
  ...
  elt = lookup (addr, hash_code, Pmode1 (Extract_rtx_Pmode (addr)));
  ...
  if (flag_expensive_optimizations
      ...
      && GET_CODE (XEXP (*loc, 1)) == CONST_INT)
    {
      ...
      do_not_record = 0;
      hash_code = HASH (XEXP (*loc, 0), Pmode1 (DEFAULT_DATA_Pmode));
      do_not_record = save_do_not_record;
      hash_arg_in_memory = save_hash_arg_in_memory;
      hash_arg_in_struct = save_hash_arg_in_struct;

      elt = lookup (XEXP (*loc, 0), hash_code, Pmode1 (DEFAULT_DATA_Pmode));
      if (elt == 0)
	return;
      ...
      while (found_better)
	{
	  ...
	  for (p = elt->first_same_value; p; p = p->next_same_value)
		...
		&& (GET_CODE (p->exp) == REG
		    || exp_equiv_p (p->exp, p->exp, 1, 0)))
	      {
		rtx new = simplify_binary_operation (GET_CODE (*loc), 
						     Pmode1 (Extract_rtx_Pmode (*loc)),
						     p->exp, c);

		if (new == 0)
		  new = gen_rtx (GET_CODE (*loc), Pmode1 (Extract_rtx_Pmode (*loc)), p->exp, c);
...

#ifdef ADI
/* Given a const expression OP with result mode MODE,
   return OP adjusted to take the mode and pointer information
   of the CONST expression into account. */

rtx
propagate_const_mode (mode, op, pointer_p)
     enum machine_mode mode;
     rtx op;
     int pointer_p;
{
  /* CONST wrapper should have same mode as its operand, unless
     its operand is VOIDmode.  Also, CONST should have pointer
     flag if operand does. */
  if ((GET_MODE (op) != VOIDmode && GET_MODE (op) != mode)
      || (RTX_ISPOINTER_P (op) && !pointer_p))
    abort();

  if (!pointer_p || GET_MODE (op) != VOIDmode
      || (GET_MODE (op) == mode && RTX_ISPOINTER_P (op)))
    return op;

  /* VOIDmode constants marked as pointers should all be
     CONST_INT, never CONST_DOUBLE or anything else. */
  if (GET_CODE (op) != CONST_INT)
    abort();

  op = gen_rtx (CONST_INT, mode, INTVAL (op));
  RTX_MARK_ISPOINTER (op);
  return op;
}
#else
#define propagate_const_mode (mode, op, pointer_p)  (op)
#endif
     ...
     rtx op0, op1;
{
  ...
  if (GET_CODE (op0) != CONST_INT || GET_CODE (op1) != CONST_INT
	...
	case PLUS:
	  ...
	  if (rtx_equal_p(op1,CONST0_RTX (mode)))
	    return op0;

	  /* Strip off any surrounding CONSTs.  They don't matter in any of 
	     the cases below.  */
	  if (GET_CODE (op0) == CONST)
	    op0 = propagate_const_mode (GET_MODE (op0), XEXP (op0, 0),
					RTX_ISPOINTER_P (op0));
	  if (GET_CODE (op1) == CONST)
	    op1 = propagate_const_mode (GET_MODE (op1), XEXP (op1, 0),
					RTX_ISPOINTER_P (op1));
	  ...
	  if (CONSTANT_P (op0) && CONSTANT_P (op1)
	    ...
	    {
#ifdef ADI
	      /* Make sure to preserve PM/DM and pointer flag on
		 const_int + const_int.  (I _think_ this is the only
		 case where this won't happen already.) */
	      if (GET_CODE (op0) == CONST_INT
		  && GET_CODE (op1) == CONST_INT)
		{
		  rtx result = gen_rtx (CONST_INT, mode,
					INTVAL (op0) + INTVAL (op1));

		  if (RTX_ISPOINTER_P (op0) || RTX_ISPOINTER_P (op1))
		    RTX_MARK_ISPOINTER (result);
     ...
     rtx insn;    
{
  ...
  switch (code)
      ...
      if (insn && GET_CODE (insn) == JUMP_INSN)
	{
	  ...
	  if (next && GET_CODE (next) == CODE_LABEL
	    ...
	    return gen_rtx (LABEL_REF, Pmode1 (Extract_rtx_Pmode (next)), next);
	}
      break;
...
#if defined (DSP21K) || defined (A21C0)
	  /*  It is dangerous to strip away SUBREGs when converting to PM or DM 
	      if only because PM register are only 24 bits 
	      We are overly conservative here, but stripping SUBREGs in this 
	      situation involves the danger of equivalencing PM and DM 
	      which is a taboo. */
	  {
	    enum machine_mode submode = GET_MODE(SUBREG_REG(x));
	    if (mode == DMmode || submode == DMmode ||
		mode == PMmode || submode == PMmode)
	      return x;
	  }
#endif
	  return x;
      ...
      if (folded_arg0 != SUBREG_REG (x))
	{
	  ...
	  if(GET_CODE(new)==CONST &&
	     GET_MODE(SUBREG_REG(x))==GET_MODE(new))
	  {
	      if(GET_MODE(x)==SImode && 
		 GET_CODE(SUBREG_REG(x)) == CONST_INT)
		  new=XEXP(new,0);
	      else if ((GET_MODE(new) == PMmode && GET_MODE(x) == DMmode) ||
		       (GET_MODE(new) == DMmode && GET_MODE(x) == PMmode))
		  new = gen_rtx(CONST, GET_MODE(x), XEXP(new, 0));
	      else if (GET_CODE(XEXP(new, 0)) == CONST_INT)
		  /* if it is equivalent to const_int, leave it along */
		  new = XEXP (new,0);
	      else
		  new = gen_rtx(CONST, GET_MODE(x), XEXP(new, 0));
	  }
	  ...
	  && subreg_lowpart_p (x))
	{
      ...
      if(GET_CODE(x)==SUBREG &&
	 GET_MODE(x)==SImode && 
	 (GET_MODE(folded_arg0)==PMmode || GET_MODE(folded_arg0)==DMmode))
	if(GET_CODE(folded_arg0)==CONST)
	  x=XEXP(folded_arg0,0);
	else if(GET_CODE(folded_arg0)==CONST_INT)
	  x=gen_rtx(CONST_INT,VOIDmode,INTVAL(folded_arg0));
#endif
	...
	if (base && GET_CODE (base) == LABEL_REF)
	  {
		...
		if (offset >= 0
		  ...
		  {
		    offset /= GET_MODE_SIZE (GET_MODE (table));
		    new = gen_rtx (MINUS, Pmode1 (Extract_rtx_Pmode (XVECEXP (table, 1, offset))), 
						 XVECEXP (table, 1, offset),
				   XEXP (table, 0));

		    if (GET_MODE (table) != Pmode1 (Extract_rtx_Pmode (XVECEXP (table, 1, offset))))
...
#if 0	/* shut up complaint about empty else */
    else if (fmt[i] == 'E')
  ...
  switch (GET_RTX_CLASS (code))
	...
	case ASHIFT:  case LSHIFTRT:  case ASHIFTRT:
	  ...
	  if (GET_CODE (folded_arg0) == REG
#ifdef ADI
	      && const_arg1 && CONSTANT_P (const_arg1))
#else
...
#endif
		  && INTVAL (const_arg1) == INTVAL (inner_const)
		  && (0
		      ...
		      || exact_log2 (- INTVAL (const_arg1)) >= 0
#endif
		      /* Well similarly if we have any constant it can be used in
			 conjunct with a post or pre modify operation. */
#if defined(HAVE_POST_MODIFY) || defined(HAVE_PRE_MODIFY)
		      || 1
#endif
		      ))
		break;
		 ...
		 A/C1/C2 is A/(C1 * C2), so if CODE == DIV, we want MULT.  */
	      ...
	      /* mmh: i'm not 100% sure if unsigned multiply should be include here */
	      associate_code
		= (code == MULT || code == UMULT || code == DIV || code == UDIV ? MULT
...
#ifdef ADI
void invalidate_post_modify(rtx x)
{
  enum rtx_code code;
  char *fmt;
  int i,j;
  ...
  for (i=GET_RTX_LENGTH (code) -1;i >=0; i--)
    if (fmt[i]=='e')
      invalidate_post_modify (XEXP (x,i));
    else if (fmt[i]=='E')
      for (j=XVECLEN(x,i)-1; j>=0; j--)
	invalidate_post_modify (XVECEXP (x,i,j));
}
#endif
  ...
  for (i = 0; i < n_sets; i++)
	...
	src_folded = src_const, src_folded_cost = -1;
	  ...
	  if (n_sets == 1 && dest == pc_rtx
		...
		trial = gen_rtx (LABEL_REF, Pmode1 (DEFAULT_CODE_Pmode), get_label_after (trial));
...
#ifdef DSP21XX
/* Grab special case where we have (SUBREG:SI y) where y is a constant.
   Don't leave it to the code below to put in the (SUBREG ), we don't want it.
   Just leave it as a constant.
*/
    case SUBREG:
	i = reg_qty[REGNO (SUBREG_REG(x))];

	/* Return a constant or a constant register.  */
	if (REGNO_QTY_VALID_P (REGNO (SUBREG_REG(x)))
	    && qty_const[i] != 0
	    && (GET_CODE(qty_const[i])==CONST_INT))
	  {
	    rtx new = gen_lowpart_if_possible (GET_MODE (x), qty_const[i]);
	    if (new && GET_MODE(x)==SImode)
	      return new;
	  }
	else if(qty_const[i] && 
		GET_CODE(qty_const[i])==CONST &&
		GET_CODE(XEXP(qty_const[i],0))==CONST_INT)
	  {
	    rtx new = gen_lowpart_if_possible (GET_MODE (x), XEXP(qty_const[i],0));
	    if (new && GET_MODE(x)==SImode)
	      return new;
	  }
	break;
...
char *cse_pathlength_str = 0;
int   cse_pathlength = 20;
     ...
     FILE *file;
{
 ...
 /* Dynamically allocate val.path so that we may have arbitrary pathlength. */
  struct cse_basic_block_data  val;
  {
    int pathlength;
    if(cse_pathlength_str &&
       sscanf(cse_pathlength_str,"%d", &pathlength) == 1)
      cse_pathlength = pathlength;
  }
  val.path = (struct branch_path *)alloca(sizeof(struct branch_path)*cse_pathlength);
...
#else
  uid_cuid = (int *) alloca (2*(max_uid + 1) * sizeof (int));
  bzero (uid_cuid, 2*(max_uid + 1) * sizeof (int));
#endif
