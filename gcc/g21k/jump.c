...
#endif
#if !defined(ADI) || (defined(ADI) && !defined(A21C0))
/*
 * Because PRESERVE_DEATH_INFO_REGNO_P does not work, in general,
 * I put this #if to get accurate death-notes for final.
 * -- lev 04/16/95
 */
		      {
	...
	if (p != lastexit)
	  {
	    ...
	    reg_map[REGNO (reg)] = gen_reg_rtx (GET_MODE (reg));
            /*
             * Copy any information we had about this thing being a pointer
             */
	    if (REGNO_POINTER_FLAG (REGNO (reg))) {
	      REGNO_POINTER_FLAG (REGNO (reg_map[REGNO (reg)])) = 1;
	      RTX_MARK_ISPOINTER (reg_map[REGNO (reg)]);
	    }
	  }
      }
