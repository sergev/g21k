...
/* This is for use in print_operand */
rtx final_insn;
...

#ifndef LISTING_WITH_SOURCE
#define LISTING_WITH_SOURCE 0
#endif
...
#endif
	{
	  ...
	  
	  {
	    ...
	    for (i=0;i<stream->max_line - stream->old_max_line;i++)
	      stream->line_loc_in_file[stream->old_max_line + 1 +i] = 0;
	    /* mmh: something screwy..???
	       bzero (&stream->line_loc_in_file[stream->old_max_line+1],
     ...
     int nopeepholes;
{
  ...
  switch (GET_CODE (insn))
	  ...
	  && GET_CODE (NEXT_INSN (insn)) == JUMP_INSN)
	{
      ...
      ASM_OUTPUT_INTERNAL_LABEL (file, "L", CODE_LABEL_NUMBER (insn));
      if (CODE_LABEL_NAME (insn))
	ASM_OUTPUT_LABEL_NAME (file, CODE_LABEL_NAME (insn));
      break;
  ...
  for(stream=first_stream->next; stream; stream=stream->next)
    {
      if (stream->file)
	{
	  if (stream->last_access < earliest_time)
	    {
