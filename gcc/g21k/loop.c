...
#ifdef ADI
  /* We need more room because we will be renumbering the uids
     after every pass. */
     ...
     rtx loop_end;
{
  ...
  bzero ((char *) reg_map, max_reg_before_loop * sizeof (rtx));
    ...
    {
	void prepare_doloop();
	if (DODOLOOPS && flag_strength_reduce && ! flag_unroll_loops)
	  ...
	  if (comparison && bl->initial_value == const0_rtx
		  ...
		  % INTVAL (bl->biv->add_val)) == 0)
	    {
		 ...
		 dead store by flow.c.  */
	      p = emit_insn_before (gen_move_insn (reg, start_value),
				    loop_start);

#ifdef ADI
	      /* Update biv info to reflect its new status.  */
	      /* Update biv info to reflect its new status.  */
	      /* This is used in iteration_info() for ADI. pr720 -- lev */
	      bl->init_insn = p;
	      bl->init_set = PATTERN(p);
#endif
