     ...
     int nregs;
{
  ...
  for (i = 0; i < N_REG_CLASSES; i++)
    ...
    || (SECONDARY_INPUT_RELOAD_CLASS (BASE_REG_CLASS1 (m, REG_NONE), m, r)
			  != NO_REGS)
#endif
#ifdef SECONDARY_OUTPUT_RELOAD_CLASS
  || (SECONDARY_OUTPUT_RELOAD_CLASS (BASE_REG_CLASS1 (m, REG_NONE), m, r)
			  != NO_REGS)
#endif
		      )
		  ...
		  if (set != 0 && GET_CODE (SET_DEST (set)) == REG
			...
			-= (MEMORY_MOVE_COST (GET_MODE (SET_DEST (set)))
		      ...
		      record_address_regs (XEXP (SET_SRC (set), 0),
		       BASE_REG_CLASS1 (Extract_rtx_Pmode (SET_SRC (set)), REG_NONE),
					   loop_cost * 2);
		      continue;
		    }
	      ...
	      for (i = 0; i < noperands; i++)
		{
		  ...
		  if (GET_CODE (recog_operand[i]) == MEM)
		    record_address_regs (XEXP (recog_operand[i], 0),
		     BASE_REG_CLASS1 
			(Extract_rtx_Pmode (XEXP (recog_operand[i], 0)), REG_NONE),
					 loop_cost * 2);
		  else if (constraints[i][0] == 'p')
		    record_address_regs (recog_operand[i],
		     BASE_REG_CLASS1 (Extract_rtx_Pmode (recog_operand[i]), REG_NONE),
					 loop_cost * 2);
		}
...
TARGET_EXTRA_CONSTRAINT_LETTERS
		...
		break;
            ...
            case 'x': case 'z':
                /*
                 * Discourage DM and PM M registers from getting pointer
                 * values as long as HW does'nt do pre-indexed addressing
                 * correctly
                 */
                if (REG_P (op) && REGNO_POINTER_FLAG (REGNO (op)))
                    break;
                else if (GET_CODE (op) == SUBREG && REGNO_POINTER_FLAG (SUBREGNO (op)))
                    break;
                /* FALL THRU */
#endif
     ...
     int scale;
{
  ...
  switch (code)
	...
	enum machine_mode mode0 = GET_MODE (arg0);
	enum machine_mode mode1 = GET_MODE (arg1);
	int reg0;
	int reg1;
	...
	else if (code1 == REG && INDEX_REG_CLASS1 (mode0, REGNO(arg1)) == 
		BASE_REG_CLASS1 (mode0, REGNO(arg1)))
	  {
	    record_address_regs (arg0, class, scale);
	    if (! CONSTANT_P (arg1))
	...
	/* If the second operand is a symbolic constant, the first operand
	  ...
	  record_address_regs(arg0,INDEX_REG_CLASS1(mode0, REG_NONE), scale);
	...
	/* If this the sum of two registers where the first is known to be a 
	  ...
	  {
	    record_address_regs(arg0,BASE_REG_CLASS1(mode0,reg1),scale*2);
	    record_address_regs(arg1,INDEX_REG_CLASS1(mode1,reg0),scale);
	  }
	...
	/* If this is the sum of two registers and neither is known to
	   ...
	   or index register.  This case should be rare.  */
		 ...
		 && ! REGNO_POINTER_FLAG (REGNO (arg1)))
	  {
	    record_address_regs (arg0, BASE_REG_CLASS1 (mode0, reg1), 
			scale / 2);
	    record_address_regs (arg0, INDEX_REG_CLASS1 (mode0, reg1), 
			scale / 2);
	    record_address_regs (arg1, BASE_REG_CLASS1 (mode1, reg1), 
			scale / 2);
	    record_address_regs (arg1, INDEX_REG_CLASS1 (mode1, reg0), 
			scale / 2);
	  }
	...
	/* In all other cases, the first operand is an index and the
	...
	else
	  {
	    record_address_regs (arg0, INDEX_REG_CLASS1 (mode0, reg1), 
			scale);
	    record_address_regs (arg1, BASE_REG_CLASS1 (mode1, reg0), 
			scale);
	  }
      }
      break;
...
#if defined (DSP21K)
    base_class = BASE_REG_CLASS1 (mode, mod_regnum);
    mod_class  = INDEX_REG_CLASS1 (mode, base_regnum);
#endif
#if defined (DSP21XX) || defined (A21C0)
    mod_class  = MODIFY_REG_CLASS (mode, base_regnum);
#endif
...
*/
	...
	if (IS_DAG1_IREG (REGNO (base))
	    || (REG_P (modifier) && IS_DAG1_MREG (REGNO (modifier))))
	  {
	    base_class = DAG1_IREGS;
	    mod_class  = DAG1_MREGS;
	  }
	else if (IS_DAG2_IREG (REGNO (base)) 
		 || (REG_P (modifier) && IS_DAG2_MREG (REGNO (modifier))))
	  {
	...
	bsp->mem_cost += (MEMORY_MOVE_COST (mode) 
			  * bcost * (loop_depth+1)) /2;
	for (i=0; i < N_REG_CLASSES; i++)
	  bsp->cost[i] += (may_move_cost[i][(int) base_class] 
			   * bcost *(loop_depth+1)) /2;
      }
    if (msp 
	&& mod_class  != NO_REGS 
	...
	msp->mem_cost += (MEMORY_MOVE_COST (mode) 
			  * mcost * (loop_depth+1)) /2;
	for (i=0; i < N_REG_CLASSES; i++)
	  msp->cost[i] += (may_move_cost[i][(int) mod_class] 
			   * mcost * (loop_depth+1)) /2;
      }
}
#endif	/* ifdef ADI */
