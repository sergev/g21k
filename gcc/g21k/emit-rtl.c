/*
 * $Source: /home/gcc/CORETOOLS/gcc/emit-rtl.c,v $
 * $Revision: 1.20 $ $Date: 1997/03/19 22:07:11 $
 * $Author: yung $
*/
...
rtx stack_pointer_rtx;		/* (REG:Pmode1 STACK_POINTER_REGNUM) */
rtx frame_pointer_rtx;		/* (REG:Pmode1 FRAME_POINTER_REGNUM) */
rtx arg_pointer_rtx;		/* (REG:Pmode1 ARG_POINTER_REGNUM) */
rtx struct_value_rtx;		/* (REG:Pmode1 STRUCT_VALUE_REGNUM) */
rtx struct_value_incoming_rtx;	/* (REG:Pmode1 STRUCT_VALUE_INCOMING_REGNUM) */
rtx static_chain_rtx;		/* (REG:Pmode1 STATIC_CHAIN_REGNUM) */
rtx static_chain_incoming_rtx;	/* (REG:Pmode1 STATIC_CHAIN_INCOMING_REGNUM) */
rtx pic_offset_table_rtx;	/* (REG:Pmode1 PIC_OFFSET_TABLE_REGNUM) */

rtx virtual_incoming_args_rtx;	/* (REG:Pmode1 VIRTUAL_INCOMING_ARGS_REGNUM) */
rtx virtual_stack_vars_rtx;	/* (REG:Pmode1 VIRTUAL_STACK_VARS_REGNUM) */
rtx virtual_stack_dynamic_rtx;	/* (REG:Pmode1 VIRTUAL_STACK_DYNAMIC_REGNUM) */
rtx virtual_outgoing_args_rtx;	/* (REG:Pmode1 VIRTUAL_OUTGOING_ARGS_REGNUM) */
  ...
  if (code == CONST_INT)
    {
      ...
      /*** We do NOT want to reuse constants which are used ***/
      /*** as pointers -- the mode might be different      ***/
...
#if defined(ADI) && defined(MEMSEG)
      /*** In case of memory segments, we want to set the mode in  ***/
      /*** cases when it is not void -- those are the cases when   ***/
      /*** an integer constant is used as a pointer -- we need the ***/
      /*** mode to figure out which segment it points to.          ***/
      if (mode != VOIDmode)
	  GET_MODE(rt_val) = mode;
#endif
...
{
  if (reg) {
    RTX_MARK_ISPOINTER (reg);
    if (regno_pointer_flag && REG_P (reg))
...
/* The case we're trying to handle here is
  ...
  if(mode==SFmode &&
    ...
    {
      rtx zero,one;
      double_const_to_singles(op, &zero, &one);
      switch(i)
	{
	case 0:
	  return zero;
	case 1:
	  return one;
	default:
	  abort(); /* why are we asking for a subword other than zero or one? */
	}
    }
#endif
     ...
     rtx orig;
{
  ...
  switch (code)
	...
	mark_reg_pointer (XEXP (XEXP (x, 0), 0));
...
init_emit ()
{
  ...
  /* Indicate that the virtual registers and stack locations are
     all pointers.  */
#if 0
  /* These should all be obsolete now */
...
#endif
  mark_reg_pointer (stack_pointer_rtx);
  mark_reg_pointer (frame_pointer_rtx);
  mark_reg_pointer (arg_pointer_rtx);
  mark_reg_pointer (virtual_incoming_args_rtx);
  mark_reg_pointer (virtual_outgoing_args_rtx);
  mark_reg_pointer (virtual_stack_dynamic_rtx);
  mark_reg_pointer (virtual_stack_vars_rtx);
}
     ...
     int line_numbers;
{
...
#if defined(ADI) && defined(MEMSEG)
      const_tiny_rtx[i][DMmode] = gen_rtx (CONST_INT, DMmode, i);
      const_tiny_rtx[i][PMmode] = gen_rtx (CONST_INT, PMmode, i);
#endif
  ...
  stack_pmode = PMSTACK ? PMmode : DMmode;
  stack_pointer_rtx = gen_rtx (REG, stack_pmode, STACK_POINTER_REGNUM);
  frame_pointer_rtx = gen_rtx (REG, stack_pmode, FRAME_POINTER_REGNUM);
  ...
  if (FRAME_POINTER_REGNUM == ARG_POINTER_REGNUM)
    ...
    arg_pointer_rtx = gen_rtx (REG, stack_pmode, ARG_POINTER_REGNUM);
  ...
  virtual_incoming_args_rtx = gen_rtx (REG, stack_pmode,
				       VIRTUAL_INCOMING_ARGS_REGNUM);
  virtual_stack_vars_rtx = gen_rtx (REG, stack_pmode,
				    VIRTUAL_STACK_VARS_REGNUM);
  virtual_stack_dynamic_rtx = gen_rtx (REG, stack_pmode,
				       VIRTUAL_STACK_DYNAMIC_REGNUM);
  virtual_outgoing_args_rtx = gen_rtx (REG, stack_pmode,
  ...
  struct_value_incoming_rtx
    = gen_rtx (REG, Pmode1 (DEFAULT_DATA_Pmode), STRUCT_VALUE_INCOMING_REGNUM);
#else
  struct_value_incoming_rtx = struct_value_rtx;
#endif
#endif
  mark_reg_pointer (struct_value_incoming_rtx);



#ifdef STATIC_CHAIN_REGNUM
  static_chain_rtx = gen_rtx (REG, Pmode1 (DEFAULT_DATA_Pmode), STATIC_CHAIN_REGNUM);

#ifdef STATIC_CHAIN_INCOMING_REGNUM
  if (STATIC_CHAIN_INCOMING_REGNUM != STATIC_CHAIN_REGNUM)
    static_chain_incoming_rtx = gen_rtx (REG, Pmode1 (DEFAULT_DATA_Pmode), STATIC_CHAIN_INCOMING_REGNUM);
  else
#endif
  ...
  static_chain_incoming_rtx = static_chain_rtx;
#endif
  mark_reg_pointer (static_chain_rtx);
  mark_reg_pointer (static_chain_incoming_rtx);
#endif

#ifdef PIC_OFFSET_TABLE_REGNUM
  pic_offset_table_rtx = gen_rtx (REG, Pmode1 (DEFAULT_DATA_Pmode), PIC_OFFSET_TABLE_REGNUM);
  mark_reg_pointer (pic_offset_table_rtx);
#endif
