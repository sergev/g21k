...
#include "tree.h"
#ifdef ADI
/* this is for plain type. and the computation of coff type
   information */
#include "c-tree.h"
#endif
#include "rtl.h"
...
#ifdef ADI
	/* If word size on the target machine is 
	   the same for char's short's etc. */

	/* Having what A21c0 has in the following statements caused the
	   21k CBUG to crash.  I have changed it back to what it had
	   before.  AS */
  ...
  switch (TREE_CODE (decl))
	  ...
	  && GET_CODE (XEXP (value, 0)) == SYMBOL_REF)
	{
	  ...
	  if (TREE_PUBLIC (decl))
	    ...
	    {
#ifdef CBUGSUPPORT
	      PUT_SDB_STATIC_DEF(XEXP (value, 0)); /* This is because Cbug wants to unmangle names.... */
#endif
	      PUT_SDB_VAL (XEXP (value, 0));
              PUT_SDB_SCL (C_STAT);
	    }
  ...
  for (; parms; parms = TREE_CHAIN (parms))
	...
	/* Report parms that live in memory but not where they were passed.  */
	  ...
	  if(GET_CODE(DECL_RTL(parms)) != REG
		...
		name=gen_fake_label();
	      PUT_SDB_DEF (name);
	      PUT_SDB_INT_VAL (REGNO(DECL_INCOMING_RTL(parms)));
	      PUT_SDB_SCL (C_REGPARM);
	      PUT_SDB_TYPE (plain_type (TREE_TYPE (parms), 0));
	      PUT_SDB_ENDEF;
	    }
     ...
     int line;
{
  ...
  MAKE_LINE_SAFE (line);
#ifdef CBUGSUPPORT
  /* Cbug used the absolute linenumber information at this point */
