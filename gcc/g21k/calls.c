...
#endif
	  funexp = force_reg (Pmode1 (Extract_rtx_Pmode (funexp)), funexp);
#endif
    }
     ...
     int ignore;
{
...
#ifdef ADI
	    if (GET_MODE(structure_value_addr) != DMmode)
	    {
		error("Compiler Restriction:");
		error(" Structure values can be returned to Data Memory only");
	    }
#endif
	}
    }
  ...
  if (structure_value_addr && struct_value_rtx == 0)
    {
	 ...
	 does not refer to virtual_outgoing_args_rtx.  */
      rtx temp = (stack_arg_under_construction
		  ...
		  : force_reg (Pmode1 (Extract_rtx_Pmode 
				      (structure_value_addr)), 
			       structure_value_addr));
#else
      rtx temp = force_reg (Pmode1 (Extract_rtx_Pmode (structure_value_addr)), 
			    structure_value_addr);
#endif
      actparms
...
#ifdef ADI
      /* No one knows why this changed > -- >= ? mmh:3/26/93 */
  ...
  if (argblock)
      ...
      for (i = 0; i < num_actuals; i++)
	{
	  ...
	  if (GET_CODE (offset) == CONST_INT)
	    ...
	    addr = gen_rtx (PLUS, Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)), 
			    arg_reg, offset);
  ...
  if (structure_value_addr && ! structure_value_addr_parm)
    {
      emit_move_insn (struct_value_rtx,
		      force_reg (Pmode1 (Extract_rtx_Pmode (structure_value_addr)),
  ...
  funexp = prepare_call_address (funexp, fndecl, &use_insns);
      ...
      POLYMORPH_LIBRARY_CALLS(funexp,exp);
#endif

#ifdef DSP21K
{
    extern void emit_epilogue_profiler();
    if (PROFILE)
	emit_epilogue_profiler(0);
}
#endif
...
#ifdef DSP21K
{
    extern void emit_prologue_profiler();
    if (PROFILE)
	emit_prologue_profiler(0);
}
#endif
  ...
  /* If we are returning a pointer then we mark the reg as a
     pointer reg. */
  ...
  if (GET_CODE (offset_rtx) == CONST_INT)
  ...
  else
    {
	 ...
	 will work in the presence of register parameters.  */
      target = gen_rtx (PLUS, Pmode1 (Extract_rtx_Pmode (args_addr)), 
			args_addr, offset_rtx);
