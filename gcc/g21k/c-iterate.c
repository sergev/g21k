...
{
  if (DECL_INITIAL (idecl) == 0
      || TREE_CODE (DECL_INITIAL (idecl)) == ERROR_MARK)
    {
      fatal ("Compiler Error! There is no initializer for iterator %s.", 
	     IDENTIFIER_POINTER (DECL_NAME (idecl)));
    }
     ...
     rtx insn;
{
  if (GET_CODE (insn) == BARRIER)
    ...
    {
      /* here we don't want to use delete_insn because this is
	 run before we hit the optimizer and code will be deleted
	 that is not supposed to be deleted. */
      PUT_CODE (insn, NOTE);
