...
#if defined(ADI) && defined(MEMSEG)
  TREE_MEMSEG(TREE_TYPE(value)) = default_memory_segment;
#endif
...
#ifdef ADI
#ifndef INSTALL_FUNCTION_AS_FARCALL
#define INSTALL_FUNCTION_AS_FARCALL(d)
#endif
    if (TREE_VALUE (a) != 0 &&
	TREE_CODE (TREE_VALUE (a))== TREE_LIST &&
	TREE_PURPOSE (TREE_VALUE (a)) == get_identifier ("segment"))
      {
	tree seg_id = TREE_VALUE (TREE_VALUE (a));

	if ((TREE_STATIC (decl) || DECL_EXTERNAL (decl)) && ! DECL_REGISTER (decl))
	  PUT_SEG_ATTR (decl, seg_id);
	else
	  error ("locals or global registers can't be placed in segments");
      }
    else if (TREE_VALUE (a) == get_identifier ("farcall"))
      {
	INSTALL_FUNCTION_AS_FARCALL (decl);
      }
    else
#endif
...
#if defined(ADI) && defined(MEMSEG)
  if (mode == PMmode)
      return build_pointer_type (build_type_variant (integer_type_node,
						     0, 0, PROG_MEM));
     ...
     tree expr;
{
  ...
  if (TREE_CODE (TREE_TYPE (expr)) == COMPLEX_TYPE)
    error ("Illegal use of complex number as conditional");
