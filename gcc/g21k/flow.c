/*
 * $Source: /home/gcc/CORETOOLS/gcc/flow.c,v $
 * $Revision: 1.25 $ $Date: 1996/04/22 17:07:02 $
 * $Author: mmh $
*/
     ...
     FILE *file;
{
  ...
  /** more of them. */
  max_uid_for_flow += max_uid_for_flow / 3;
#endif
...
#ifdef HAVE_POST_MODIFY
      /**** Depending on the situation, conversion to POST-MODIFY involves
    ...
    rtx *p_incr_insn, *p_incr_val, *p_modifier;
{
    ...
    if (BLOCK_NUM (incr_insn) != BLOCK_NUM (insn)) 
      return IMPOSSIBLE;
	...
	|| (GET_CODE (incr_plus = SET_SRC (incr_pat)) != PLUS)) 
      return IMPOSSIBLE;
    ...
    if (GET_CODE (mreg) == REG) 
      {
	if (reg_set_between_p (mreg, PREV_INSN (insn), incr_insn))
	  return IMPOSSIBLE;
      }

    else if (! CONST_OK_FOR_AUTO_MODIFY (mreg))
      return IMPOSSIBLE;
     ...
     rtx insn, memref, new_addr, modifier;
{
  ...
  /* see (query-pr "1037 --full") where I explain why this is disabled for the CSP */
  ...
  if (CONSTANT_P (modifier))
	...
	modifier = gen_rtx (REG, GET_MODE (modifier), 
			    PLUS1_MREG (GET_MODE(modifier)));
#endif
