...
init_expr_once ()
{
  ...
  start_sequence ();
  insn = emit_insn (gen_rtx (SET, 0, 0, NULL_RTX));
  pat = PATTERN (insn);
     ...
     int unsignedp;
{
  ...
  if (from_mode == SImode && to_mode == DMmode)
    {
#ifdef HAVE_truncsidm2
      if (HAVE_truncsidm2)
	{
	  emit_unop_insn (CODE_FOR_truncsidm2, to, from, UNKNOWN);
	  return;
	}
#endif
    }

  if (from_mode == SImode && to_mode == PMmode)
    {
#ifdef HAVE_truncsipm2
      if (HAVE_truncsipm2)
	{
	  emit_unop_insn (CODE_FOR_truncsipm2, to, from, UNKNOWN);
	  return;
	}
#endif
    }

  if (from_mode == PMmode && to_mode == SImode)
    {
#ifdef HAVE_extendpmsi
      if (HAVE_extendpmsi)
	{
	  emit_unop_insn (CODE_FOR_extendpmsi, to, from, UNKNOWN);
	  return;
	}
#endif
    }

  if (from_mode == DMmode && to_mode == SImode)
    {
#ifdef HAVE_extenddmsi
      if (HAVE_extenddmsi)
	{
	  emit_unop_insn (CODE_FOR_extenddmsi, to, from, UNKNOWN);
	  return;
	}
#endif
    }

  /*** We have to allow conversions between PMmode and DMmode ***/
  /*** to allow the user to do casting ***/

  if (from_mode == DMmode && to_mode == PMmode)
    {
#ifdef HAVE_truncdmpm2
      if (HAVE_truncdmpm2)
	{
	  emit_unop_insn (CODE_FOR_truncdmpm2, to, from, UNKNOWN);
	  return;
	}
#endif
    }

  if (from_mode == PMmode && to_mode == DMmode)
    {
#ifdef HAVE_extendpmdm
      if (HAVE_extendpmdm)
	{
	  emit_unop_insn (CODE_FOR_extendpmdm, to, from, UNKNOWN);
	  return;
	}
#endif
    }
     ...
     int unsignedp;
{
  ...
  /* We can do this with a gen_lowpart if both desired and current modes
     ...
     the operand.  */
#if 0
  /* Although the inclusion of this makes sence
     it create superflous moves that don't make any sence. */
#ifdef ADI
  /* If we are using a single integer in a place where we need
     something in PMmode or DMmode then.  Create a new register
     and copy it. pr:1298 */
...
#if defined (DSP21XX) || defined (A21C0)
       || GET_CODE (from_addr) == POST_MODIFY
#endif
       );
...
#ifdef ADI
/*
 * On 21csp (A21C0) we do not use this code because if post_modify
 * is used inside a loop, the post modified reg has wrong value
 * after the first loop iteration. PR 1138. Test bugs/ck_pointer.c.
 * I think that the same problem exists on 21xx.
 * 07/17/96 -- lev
 */
#if (defined (DSP21XX) && 0)
      if( !data.autinc_from)
	{
	  rtx temp=gen_reg_rtx(GET_MODE(from_addr));
	  if(mod_reg_equiv_size < REGNO(temp)+1)
	    {
	      mod_reg_equiv_const=(int *)realloc(mod_reg_equiv_const,(1+REGNO(temp))*sizeof(int));
	      if(!mod_reg_equiv_const)
		abort();
	      bzero(&mod_reg_equiv_const[mod_reg_equiv_size],
		    (REGNO(temp)-mod_reg_equiv_size)*sizeof(int));
	      mod_reg_equiv_size=REGNO(temp);
	    }
	  if(data.reverse)
	    {
	      emit_insn (gen_rtx (SET,GET_MODE(from_addr),temp,constm1_rtx));
	      mod_reg_equiv_const[REGNO(temp)]=-1;
	      data.from_addr=copy_addr_to_reg(plus_constant(from_addr,len-1));
	    }
	  else
	    {
	      emit_insn (gen_rtx (SET,GET_MODE(from_addr),temp,const1_rtx));
	      mod_reg_equiv_const[REGNO(temp)]=1;
     ...
     struct move_by_pieces *data;
{
  ...
  while (data->len >= size)
	...
	emit_insn (gen_add2_insn (data->from_addr,
				  gen_rtx (CONST_INT, VOIDmode, size*data->explicit_inc_from)));
#endif
  ...
  if (GET_CODE (size) == CONST_INT
	   ...
	   mode = GET_MODE_WIDER_MODE (mode))
	{
      ...
      emit_library_call (bcopy_libfunc, 0,
			 VOIDmode, 3, XEXP (y, 0), Pmode1 (Extract_rtx_Pmode (XEXP (x,0))),
     ...
     rtx x, y;
{
  ...
  if (mode == BLKmode || (GET_MODE (y) != mode && GET_MODE (y) != VOIDmode)){
      convert_move(x, y, 0); /* mmh: (well if were gonna fail try this..) abort (); */
     ...
     int extra, below;
{
  ...
  if (CONSTANT_P (size))
    ...
    {
      rtx temp = copy_to_mode_reg (Pmode1 (DEFAULT_DATA_Pmode), size);
      if (extra != 0)
	temp = expand_binop (Pmode1 (DEFAULT_DATA_Pmode), add_optab,
			     temp,
			     GEN_INT (extra),
  ...
  if (GET_CODE (size) == CONST_INT)
    ...
    temp = gen_rtx (PLUS, Pmode1 (DEFAULT_DATA_Pmode), virtual_outgoing_args_rtx,
		    negate_rtx (Pmode1 (DEFAULT_DATA_Pmode), plus_constant (size, extra)));
  else
    temp = gen_rtx (PLUS, Pmode1 (DEFAULT_DATA_Pmode), virtual_outgoing_args_rtx,
		    negate_rtx (Pmode1 (DEFAULT_DATA_Pmode), size));
#endif
...
gen_push_operand ()
{
  enum machine_mode mode = Extract_rtx_Pmode(stack_pointer_rtx);
#if defined (DSP21XX) || defined (A21C0)
  {
    rtx temp=gen_reg_rtx (mode);
    emit_insn (gen_rtx (SET,mode,temp,constm1_rtx));
    if(mod_reg_equiv_size < REGNO(temp)+1)
      {
	mod_reg_equiv_const=(int *)realloc(mod_reg_equiv_const,(1+REGNO(temp))*sizeof(int));
	if(!mod_reg_equiv_const)
    ...
    mod_reg_equiv_const[REGNO(temp)]=-1;
    return gen_rtx (POST_MODIFY, mode, stack_pointer_rtx,temp);
  }
#else
  return gen_rtx (STACK_PUSH_CODE, mode, stack_pointer_rtx);
#endif
}
  ...
  if (temp != target && TREE_CODE (exp) != ERROR_MARK)
      ...
      else if (GET_MODE (temp) == BLKmode && TREE_CODE (exp) == STRING_CST)
	{
	  ...
	  if (GET_CODE (size) == CONST_INT
		...
		{
		  enum machine_mode size_mode = Pmode1(Extract_tree_Pmode(target));

		  temp = force_reg (Pmode1(Extract_tree_Pmode(target)), XEXP (target, 0));
...
/* Try to generate multiply accumulate if HAVE_macsi3 or HAVE_smacsi3 
   ...
   These functions are not to be used for non mixed mode operations,
   Thoose can easily be recognized by the combiner phase of the compiler.

   EXP    - The SUM tree node
   TARGET - The target rtx operand
   CODE   - Either PLUS or MINUS

   This function returns the target of the mac if a mac can be created
   otherwise NULL_RTX is returned.
*/

rtx
expand_multiply_accumulate (exp, target, code)
     tree exp;
     rtx  target;
     RTX_CODE code;
{
  ...
  /* Added special macsi3. for DSP architectures mmh (March 24, 1994) */
  if (TREE_CODE (TREE_TYPE (exp)) == INTEGER_TYPE
      && (TREE_CODE (TREE_OPERAND (exp, 1)) == MULT_EXPR
	  || (TREE_CODE (TREE_OPERAND (exp, 1)) == NOP_EXPR
	      && TREE_CODE (TREE_OPERAND (TREE_OPERAND (exp, 1), 0)) == MULT_EXPR
	...
	x0 = TREE_OPERAND (TREE_OPERAND (exp, 1), 0);
	x1 = TREE_OPERAND (TREE_OPERAND (exp, 1), 1);
      }

      /* If we are not multiplying to SI quantities return */
      if (TYPE_MODE (TREE_TYPE (x0)) != SImode
	  || TYPE_MODE (TREE_TYPE (x1)) != SImode)
	return NULL_RTX;

      op0 = expand_expr (x0, NULL_RTX, VOIDmode, 0);
	...
	rout = target = force_reg (TYPE_MODE (TREE_TYPE (sum)), rout);
      ...
      else if (target == NULL_RTX)
	target = gen_reg_rtx (TYPE_MODE (TREE_TYPE (sum)));

      if (code == PLUS)
	mac = gen_macdisi3 (target,
			    rout,
			    force_reg (SImode, op0),
			    force_reg (SImode, op1));
      else if (code == MINUS)
	mac = gen_smacdisi3 (target,
			     rout,
			     force_reg (SImode, op0),
			     force_reg (SImode, op1));
      else
...
rtx
expand_expr_1 (exp, target, tmode, modifier)
     register tree exp;
     rtx target;
     enum machine_mode tmode;
     enum expand_modifier modifier;
{
  ...
  switch (code)
    ...
    case INTEGER_CST:
#ifdef ADI
      /* In case of   " *(int * pm) 100 " we have to keep */
      /* non-void mode on the constant: */
    {
      rtx c2, c1 = immed_double_const (TREE_INT_CST_LOW (exp),
				       TREE_INT_CST_HIGH (exp),
				       mode);
      if (PMODE_P (mode)) {
	  c1 = gen_rtx(CONST, mode, c1);
      }
      return c1;
    }
#else
...
#if defined(ADI) && defined(MEMSEG)
	    && PMODE_P (TYPE_MODE (TREE_TYPE (exp1)))
	    && PMODE_P (TYPE_MODE (TREE_TYPE (exp2)))
#else
	    && TYPE_MODE (TREE_TYPE (exp1)) == Pmode1
	    && TYPE_MODE (TREE_TYPE (exp2)) == Pmode1
#endif
	    )
          {
...
#endif
	/* If this is a pointer thingy then we need to mark its
	   register as a pointer thingy. */
...
#if defined(ADI) && defined(MEMSEG)
	  /*** We do not want multiplication of type pointer -- we ***/
	  ...
	  
	{
	    ...
	    elt = build1 (INDIRECT_REF, type,
			  fold (build (PLUS_EXPR,
				       ...
				       array_adr,
				       convert (TYPE_POINTER_TO (variant_type),
						offset))));
      }
#else
	  elt = build1 (INDIRECT_REF, type,
	      ...
	      || RTL_EXPR_RTL (TREE_OPERAND (exp, 1)) == arg_pointer_rtx))
	{
      ...
      /* If the result is to be Pmode1 and we are adding an integer to
	      ...
	      ))
	{
		   ...
		   ))
	  {
	    op0 = expand_expr (TREE_OPERAND (exp, 0), subtarget, VOIDmode,
			       EXPAND_SUM);
#ifdef MEMSEG
	    /* If op0 is not a pointer then it has no SEGEMENT
	       associated with it.  So then all subsequent arithmetic will
	       be in the mode of op0 which is probably SImode. */
	    if (TREE_CODE (TREE_TYPE (TREE_OPERAND (exp, 1))) == POINTER_TYPE
		&& TREE_CODE (TREE_TYPE (TREE_OPERAND (exp, 0))) != POINTER_TYPE)
	      {
		enum machine_mode mode = extract_tree_pmode (exp);
		op0 = force_reg (mode, op0);
		if (GET_MODE (op0) != mode)
		  op0 = gen_rtx (SUBREG, mode, op0, 0);
	      }
#endif

#ifdef DSP21K
	    /* -- This is to handle the following type of code
		     ((int *)(0x1111))[i]
		  We need to make force the pointer (0x1111) into a
		  register to keep it from being treated as an offset.
		  (On the SHARC pre 2.0, a[b] != b[a].) */

	    if (ADSP210Z3u
		&& TREE_CODE (TREE_TYPE (TREE_OPERAND (exp, 0)))
		         != POINTER_TYPE
		&& TREE_CODE (TREE_TYPE (TREE_OPERAND (exp, 1)))
		         == POINTER_TYPE
		&& TREE_CODE (TREE_OPERAND (exp, 1)) == INTEGER_CST) {
	      op1 = GEN_INT (TREE_INT_CST_LOW (TREE_OPERAND (exp, 1)));
...
#ifdef HAVE_smacdisi3
      if (optimize > 1
	  && (temp = expand_multiply_accumulate (exp, target, MINUS)))
	return temp;
#endif
...
#ifdef HAVE_avgsi3
      /* Added to handle the Avg. on the 21k mmh (March 24, 1994) */
      if (TREE_CONSTANT (TREE_OPERAND (exp, 1))
	  && TREE_INT_CST_LOW (TREE_OPERAND (exp, 1)) == 2
	  && TREE_INT_CST_HIGH (TREE_OPERAND (exp, 1)) == 0
	  && TREE_CODE (TREE_TYPE (exp)) == INTEGER_TYPE
	  && TREE_CODE (TREE_OPERAND (exp, 0)) == PLUS_EXPR)
	{
	  tree x0 = TREE_OPERAND (TREE_OPERAND (exp, 0), 0);
	  tree x1 = TREE_OPERAND (TREE_OPERAND (exp, 0), 1);
	  rtx avgrtx;
	  op0 = expand_expr (x0, NULL_RTX, VOIDmode, 0);
	  op1 = expand_expr (x1, NULL_RTX, VOIDmode, 0);
	  if (target == NULL_RTX)
...
/* Could determine the answer when only additive constants differ.
   Also, the addition of one can be handled by changing the condition.  */
	...
	op0 = force_reg (Pmode1 (Extract_rtx_Pmode (op0)), op0);
     ...
     enum expand_modifier modifier;
{
  ...
  rtx rval;
  rval = expand_expr_1 (exp, target, tmode, modifier);
     ...
     int ignore;
{
  ...
  switch (DECL_FUNCTION_CODE (fndecl))
	...
	if (!(TYPE_ARG_TYPES (fntype) != 0
      ...
      return expand_binop (Pmode1 (DEFAULT_DATA_Pmode), add_optab,
...
#endif
	      tem = memory_address (Pmode1 (Extract_rtx_Pmode (tem)), tem);
	      tem = copy_to_reg (gen_rtx (MEM, Pmode1 (Extract_rtx_Pmode (tem)), tem));
	    }
      ...
      else
	{
	  ...
	  src_rtx = memory_address (BLKmode,
				    expand_expr (src, NULL_RTX, Pmode1 (Extract_rtx_Pmode (src)),
						 EXPAND_NORMAL));
	  if (! (*insn_operand_predicate[(int)icode][1]) (src_rtx, Pmode1 (Extract_rtx_Pmode (src))))
	    src_rtx = copy_to_mode_reg (Pmode1 (Extract_rtx_Pmode (src)), src_rtx);
      ...
      else
	{
	  ...
	  dest_rtx = expand_expr (dest, NULL_RTX, Pmode1 (Extract_rtx_Pmode (dest)), EXPAND_NORMAL);
	  dest_mem = gen_rtx (MEM, BLKmode,
			      memory_address (BLKmode, dest_rtx));
	  src_mem = gen_rtx (MEM, BLKmode,
			     memory_address (BLKmode,
					     expand_expr (src, NULL_RTX,
							  Pmode1 (Extract_rtx_Pmode (src)),
							  EXPAND_NORMAL)));
     ...
     enum machine_mode mode;
{
  ...
  /* If index is in range, it must fit in Pmode1.
     Convert to Pmode1 so we can index with it.  */
  if (mode != Pmode1 (DEFAULT_TABLE_Pmode))
    index = convert_to_mode (Pmode1 (DEFAULT_TABLE_Pmode), index, 1);
  ...
  index = memory_address_noforce
    (CASE_VECTOR_MODE,
     gen_rtx (PLUS, Pmode1 (DEFAULT_TABLE_Pmode),
	      gen_rtx (MULT, Pmode1 (Extract_rtx_Pmode (index)), index,
		       GEN_INT (GET_MODE_SIZE (CASE_VECTOR_MODE))),
	      gen_rtx (LABEL_REF, Pmode1 (DEFAULT_TABLE_Pmode), table_label)));
