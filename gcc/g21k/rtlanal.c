...
reg_overlap_mentioned_p (x, in)
     rtx x, in;
{
  int regno, endregno=-1;

#ifdef ADI
  /* I put this stuff in here so that this function wouldn't abort() when it was given
     a strict low part.  Then I noticed that it assumed a strict low part.  So I fixed
     both at the same time. -jk 1/13/92
     */
  ...
  if (GET_CODE(x) == STRICT_LOW_PART)
	...
	{
	  regno= REGNO(SUBREG_REG(XEXP(x,0)));
	  regno +=SUBREG_WORD(XEXP(x,0));
	  endregno = regno;
	}
      else
	{
	  endregno = regno = REGNO (XEXP (x, 0));
	}
    } else
#endif
...
#ifdef ADI
	endregno = regno + HARD_REGNO_NREGS(regno, GET_MODE(SUBREG_REG(x)));
#else
	regno += SUBREG_WORD (x);
#endif
    }
