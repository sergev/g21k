     ...
     rtx structure_value_addr;
{
  ...
  if (TYPE_MODE (type) == VOIDmode)
	...
	{
	  temp = force_reg (Pmode1 (Extract_rtx_Pmode (structure_value_addr)), 
  ...
  switch (code)
      ...
      if (regno <= LAST_VIRTUAL_REGISTER)
	{
	  ...
	  if (regno == VIRTUAL_STACK_VARS_REGNUM)
		...
		= force_reg (Pmode1 (DEFAULT_DATA_Pmode), 
			     force_operand (loc, NULL_RTX));
      ...
      if (map->reg_map[regno] == NULL)
	{
	  map->reg_map[regno] = gen_reg_rtx (mode);
          REGNO_POINTER_FLAG (REGNO (map->reg_map[regno])) = REGNO_POINTER_FLAG (regno);
	  REG_USERVAR_P (map->reg_map[regno]) = REG_USERVAR_P (orig);
	  REG_LOOP_TEST_P (map->reg_map[regno]) = REG_LOOP_TEST_P (orig);
	  RTX_UNCHANGING_P (map->reg_map[regno]) = RTX_UNCHANGING_P (orig);
	  RTX_ISPOINTER_P (map->reg_map[regno]) = RTX_ISPOINTER_P (orig);
      ...
      if (CONSTANT_POOL_ADDRESS_P (orig))
	{
	  ...
	  if (GET_CODE (constant) == LABEL_REF)
		...
		= map->label_map[CODE_LABEL_NUMBER (XEXP (constant, 0))];
	      LABEL_OUTSIDE_LOOP_P (copy) = LABEL_OUTSIDE_LOOP_P (orig);
	      copy = force_const_mem (Pmode1 (Extract_rtx_Pmode (orig)), copy);
	      return XEXP (copy, 0);
	    }
	}
      return orig;
