...
#if defined (__WATCOMC__) || defined (EMX)
#define HAVE_CLOCK
#endif
...
extern void get_path_from_name(char *name);  /* defined in final.c */
#endif
...
int flag_software_loop_pipeline = 0;
...
struct { char *string; int *variable; int on_value;} f_options[] =
{
  ...
  {"software-loop-pipeline", &flag_software_loop_pipeline, 1},
  {"pic", &flag_pic, 1},
  {"PIC", &flag_pic, 2},
     ...
     unsigned size;
{
  ...
  if (value == 0)
    fatal ("malloc: virtual memory exhausted (%d)", size);
  return value;
}
     ...
     int size;
{
  ...
  if (!result)
    fatal ("realloc: virtual memory exhausted (%d)", size);
  return result;
}

...
#ifdef DEFAULT_SHORT_DOUBLES
  /* Initialize Doubles or FLOATS */
  {
    extern int flag_short_double;
    flag_short_double = DEFAULT_SHORT_DOUBLES;
  }
#endif
...
#ifdef INSN_SOFTWARE_LOOP_PIPELINING
      flag_software_loop_pipeline = 1;
#endif
#endif
    }
...
print_switch_values ()
{
  ...
  { extern int flag_short_double;
    if (flag_short_double)
      print_single_switch ("-f", "short-double");  }
