     ...
     register rtx x;
{
  if (GET_CODE (x) == MEM
	...
	x = gen_rtx (GET_CODE (x), Pmode1 (Extract_rtx_Pmode (op0)), op0, op1);
    }
  return x;
}
...
rtx
memory_address (mode, oldx)
     enum machine_mode mode;
     rtx oldx;
{
  register rtx x = oldx;
  /*
   * May need memory-bank info, so can't directly use Pmode
   */
  enum machine_mode Bmode = Pmode1 (Extract_mode_Pmode (x,mode));
  ...
  /* We get better cse by rejecting indirect addressing at this stage.
    ...
    win2:
      x = oldx;
    win:
      Bmode = Pmode1 (Extract_rtx_Pmode (x));
     ...
     rtx x;
{
  ...
  if (REG_P (x) 
      && (RTX_ISPOINTER_P (x)
	  || (regno_pointer_flag && REGNO_POINTER_FLAG (REGNO (x)))))
    mark_reg_pointer (temp);

  return temp;
}

/* Like copy_to_reg but always give the new register mode Pmode1
...
copy_addr_to_reg (x)
     rtx x;
{
  return copy_to_mode_reg (Pmode1 (Extract_rtx_Pmode (x)), x);
}
     ...
     rtx x;
{
...
#if defined(ADI) || defined(MEMSEG)
  {
    ...
    /*** We assign temp to x to avoid extra move generated **/
    /*** at the end of this function ***/
#ifdef HAVE_truncsipm2    
    if (HAVE_truncsipm2 && xmode == SImode && mode == PMmode) 
      {
	rtx si=force_reg(SImode,x);
	temp = x = gen_rtx(SUBREG,PMmode,si,0);
      }
#endif
#ifdef HAVE_truncsidm2    
    if (HAVE_truncsidm2 && xmode == SImode && mode == DMmode) 
      {
	rtx si=force_reg(SImode,x);
	temp = x = gen_rtx(SUBREG,DMmode,si,0);
      }
#endif
#ifdef HAVE_extendpmsi    
    if (HAVE_extendpmsi && xmode == PMmode && mode == SImode) 
      {
	rtx pm=force_reg(PMmode,x);
	x = temp = gen_rtx(SUBREG,SImode,pm,0);
      }
#endif
#ifdef HAVE_extenddmsi    
    if (HAVE_extenddmsi && xmode == DMmode && mode == SImode) 
      {
	rtx dm=force_reg(DMmode,x);
	x = temp = gen_rtx(SUBREG,SImode,dm,0);
      }
#endif
#ifdef HAVE_extendpmdm
    if (HAVE_extendpmdm && xmode == PMmode && mode == DMmode) 
      {
	rtx pm=force_reg(PMmode,x);
	x=temp=gen_rtx(SUBREG,DMmode,pm,0);
      }
#endif
#ifdef HAVE_truncdmpm2    
    if (HAVE_truncdmpm2 && xmode == DMmode && mode == PMmode) 
      {
	rtx dm=force_reg(DMmode,x);
	temp = x = gen_rtx(SUBREG,PMmode,dm,0);
      }
#endif

#if (defined DSP21XX) || ! defined (MEMSEG)
      /*** This code will go away once 21xx switches to pm/dm **/
      
      if(GET_MODE(x)==SImode && mode==PSImode)
	{
	  rtx si=force_reg(SImode,x);
	  emit_insn(gen_rtx(SET,PSImode,temp,gen_rtx(TRUNCATE,PSImode,si)));
	  x=temp;
	}
      else if(GET_MODE(x)==PSImode && mode==SImode)
	{
	  rtx psi=force_reg(PSImode,x);
	  emit_insn(gen_rtx(SET,SImode,temp,gen_rtx(SIGN_EXTEND,SImode,psi)));
	  x=temp;
	}
#endif
  }
     ...
     rtx adjust;
{
  ...
  temp = expand_binop (Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)),
     ...
     rtx size;
{
  ...
  if (GET_CODE (size) == CONST_INT)
    ...
    {
      size = expand_divmod (0, CEIL_DIV_EXPR, 
			    Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)), size,
			    GEN_INT (align),
			    NULL_RTX, 1);
      size = expand_mult (Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)), size,
...
{
  rtx sa = *psave;
  /* The default is that we use a move insn and save in a Pmode1 object.  */
  rtx (*fcn) () = gen_move_insn;
  enum machine_mode mode = Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx));
     ...
     int known_align;
{
  ...
  if (GET_MODE (size) != VOIDmode 
      && GET_MODE (size) != Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)))
      size = convert_to_mode (Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)), size, 1);
...
#define MUST_ALIGN
  ...
  if (known_align % BIGGEST_ALIGNMENT != 0)
	...
	size = expand_binop (Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)),
			     add_optab, size,
  ...
  {
    rtx dynamic_offset
      = expand_binop (Pmode1 (Extract_rtx_Pmode (virtual_stack_dynamic_rtx)),
		      sub_optab, virtual_stack_dynamic_rtx,
		      stack_pointer_rtx, NULL_RTX, 1, OPTAB_LIB_WIDEN);
    size = expand_binop (Pmode1 (Extract_rtx_Pmode (virtual_stack_dynamic_rtx)), 
			 add_optab, size, dynamic_offset,
			 NULL_RTX, 1, OPTAB_LIB_WIDEN);
  }
  ...
  if (target == 0 || GET_CODE (target) != REG
    ...
    target = gen_reg_rtx (Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)));
...
#endif
    {
      target = expand_divmod (0, CEIL_DIV_EXPR, 
			      Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)),
			      target,
			      GEN_INT (BIGGEST_ALIGNMENT / BITS_PER_UNIT),
			      NULL_RTX, 1);

      target = expand_mult (Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)),
			    target,
			    GEN_INT (BIGGEST_ALIGNMENT / BITS_PER_UNIT),
			    NULL_RTX, 1);
    }
  ...
  if ( REG_P (target) )
    mark_reg_pointer (target);
  else if (GET_CODE (target) == SUBREG) {
    rtx reg = target;
