/*
 * $Source: /home/gcc/CORETOOLS/gcc/varasm.c,v $
 * $Revision: 1.31 $ $Date: 1997/08/27 13:22:42 $
 * $Author: vdelvecc $
*/
  ...
  if (DECL_RTL (decl) == 0)
    {
	 ...
	 = gen_rtx (MEM, DECL_MODE (decl),
		    gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), name));
...
#ifdef ADI
      /* Case insensitive search on 21k */
      for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
	 if (reg_names[i][0])
	   {
	     char *cp1=reg_names[i], *cp2=asmspec;
	     /* (|0x20) used to be tolower(), but pc has problems */
	     while(*cp1 && *cp2 && ((*cp1)|0x20)==((*cp2)|0x20))  
	       {
		 cp1++;
		 cp2++;
	       }
  ...
  if (DECL_RTL (decl) == 0
      ...
      if (DECL_RTL (decl) == 0)
	{
	  ...
	  DECL_RTL (decl) = gen_rtx (MEM, DECL_MODE (decl),
				     gen_rtx (SYMBOL_REF, Pmode1 (Extract_tree_Pmode (decl)), 
					      name));
	  if (TREE_THIS_VOLATILE (decl))
...
#if defined(MAIN_REFERENCE_SYMBOL)
      if (is_main_func(fnname)) {
	ASM_OUTPUT_EXTERNAL(asm_out_file, NULL, MAIN_REFERENCE_SYMBOL);
      }
#endif
     ...
     int at_end;
{
  ...
  int reloc = 0;
#ifdef ADI
  int user_segment_p=0;  /* True if the decl has segment attribute */
#endif
  ...
  /*** have to choose sections even for non-initialized decls ***/
  /*** Note that "reloc" is 0 because uninitialized variables ***/
  /*** can't refer to anybody's address ***/
#ifdef DSP21K
  user_segment_p = user_defined_segment(decl);
  if (user_segment_p)
    in_section=no_section;
  else
    SELECT_SECTION (decl, 0);
#else
  SELECT_SECTION(decl, 0);
#endif
#endif
...
#if defined(DSP21XX) && defined(ASM_OUTPUT_INIT_ASSIGN)
      ASM_OUTPUT_INIT_ASSIGN (asm_out_file);
      assemble_zeros (size);
      ASM_OUTPUT_DECL_END(asm_out_file);
#endif
      goto finish;
    }
...
#if defined (ADI) && defined(ASM_OUTPUT_INIT_ASSIGN)
  {
      ASM_OUTPUT_INIT_ASSIGN(asm_out_file);
#endif
...
#if defined (ADI) && defined(ASM_OUTPUT_DECL_END)
  ASM_OUTPUT_DECL_END(asm_out_file);
#endif
 finish:
#ifdef XCOFF_DEBUGGING_INFO
     ...
     int size;
{
  ...
  x = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_DATA_Pmode), namestring);
#ifdef ASM_OUTPUT_ALIGNED_LOCAL
...
assemble_trampoline_template ()
{
  ...
  return gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_DATA_Pmode), name);
}

...
#if (defined ADI) && (defined REAL_VALUE_QUANTIZE)
  if(GET_MODE_BITSIZE(mode)<=HOST_BITS_PER_INT)
    REAL_VALUE_QUANTIZE(i0,i1);
#endif
     ...
     tree exp;
{
  ...
  def = gen_rtx (SYMBOL_REF, Pmode1 (Extract_tree_Pmode (exp)), desc->label);
...
#if defined (ADI) && defined(ASM_DECLARE_INTERNAL_OBJECT_NAME)
      ASM_DECLARE_INTERNAL_OBJECT_NAME(asm_out_file, label, exp);
#else
...
#if defined (ADI) && defined(ASM_OUTPUT_INTERNAL_INIT_ASSIGN)
      ASM_OUTPUT_INTERNAL_INIT_ASSIGN(asm_out_file);
#else 

#ifdef ASM_OUTPUT_INIT_ASSIGN
      ASM_OUTPUT_INIT_ASSIGN (asm_out_file);
#endif
#endif
     ...
     struct rtx_const *value;
{
  ...
  switch (GET_CODE (x))
	...
	decode_rtx_const (GET_MODE (x), x, value);
      else
	abort ();
      break;
     ...
     rtx x;
{
  ...
  def = gen_rtx (MEM, mode, gen_rtx (SYMBOL_REF, Pmode1 (Extract_rtx_Pmode (found)), 
				     found));
...
#if (defined DSP21XX)
      ASM_OUTPUT_POOL_LABEL(asm_out_file, "LC", pool->labelno);
...
#ifdef DSP21XX /* These ought to be done in macros, but it seems a bit inconvenient */
	fprintf(asm_out_file,";\n");
#endif
    done: ;
    }
...
#if defined (ADI) && defined(ASM_OUTPUT_ELT_SEP)
      ASM_OUTPUT_ELT_SEP(asm_out_file);
#endif
...
#if defined (ADI) && defined(ASM_OUTPUT_ELT_SEP)
	    /*** put separators (usually commas) between the elements
	         if there is any following data ***/
	    if (TREE_CHAIN(link)) 
		ASM_OUTPUT_ELT_SEP(asm_out_file);
#endif
	}
...
#if defined (ADI) && defined(ASM_OUTPUT_ELT_SEP)
	/*** put separators (usually commas) between the elements,
	     given that any data has been output ***/
	if(total_bytes)
	    ASM_OUTPUT_ELT_SEP(asm_out_file);
#endif
