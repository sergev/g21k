...
static char* __ident__ = "$Header: /home/gcc/CORETOOLS/gcc/c-decl.c,v 1.21 1994/11/14 15:52:05 mmh Exp $";
...
#define FRACT_TYPE_SIZE BITS_PER_WORD
#endif

#ifndef LONG_FRACT_TYPE_SIZE
#define LONG_FRACT_TYPE_SIZE (BITS_PER_WORD * 2)
#endif
...
tree double_ftype_double, double_ftype_double_double, float_ftype_float;
     ...
     register tree newdecl, olddecl;
{
  ...
  else if (TREE_MEMSEG(olddecl) != TREE_MEMSEG(newdecl))
  {
      error_with_decl (newdecl, "conflicting memory segments for `%s'");
      error_with_decl (olddecl, "previous declaration of `%s'");
  }
#endif
  else
...
init_decl_processing ()
{
...
#ifdef ADI
  TREE_TYPE (complex_double_type_node) = float_type_node;
#else
...
#ifdef ADI
  TREE_TYPE (complex_long_double_type_node) = float_type_node;
#else
...
#if defined (ADI) && 0
  fract_type_node = make_node (FRACT_TYPE);
  pushdecl (build_decl (TYPE_DECL, get_identifier ("fract"),
			fract_type_node));
  TYPE_PRECISION (fract_type_node)   = FRACT_TYPE_SIZE;
  TYPE_BINARYPOINT (fract_type_node) = 1;
  layout_type (fract_type_node);
#endif
  ...
  /* make a type for arrays of characters.
     ...
     array type.  */
#if defined(ADI)
  index_256 = build_index_type (build_int_2 ( 255, 0));
  char_array_type_node
    = build_array_type (char_type_node, index_256);
  /* Likewise for arrays of ints.  */
  int_array_type_node
    = build_array_type (integer_type_node, index_256);

  /* This is for wide string constants.  */
  wchar_array_type_node
    = build_array_type (wchar_type_node, index_256);
#else
  char_array_type_node
...
#if defined(ADI) && defined(MEMSEG)
{
    ...
    tree tt = TREE_TYPE(ptr_type);
    if (PMSTACK) {
	tt = c_build_type_variant(tt, TYPE_READONLY(tt),
				  TYPE_VOLATILE(tt),
				  PROG_MEM);
	ptr_type = build_pointer_type(tt);
    }

    builtin_function ("__builtin_alloca",
		      build_function_type (ptr_type,
					   tree_cons (NULL_TREE,
						      sizetype,
						      endlink)),
...
/* EXPAND_BUILTIN_VARARGS is obsolete.  */
  ...
  builtin_function ("__builtin_fsqrt", double_ftype_double, 
		    BUILT_IN_FSQRT, "sqrt");
#ifdef ADI
  builtin_function ("__builtin_fsqrtf", float_ftype_float, 
  ...
  if (!(TREE_CODE (decl) == FUNCTION_DECL && DECL_INLINE (decl))
      && temporary && TREE_PERMANENT (decl))
    {
	 ...
	 to have a copy of the top-level decl's DECL_INLINE.  */
      ...
      if (ITERATOR_P (decl))
	DECL_INITIAL (decl) = build_int_2_wide (TREE_INT_CST_LOW (DECL_INITIAL (decl)),
						TREE_INT_CST_HIGH (DECL_INITIAL (decl)));
...
#if defined(ADI) || defined(MEMSEG)
	/*** should not ever set segment on a node to UNKNOWN_MEM ***/
#define SEG_DEFAULT(SEG) ((SEG)==UNKNOWN_MEM?default_memory_segment:(SEG))
#endif
     ...
     int initialized;
{
...
#if defined ADI && defined MEMSEG
  int pm_p = 0, dm_p = 0;
  memory_segment seg = UNKNOWN_MEM;
#endif
...
#if defined(ADI) && defined(MEMSEG)
  pm_p += (!! (specbits & 1 << (int) RID_PM));
  dm_p += (!! (specbits & 1 << (int) RID_DM));
  if (pm_p >= 1&& dm_p >= 1) 
      error("Both `pm' and `dm' declared for '%s'", name);
  if (pm_p > 1) warning("duplicated `pm'");
  if (dm_p > 1) warning("duplicated `dm'");
  if (dm_p) seg = DATA_MEM;
  if (pm_p) seg = PROG_MEM;
#endif
...

  {
	...
	type = error_mark_node;
	     ...
	     && volatilep)
      {
	error ("iterator `%s' has been declared volatile", name);
	type = error_mark_node;
      }
...
#endif
#if defined(ADI) && defined(MEMSEG)
	  if (constp || volatilep || (seg != UNKNOWN_MEM))
...
#if defined(ADI) && defined(MEMSEG)
	  /*** When we do not know which segment the pointer points to, ***/
	  /*** we assume the default ***/

	  type = c_build_type_variant
	      (type, constp, volatilep, SEG_DEFAULT(seg));
	  seg = UNKNOWN_MEM;
	  pm_p = 0;
	  dm_p = 0;
#else
...
#if defined(ADI) && defined(MEMSEG)
		  else if (TREE_VALUE (typemodlist) ==
			   ridpointers[(int) RID_PM]) {
		      seg = PROG_MEM;
		      pm_p++;
		  }
		  else if (TREE_VALUE (typemodlist) ==
			   ridpointers[(int) RID_DM]) {
		      seg = DATA_MEM;
		      dm_p++;
		  }
#endif
...
#if defined(ADI) && defined(MEMSEG)
	      if (pm_p >= 1 && dm_p >= 1)
		  error("Conflicting memory segments declared for '%s'", name);
	      else if (pm_p > 1)
		  warning("PM segment specified more than once");
	      else if (dm_p > 1)
		  warning("DM segment specified more than once");
	      if (pm_p) seg = PROG_MEM;
	      if (dm_p) seg = DATA_MEM;
#endif
...
#if defined(ADI) && defined(MEMSEG)
      if (seg != UNKNOWN_MEM)
	  warning ("Memory segment ignored in typedef `%s'", name);
      if (constp || volatilep)
	  type = c_build_type_variant (type, constp, volatilep,
				       UNKNOWN_MEM);
#else
...

  {
	...
	if (TREE_CODE (type) == ARRAY_TYPE)
	  {
	    /* Transfer const-ness of array into that of type pointed to.  */
		  ...
		  (c_build_type_variant
		   (TREE_TYPE(type), constp, volatilep, SEG_DEFAULT(seg)));
	      seg = UNKNOWN_MEM;
	      pm_p = dm_p = 0;
#else
	    type = build_pointer_type
	...
	/* Don't use TYPE_PRECISION to decide whether to promote,
	  ...
	  {
#ifdef ADI
	    /*** to make char's integers when chars have the same ***/
	    /*** size as integers ***/
...
#if defined(ADI) && defined(MEMSEG) && 0
	/* This has already been checked above, I am just reluctant to 
	   part with this code */
	if ((seg != UNKNOWN_MEM) && (seg != PROG_MEM))
	    warning ("Memory segment is ignored for function `%s'", name);
	TREE_MEMSEG(decl) = PROG_MEM;
#endif
	if (pedantic && (constp || volatilep)
...
#ifdef ADI
    if (debug_info_level==DINFO_LEVEL_NONE && specbits & (1 << (int) RID_REGISTER))
  ...
  /*** Handle missing and ignored memory segment specs ***/

  switch(TREE_CODE (decl)) {
    case PARM_DECL:
    case FIELD_DECL:
      /*** Parms don't need segment, while fields will inherit it **/
      /*** from the top-level record ****/
      if (seg != UNKNOWN_MEM) {
	  warning_with_decl
	      (decl, "Segment spec. will be ignored for `%s'");
	  TREE_MEMSEG (decl) = UNKNOWN_MEM;
      }
      break;
    case VAR_DECL:
	  ...
	  if (seg != UNKNOWN_MEM)
	      warning_with_decl
		  (decl, "Segment will be ignored for non-static `%s'");

	  /** We assume that everything non-static and non-external **/
	  /** is either an automatic variable or a parameter **/
	  /** Parameters can reside in memory only on a stack, so **/
	  /** we treat them as automatic. When parameters are passed **/
	  /** in registers, their MEMSEG shouldn't matter at all, just **/
	  /** as for automatic register variables */
	  /* If the decl is an array, we have to reset its type too */

	  if (PMSTACK) {
	      tree decl_type = TREE_TYPE(decl);
	      seg = PROG_MEM;
	      if (TREE_CODE(decl_type) == ARRAY_TYPE) {
		  /* We can't simply reassign new segment to types */
		  /* but must rebuild the array type because of the */
		  /* posibility of shared nodes */
		  TREE_TYPE(decl) = 
		      build_array_type
			  (c_build_type_variant (TREE_TYPE (type),
						 constp, volatilep,
						 SEG_DEFAULT(seg)),
			   TYPE_DOMAIN (type));
	      }
	  }
	  else seg = SEG_DEFAULT(seg);
      }

      TREE_MEMSEG(decl) = SEG_DEFAULT(seg);

      break;
    case FUNCTION_DECL:
      if (seg != UNKNOWN_MEM && seg != PROG_MEM) {
	  warning_with_decl
	      (decl, "Segment will be ignored for function `%s'");
	  }
      TREE_MEMSEG(decl) = PROG_MEM;
      break;
    default: abort();
