...
#define MAX_DOLOOP_DEPTH 4

#define target_A21XX 1
#define target_A21C0 0
...
#define REG_OK_FOR_INDEX_STRICT_P(X) (0)             /*EK*/
#define REG_OK_FOR_INDEX_NON_STRICT_P(X) (0)         /*EK*/

#define REG_OK_FOR_BASE_STRICT_P(X)      (IS_IREG(REGNO(X)))
#define REG_OK_FOR_BASE_NON_STRICT_P(X)  (IS_IREG(REGNO(X))||IS_PSEUDO(REGNO(X)))
...
/* Nonzero if X is a hard reg that can be used as a base reg
   or if it is a pseudo reg.  */
#define REG_OK_FOR_BASE_P(X,MODE)  REG_OK_FOR_BASE_NON_STRICT_P(X)
...
/* Nonzero if X is a hard reg that can be used as a base reg.  */
#define REG_OK_FOR_BASE_P(X,MODE)  REG_OK_FOR_BASE_STRICT_P(X)
...
#define BASE_REG_P(X)  ( REG_OK_FOR_BASE_P (X))

#define INDEX_REG_P(X) ( REG_OK_FOR_INDEX_P(X))
...
#define CPP_PREDEFINES "-DADSP21xx"
#define ASM_SPEC "-c "
...
#define ASM_OUTPUT_INT(FILE,VALUE)  asm_output_int(FILE,VALUE)
...
#define RE_RECOG                (target_flags & 040)
#define RE_RECOG_FLOW           (target_flags & 100)
#define RE_RECOG_RELOAD         (target_flags & 200)
...
#define DODOLOOPS                 (!(target_flags & 16)) /*EK*/
...
#define TARGET_VERSION fprintf (stderr, " (adsp21xx early primitive -1.0):");
...
/*EK* BUG: very unsure here: do we have any pseudo regs at all, here?
 *EK* BUG: the other two versions have LAST_REG+10, BTW */
...
/*EK* comment: it seems a shame that at least the PLUS1 was left out... */
...
#define REG_CLASS_CONTENTS  { {0x00000000, 0x00000000}, /* NO_REGS      */ \
			      {0x00010000, 0x00000000}, /* A_R_REG      */ \
			      {0x00000300, 0x00000000}, /* M_R_REG      */ \
			      {0x0000c000, 0x00000000}, /* S_R_REG      */ \
			      ...
			      {0x0000c303, 0x00000000}, /* AXNR_REG     */ \
			      {0x0001c303, 0x00000000}, /* A_X_REG      */ \
			      {0x0001c330, 0x00000000}, /* M_X_REG      */ \
			      {0x00002000, 0x00000000}, /* S_E_REG      */ \
			      {0x0001d300, 0x00000000}, /* S_I_REG      */ \
			      {0x0001c30f, 0x00000000}, /* ALU_REGS     */ \
			      {0x0000f3ff, 0x00000000}, /* DNAR_REGS    */ \
			      {0x0001f3ff, 0x00000000}, /* D_REGS       */ \
...
/*EK* BUG: check that NO_REGS is still OK */

#define INDEX_REG_CLASS(MODE,R) NO_REGS

/*EK* added argument R for the following macro modelled after 21csp */
/*EK* BUG: someone should verify whether this makes sense! */
#define MODIFY_REG_CLASS(MODE,R) \
...
/*EK* was:
    (((MODE)==DMmode)?DAG1_IREGS:((MODE)==PMmode)?PM_IREGS:NO_REGS)
   */

#define ALL_BASE_REG_CLASS  DM_IREGS

/* *EK* added argument r as a dummy for the two following macros. */
...
#define CONST_DOUBLE_OK_FOR_LETTER_P(VALUE, C)  \
  ((C=='G')?const_double_ok_for_letter_p(VALUE,C):0)
...
/*(shell-command "rm regclass.o") */
#define VALUE_REGNO(MODE) (((MODE)==SFmode||(MODE)==DImode)?(REG_SR0):(REG_AR))
...
/*EK* BUG: this must be examined VEERY carefully! */
/*EK* BUG: I have assumed that the only supported constant is 0 (REG_M2) */
/*EK* BUG: since DM_MINUS1_MREG and DM_PLUS1_MREG are unavailable */
#define CONST1_OK_FOR_AUTO_MODIFY(N) ((N)==0)
...
/* BUG: PEEP_PARALLEL should probably be defined, but then we need:
	short_hand_p, peep_parallelizeable_2
		      peep_parallelizeable_3
		      dump_parallel
*/

/* #define PEEP_PARALLEL */                    /*EK*/
#undef  PEEP_PARALLEL

extern void prereload_calls();                 /*EK*/
