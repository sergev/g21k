     ...
     FILE *file;
{
  ...
  if (file)
    {
      fprintf (file, ";; Global alloc:\n");
      for (i= FIRST_PSEUDO_REGISTER; i< max_regno ; i++)
	{
	  if (reg_renumber[i] > 0)
	    fprintf (file, ";; pseudo %3d allocated to %2d (%s)\n",
		     i, reg_renumber[i], 
		     reg_names[reg_renumber[i]]);
	}
    }
     ...
     FILE *file;
{
  ...
  for (i = FIRST_PSEUDO_REGISTER, j = 0; i < max_regno; i++)
    if (reg_renumber[i] >= 0)
      {
	fprintf (file, "%d in %d(%s)  ", i, reg_renumber[i], reg_names [reg_renumber [i]]);
        if (++j % 6 == 0)
	  fprintf (file, "\n");
      }
