     ...
     int warned;
{
  ...
  for (link = declspecs; link; link = TREE_CHAIN (link))
	...
	{
	  if (!warned)
	    pedwarn ("useless keyword or type name in empty declaration", 0, 0);
	  warned = 1;
	}
    }
     ...
     int initialized;
{
...
#ifdef DSP21XX
  interruptp = !! (specbits & (1 << (int) RID_INTERRUPT));
  if(interruptp && defaulted_int)
      type=void_type_node;
#endif
...
#ifdef DSP21XX
    if (specbits & 1 << (int) RID_INTERRUPT) nclasses++;
#endif
...
#ifdef DSP21XX
    if((specbits & 1 << (int) RID_INTERRUPT) &&
       !funcdef_flag)
      {
	error("Can't use interrupt qualifier on something that isn't a function");
	type = error_mark_node;
      }
#endif
...

  {
	...
	if(interruptp)
	  {
	    DECL_INTERRUPT (decl) = 1;
	    if(TREE_TYPE (type) != void_type_node)
	      error("Interrupt function must have type void");
	  }
#endif
      }
    else
