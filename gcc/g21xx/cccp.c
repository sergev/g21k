...
#endif
static char DIR_SEPARATOR_string[] = { DIR_SEPARATOR, 0 };
...
#ifdef ADI
	/*** This is a kludge to avoid calls to __builtin_alloca ***/
	/*** when compiling for MS_DOS ***/
#if __MSDOS__
#undef alloca
#endif
#endif
...
void error (char *, ...);
void warning (char *, ...);
static void error_with_line (int, char *, ...);
void pedwarn (char *, ...);
#else
void error ();
void warning ();
...
static void pedwarn_with_file_and_line (char *, int, char *);
static void fatal ();
void fancy_abort ();
     ...
     char **argv;
{
  ...
  for (i = 1; i < argc; i++) {
	...
	if (!strcmp (argv[i], "-include")) {
	  if (i + 1 == argc)
	    fatal ("Filename missing after -include option", 0);
	  else
	    pend_includes[i] = argv[i+1], i++;
	}
	if (!strcmp (argv[i], "-imacros")) {
	  if (i + 1 == argc)
	    fatal ("Filename missing after -imacros option", 0);
	  else
  ...
  /* Some people say that CPATH should replace the standard include dirs,
     ...
     anyway.  */
#if defined(USE_ADI_DSP) && defined(ADII)
  p = (char *) getenv (ADII);
#else
  p = (char *) getenv ("CPATH");  
#endif
...
#ifdef USE_ADI_DSP
	if ( epath = getenv (USE_ADI_DSP) ) { 
	  char *cp = epath + strlen(epath);
	  char *x = alloca(strlen(epath)+20);
	  strcpy(x,epath);
	  if (*(cp-1) == DIR_SEPARATOR)
	    strcat(x,INCLUDE_FILES_LOCATION);
	  else {
	    strcat(x, DIR_SEPARATOR_string);
	    strcat(x,INCLUDE_FILES_LOCATION);
	  }
	  epath=x;
	}
  ...
  if (print_deps) {
    ...
    if (deps_target) {
      ...
      while (*p1) {
	if (*p1 == DIR_SEPARATOR) /* MMH: This should also handle '\\' */
	  p = p1 + 1;
	p1++;
      }
  ...
  if (! S_ISREG (st_mode)) {
    ...
    long i;
    fp->length = 0;
    fp->buf = (U_CHAR *) xmalloc ((int)st_size + 2);
  ...
  if (pedantic && missing_newline)
    pedwarn ("file does not end in newline", 0);
     ...
     FILE_BUF *buf;
{
  ...
  if (warn_trigraphs && fptr != bptr)
    warning ("%d trigraph(s) encountered", (fptr - bptr) / 2, 0);
}

     ...
     FILE_BUF *ip, *op;
{
  ...
  if (ident_length == 0 || !is_idstart[*ident]) {
	...
	if (pedantic && !lang_asm)
	  warning ("invalid preprocessor directive", 0, 0);
	return 0;
      }
    }
     ...
     struct directive *keyword;
{
  ...
  if (importing && warn_import && !inhibit_warnings
    ...
    import_warning = 1;
    warning ("using `#import' is not recommended", 0, 0);
...
get_filename:
  ...
  case '\"':
      ...
      for (fp = &instack[indepth]; fp >= instack; fp--)
	{
	  ...
	  if ((nam = fp->nominal_fname) != NULL) {
	    ...
	    search_start = dsp;
#ifndef VMS
	    ep = rindex (nam, DIR_SEPARATOR);     /* MMH: handle '\\' also */
#else                           /* VMS */
	    ep = rindex (nam, ']');
  ...
  if (*fbeg == '/') {
	...
	strcat (fname, DIR_SEPARATOR_string);   /* DIRSEP? MMH: If this is DOS we need '\\' */
  ...
  if (f < 0) {
    ...
    if (pcfbuf) {
      ...
      pcfinclude ((U_CHAR *)pcfbuf, (U_CHAR *)pcfbuflimit, (U_CHAR *)fname, op);
    }
    else
     ...
     FILE_BUF *op;
{
  ...
  if (*bp == '(') {
	...
	pedwarn ("another parameter follows `%s'",
      ...
      if (!is_idstart[*bp])
	pedwarn ("invalid character in macro parameter name", 0);
     ...
     struct directive *keyword;
{
  ...
  if (pedantic && done_initializing && !instack[indepth].system_header_p)
    pedwarn ("ANSI C does not allow `#assert'", 0);
     ...
     struct directive *keyword;
{
  ...
  if (pedantic && done_initializing && !instack[indepth].system_header_p)
    pedwarn ("ANSI C does not allow `#unassert'", 0);
     ...
     struct arglist *tokens;
{
  ...
  if (pedantic && !instack[indepth].system_header_p)
    pedwarn ("ANSI C does not allow testing assertions", 0);
     ...
     struct directive *keyword;
{
  ...
  while ((hp = lookup (buf, sym_length, -1)) != NULL) {
    ...
    if (hp->type != T_MACRO)
      warning ("undefining `%s'", hp->name, 0);
    delete_macro (hp);
  }
  ...
  if (pedantic) {
    ...
    SKIP_WHITE_SPACE (buf);
    if (buf != limit)
      pedwarn ("garbage after `#undef' directive", 0);
  }
  return 0;
}

     ...
     U_CHAR *buf, *limit;
{
  ...
  if (pedantic && !instack[indepth].system_header_p)
    pedwarn ("ANSI C does not allow `#ident'", 0);
  return 0;
}
     ...
     U_CHAR *buf, *limit;
{
  ...
  if (!strncmp (buf, "once", 4)) {
    ...
    if (!instack[indepth].system_header_p)
      warning ("`#pragma once' is obsolete", 0, 0);
    do_once ();
  }
...
nope:
  fatal ("You are in a maze of twisty compiler features, all different", 0);
}
#endif
...
{
  if (pedantic)
    pedwarn ("ANSI C does not allow `#sccs'", 0);
  return 0;
}

/*
     ...
     int any;
{
  ...
  while (bp < endb) {
	...
	if (p == ident) {
	  ...
	  if (*p == '\n') {
	    if (pedantic && !lang_asm)
	      pedwarn ("invalid preprocessor directive", 0);
	    continue;
	  }
	}

	if (!lang_asm && pedantic)
	  pedwarn ("invalid preprocessor directive name", 0);
	continue;
      }
     ...
     struct directive *keyword;
{
  ...
  if (pedantic) {
    SKIP_WHITE_SPACE (buf);
    if (buf != limit)
      pedwarn ("text following `#else' violates ANSI standard", 0);
  }
     ...
     struct directive *keyword;
{
  if (pedantic) {
    SKIP_WHITE_SPACE (buf);
    if (buf != limit)
      pedwarn ("text following `#endif' violates ANSI standard", 0);
  }
     ...
     register U_CHAR *p;
{
  ...
  if (*p && *p != '\n')
    pedwarn ("text following `#else' or `#endif' violates ANSI standard", 0);
}

...
static void
#ifdef __USE_STDARGS__
error_with_line (int line, char *msg,...)
#else
error_with_line (va_alist)
     va_dcl
#endif
{
  int i;
...
static void
pedwarn_with_file_and_line (file, line, msg)
     char *file;
     int line;
     char *msg;
{
