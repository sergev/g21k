     ...
     tree label;
{
  ...
  if (nonlocal_goto_handler_slot == 0)
	...
	= assign_stack_local (Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)),
			      GET_MODE_SIZE (Pmode1 (Extract_rtx_Pmode 
						    (stack_pointer_rtx))),
			      0);
  ...
  if (context != 0 && context != current_function_decl)
    {
      ...
      rtx label_ref = gen_rtx (LABEL_REF, Pmode1 (DEFAULT_CODE_Pmode), 
			       label_rtx (label));
      rtx temp;
     ...
     int dont_jump_in;
{
  ...
  if (function_call_count != thisblock->data.block.function_call_count
	     ...
	     || thisblock->data.block.stack_level != 0)))
    {
      ...
      rtx save_receiver = gen_reg_rtx (Pmode1 (DEFAULT_CODE_Pmode));
      ...
      if (fixed_regs[ARG_POINTER_REGNUM])
	{
	  ...
	  if (i == sizeof elim_regs / sizeof elim_regs [0])
		...
		arg_pointer_save_area
  	          = assign_stack_local 
		    (Pmode1 (Extract_rtx_Pmode (arg_pointer_save_area)), 
		     GET_MODE_SIZE (Pmode1 (Extract_rtx_Pmode (arg_pointer_save_area))), 
		     0);
	...
	if (! DECL_TOO_LATE (TREE_VALUE (link)))
	  {
	    ...
	    do_jump_if_equal (static_chain_rtx,
			      gen_rtx (LABEL_REF, 
				       Pmode1 (Extract_tree_Pmode (DECL_RTL (TREE_VALUE (link)))),
      ...
      emit_library_call (gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "abort"), 0,
			 VOIDmode, 0);
     ...
     register tree decl;
{
  ...
  if (type == error_mark_node)
	...
	DECL_RTL (decl) = gen_rtx (MEM, BLKmode, gen_reg_rtx (Pmode1 (Extract_tree_Pmode (decl))));
    }
...
#define CASE_VALUES_THRESHOLD 5
      ...
      else
	{
	  ...
	  for (n = thiscase->data.case_stmt.case_list; n; n = n->right)
		...
		{
		  labelvec[i]
		    = gen_rtx (LABEL_REF, Pmode1 (DEFAULT_CODE_Pmode), label_rtx (n->code_label));
	  ...
	  for (i = 0; i < ncases; i++)
	    if (labelvec[i] == 0)
	      labelvec[i] = gen_rtx (LABEL_REF, Pmode1 (DEFAULT_CODE_Pmode), default_label);
	  ...
	  if (
	    ...
	    emit_jump_insn (gen_rtx (ADDR_DIFF_VEC, CASE_VECTOR_MODE,
				     gen_rtx (LABEL_REF, Pmode1 (DEFAULT_CODE_Pmode), table_label),
...
#ifdef ADI
  /* we must not only make unsigned branches, but also unsigned comparisons!!!*/
#define GTx (unsignedp?GTU:GT)
#define GEx (unsignedp?GEU:GE)
#define LTx (unsignedp?LTU:LT)
#define LEx (unsignedp?LEU:LE)
#else
#define GTx GT
#define GEx GE
#define LTx LT
