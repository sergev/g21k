...
#ifdef __USE_STDARGS__ /*EK* was HAVE_VPRINTF */
void fatal (char *,...);
void error (char *,...);
#else
void fatal ();
...
#ifndef LIB_SPEC
#ifdef ADI
#define LIB_SPEC "%{!p:%{!pg:-lib}}%{p:-lc_p}%{pg:-lc_p}" /* Do not pass -g down */
#else
#define LIB_SPEC "%{g*:-lg} %{!p:%{!pg:-lib}}%{p:-lc_p}%{pg:-lc_p}"
#endif /* ADI */
#endif
...
"                  %{S1:asm21 %{R} %{j} %{J} %{h} %{d2} %a "
"                     %{c:%W{o*}%{!o*:-o %w%b}}%{!c:-o %d%w%b.o}"
"                      %g.s %A\n }}}}}"},
				   ...
				   " %{!S1:cpp -P"
					 ...
					 " %{H} %{d*} %C"
					   " %g.s %{!E:%g.is}"
						 ...
						 " %{!c:-o %d%w%b}"
						   " %g.is %A \n}"
						     " %{S1:asm2 %{R} %{j} %{J} %{h} %{d2} %a"
						       " %{c:%W{o*}%{!o*:-o %w%b}}%{!c:-o %d%w%b}"
							 " %g.s %A\n }}}}}"},
#endif  /* !DSP21K */
#endif /* !ADI */
  {"-",
...
"       %{S:%W{o*}%{!o*:-o %b.s}}%{!S:-o %{|!pipe:%g.s}} |\n"
"    %{!S:asm2 %{R} %{j} %{J} %{h} %{d2} %a "
"            %{c:%W{o*}%{!o*:-o %w%b}} %{!c:-o %d%w%b} %{!pipe:%g.s} %A\n }"},
  {".s",   "@assembler"},
  {".S",   "@assembler"},
  {".asm", "@assembler" },
  {".ASM", "@assembler" },
...
"%{!M:%{!MM:%{!E:%{!pp:%{!S:%{!c:ld21 %S %l %o %{L*} %{phonymap:-x} -g -gcc  "
"              %{ansi:} "
"               -e %{nomem: %{o*: %*} %{!o*: 210x}}"
"                   %{!nomem:"
"                      %{!save-temps:  %d%{o*:%$} %d%{!o*:210x}}"
"                      %{save-temps: %{o*: %$} %{!o*: 210x}}}"
"               -a %{arch*: %*} %{!arch: 210x.ach%s} "
"                %{!nostdlib: %L} %X}}}}}}}";
...
#if defined (MSDOS) && !defined (__WATCOMC__) && !defined (MSC)
  {
    char *p;
    for (p=base; *p; p++) 
      if (*p == '\\')
...
execute ()
{
...

  {
	...
	if (pid < 0)
	  {
#if defined (MSC)
	    fprintf(stderr,"wait status negative\n");
#endif
	    abort ();
	  }
...
{
    "-E              preprocess sources files only",
     ...
     char **argv;
{
...
#endif
  if (!temp) {
      error ("Must set ADI_DSP environment variable to the root of \n");
      exit(-99);
  }
  ...
  if (temp)
  {
      char *libp = (char *)alloca (strlen (temp) + 15);
      char *execp = (char *)alloca (strlen (temp) + 15);
      char *cp = temp +strlen(temp);
	...
	dir_sep[1] = 0;
	strcat (libp,dir_sep);
	strcat (execp, dir_sep);
      }

      strcat(libp,LIB_LOCATION);
      strcat(execp,CC1_LOCATION);

      add_prefix (&exec_prefix, execp, 0, 0, 0);
      add_prefix (&startfile_prefix, libp, 0, 0, 0);
      add_prefix (&library_prefix, libp, 0, 0, 0);
  }
#else
  ...
  if (*p != '}')
      ...
      while (count > 0)
	{
	  if (*q == '{')
	    ...
	    {
#if defined (MSC)
	      fprintf(stderr,"Hit end of string too soon\n");
#endif
	      abort ();
	    }
	  q++;
     ...
     char **argv;
{
  ...
  /* read file names and flags from a file */
...
#ifdef ADI
  free_argv();       /* cleanup up extra space allocated by
			process_architecture_file for extra args */
#endif
