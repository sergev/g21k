...
#ifdef ADI
     1 in a NOTE_INSN_LOOP_BEG if the loop is a do {} while(),
       that is, the condition is not tested before the first iteration
#endif
       */
...
#ifdef DSP21XX
		, INSN_START_CALL=16, FAKE_CLOBBER=17, DUMMY_MREG=18
#endif
};
...
#ifdef ADI
/* In NOTE_INSN_LOOP_BEG used contains do{}while flag */
#define SET_WHILE(RTX) ((RTX)->used=1)
#define TEST_WHILE(RTX) ((RTX)->used==1)
#endif
...
#if (defined (HAVE_PRE_INCREMENT) || defined (HAVE_PRE_DECREMENT) || defined (HAVE_POST_INCREMENT) || defined (HAVE_POST_DECREMENT) || defined (HAVE_POST_MODIFY))
