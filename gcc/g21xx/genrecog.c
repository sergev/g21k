...
#if defined(ADI) && defined(COMPUTE_STATS)
{
    char buf_a[111], buf_b[111];
    if (*c_test == '\0') {
	sprintf(buf_b, "trace_match_before(insn, %d)",   next_insn_code);
	sprintf(buf_a, "&& trace_match_after(insn, %d)", next_insn_code);
	last->c_test = concat(buf_b, buf_a);
    }
    else {
	sprintf(buf_b, "trace_match_before(insn, %d) && (", next_insn_code);
	sprintf(buf_a, ") && trace_match_after(insn, %d)",  next_insn_code);
	last->c_test = concat (concat (buf_b, c_test), buf_a);
    }
}
#else
