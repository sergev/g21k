...
/* I've change reg_reloaded_contents[] from an array of ints to
   an array of rtx's.  This way, we can track if a spill reg
   points to something other than another hard register */

static rtx reg_reloaded_contents[FIRST_PSEUDO_REGISTER];
...
/* Set to 1 when reload is in operation -- this is used by
   extra-constraint to check if an address might have been reloaded! */
#ifdef ADI
int  somewhere_in_reload = 0;
#endif
...
static rtx modify_for_reload ();
static int constraint_accepts_reg_p ();
static int count_occurrences ();
static void mark_reg_inc_notes(rtx, rtx);
...
init_reload ()
{
  ...
  register rtx tem
    = gen_rtx (MEM, Pmode1 (DEFAULT_DATA_Pmode),
	       gen_rtx (PLUS, Pmode1 (DEFAULT_DATA_Pmode),
			gen_rtx (REG, Pmode1 (DEFAULT_DATA_Pmode), LAST_VIRTUAL_REGISTER + 1),
			GEN_INT (4)));
  ...
  while (memory_address_p (QImode, tem))
    {
      ...
      tem = gen_rtx (MEM, Pmode1 (DEFAULT_DATA_Pmode), tem);
    }

  /* See if indirect addressing is valid for (MEM (SYMBOL_REF ...)).  */

  tem = gen_rtx (MEM, Pmode1 (DEFAULT_DATA_Pmode), 
		 gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_DATA_Pmode), "foo"));
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
    {
      tem = gen_rtx (PLUS, Pmode1 (DEFAULT_DATA_Pmode),
		     gen_rtx (REG, Pmode1 (DEFAULT_DATA_Pmode), FRAME_POINTER_REGNUM),
		     gen_rtx (REG, Pmode1 (DEFAULT_DATA_Pmode), i));
...
#ifdef ADI
#ifdef HAVE_reload_inpsi
  if(HAVE_reload_inpsi)
    reload_in_optab[(int) PSImode] = CODE_FOR_reload_inpsi;
#endif
#ifdef HAVE_reload_indm
  if(HAVE_reload_indm)
    reload_in_optab[(int) DMmode] = CODE_FOR_reload_indm;
#endif
#ifdef HAVE_reload_inpm
  if(HAVE_reload_inpm)
    reload_in_optab[(int) PMmode] = CODE_FOR_reload_inpm;
#endif
#ifdef HAVE_reload_outdm
  if(HAVE_reload_outdm)
    reload_out_optab[(int) DMmode] = CODE_FOR_reload_outdm;
#endif
#ifdef HAVE_reload_outpm
  if(HAVE_reload_outpm)
    reload_out_optab[(int) PMmode] = CODE_FOR_reload_outpm;
#endif
#endif /* ADI */
     ...
     FILE *dumpfile;
{
...
#ifdef ADI
      /*
	We use this variable "more globally" (az, 4/26/92)
       */
  ...
  /* And for 21xx, look for REG_INC notes.  Anything with a REG_INC note
     shouldn't be equivalent to a constant ! */
...
#ifdef DSP21XX
      /* Any register which is the subject of an
	 auto-inc pattern should not be listed as equivalent to a
	 constant.
  ...
  /* Count the number of eliminable registers and build the FROM and TO
     ...
     gen_rtx (REG, Pmode1 (DEFAULT_DATA_Pmode), STACK_POINTER_REGNUM) to equal stack_pointer_rtx.
     We depend on this.  */
  for (ep = reg_eliminate; ep < &reg_eliminate[NUM_ELIMINABLE_REGS]; ep++)
    {
      num_eliminable += ep->can_eliminate;
      ep->from_rtx = gen_rtx (REG, Pmode1 (DEFAULT_DATA_Pmode), ep->from);
      ep->to_rtx = gen_rtx (REG, Pmode1 (DEFAULT_DATA_Pmode), ep->to);
    }
  ...
  while (something_changed)
	...
	{
	  group_mode[(int) caller_save_spill_class] = Pmode1 (DEFAULT_DATA_Pmode);
...
#ifdef DSP21XX	  
	  /*
	    We want to "pre-reload" all call_insn's.  We know that
	    certain registers are availible "for free" in a call-insn.
	    We also shouldn't use any call-clobbered registers in trying
	    to formulate call-clobbering insns.
	    */
	  if(GET_CODE(insn)==CALL_INSN)
	    {
	      rtx prev = PREV_INSN (insn);
	      prereload_calls(insn);
	      insn = NEXT_INSN (prev);
	    }
#endif
	  ...
	  looks for non-strict memory addresses.  */
	      ...
	      if (GET_CODE (insn) == CALL_INSN)
		{
		  if (GET_CODE (PATTERN (insn)) == SET)
		    ...
		    after_call = 0;
/* 
  I'm putting this in here for this reason:  Alex Zatsman added code
  elsewhere in reload, when the reloads are substituted, making the
  use of return regs even more conservative.  In some obscure cases, 
  though, we will get spill regs thinking that we can use the
  return reg, but we will not be allowed to use them when allocating
  spill regs for reloads.  This would cause a problem.   My solution,
  make us just as "conservative" here. -Josh Kablotsky 5/25/93
*/
#ifdef ADI
	      if (find_reg_note(insn, REG_UNUSED, after_call))
		  after_call = 0;
#endif
   
		}
#ifdef ADI
	  /* we are more conservative -- we  keep avoid_return_reg all 
	     the way from call_insn until it is used by an insn or until 
...
#ifdef DSP21XX
	      fix_param_bug (insn, dumpfile);
	      change_reloads (insn, 0);
#endif	      
...
#ifdef ADI
      /*
	Check if avoid_return_reg is still valid. 
	It must be invalidated if we
	a) reached the end of the basic block, or
	b) used it in a regular insn.
	(Note: we should check for implicit use by call_insn when the register
	is passed on as an argument, but for now we are over-conservative.
	(az, 4/26/93)
       */
    {
	if (avoid_return_reg) {
		...
		if (reg_overlap_mentioned_p(avoid_return_reg, pat))
		    avoid_return_reg = 0;
	    }
	}
  }
...
#if defined DSP21XX
	    /* for 21xx, put in this fix (at least for now).  
	       if we're doing any caller-saves, then we'll
	       need a dag2_ireg -- simple as that -- if we were
	       to use the macro for base_reg_class, we might end
	       up with either a dag1 or a dag2 i reg -- and
	       that is unacceptable
	       */
	    = DAG2_IREGS;
#else
	    = double_reg_address_ok ?
		INDEX_REG_CLASS1 (stack_pmode) :
		    BASE_REG_CLASS1 (stack_pmode);
#endif

	  caller_save_group_size
	    = CLASS_MAX_NREGS (caller_save_spill_class, Pmode1 (DEFAULT_DATA_Pmode));
	  something_changed = 1;
	}
...
#ifdef ADI
  /* Make all insn's unrecognized because old patterns might not 
     be good any more */
  for (insn = first; insn; insn = NEXT_INSN (insn)) {
...
#ifdef DSP21XX
	    if (STATIC_SPILL) {
		x = assign_spill_area_slot(GET_MODE(regno_reg_rtx[i]),
					   total_size);
		adjust=0;
	    }
	    else {
...
#ifdef DSP21XX
	  if (STATIC_SPILL) {
	      x = assign_spill_area_slot(mode, total_size);
	      adjust = 0;
	  }
	  else {
	  ...
	  && CONSTANT_P (XEXP (x, 1)))
	{
	  for (ep = reg_eliminate; ep < &reg_eliminate[NUM_ELIMINABLE_REGS];
		...
		if (mem_mode != 0 && GET_CODE (XEXP (x, 1)) == CONST_INT
		  ...
		  return gen_rtx (PLUS, Pmode1 (DEFAULT_DATA_Pmode), ep->to_rtx,
...
order_regs_for_reload ()
{
...
#if !(defined DSP21XX) && !(defined DSP21K)
	  SET_HARD_REG_BIT (bad_spill_regs, i);
#endif
	}
    }
...
#ifdef ADI
  for (i = 0; i < FIRST_PSEUDO_REGISTER && reg_alloc_order[i]!=FIRST_PSEUDO_REGISTER; i++)
#else
  for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
#endif
    {
  ...
  /* I am not sure this is the best way to prevent the use of 
     fixed regs, like stack pointer,  for spills, but I don't 
     know of a better way (az, 3/28/94) */
     ...
     int live_known;
{
  ...
  for (i = 0; i < n_spills; i++)
    {
      reg_reloaded_contents[i] = NULL_RTX;
  ...
  for (insn = first; insn;)
...
#ifdef ADI
      /*
	(This is copied from reload() (az 4/16/94))
	Check if avoid_return_reg is still valid. 
	It must be invalidated if we
	a) reached the end of the basic block, or
	b) used it in a regular insn.
       ...
       */
	{
	  ...
	  if (n_reloads > 0)
...
#ifdef DSP21XX
	      fix_param_bug (insn, NULL);
	      change_reloads (insn, 1);
#endif	      
		...
		for (p = NEXT_INSN (prev); p != next; p = NEXT_INSN (p))
	      ...
	      /* Need to add REG_INC notes if appropriate */
#ifdef DSP21XX
	      /* Sometimes, the whole post-modify expression got reloaded.
		 So, we'll need to scan forward to from "prev" to "next" and
		 if there are any post-modify expressions, we need to add
		 the appropriate REG_INC note. 
		 
		 There might be a more elegant way of doing this, but this
		 seems robust.
		 */
	      
	      for (x= NEXT_INSN (prev); x!= next; x = NEXT_INSN (x))
		mark_reg_inc_notes(x, PATTERN(x));
#endif	     

	    }
#ifdef DSP21XX
	  for (x= insn; x!=next; x=NEXT_INSN (x))
	    if (GET_CODE (x) == INSN)
	...
	if (avoid_return_reg) {
	    ...
	    {
		rtx pat = PATTERN(insn);
		if (reg_mentioned_p(avoid_return_reg, pat))
		    avoid_return_reg = 0;
	    }
	}
  }
  ...
  for (insn=first; insn; )
    {
      ...
      if (GET_CODE (insn) == INSN &&
	  find_reg_note(insn, DUMMY_MREG, NULL) &&
	  GET_CODE (PATTERN (insn)) == SET &&
	  REG_P (SET_DEST (PATTERN (insn))) &&
	  CONST_INT_P (SET_SRC (PATTERN (insn))) &&
	  INTVAL (SET_SRC (PATTERN (insn))) == 0)
	delete_insn (insn);
      insn=next;
    }
#endif	 
     ...
     rtx x;
{
...
#ifdef DSP21XX 
  {
    int i;
    /*
      Here we want to see if we thought any spill register was equal to
      this thing which is being set.  If so, then that spill register
      isn't equal any more!
      */
    for(i=0;i<n_spills;i++)
      {
	if(rtx_equal_p(reg_reloaded_contents[i], x) ||
	   (GET_CODE (x)==REG && REGNO (x)>=FIRST_PSEUDO_REGISTER &&
	    reg_equiv_address[REGNO (x)] && reg_reloaded_contents[i] &&
	    GET_CODE (reg_reloaded_contents[i]) == MEM &&
	    rtx_equal_p(reg_equiv_address[REGNO (x)], XEXP(reg_reloaded_contents[i],0))) ||
	   reg_mentioned_p (x,reg_reloaded_contents[i]))
	  if(n_reloads==0 || 
	     (REG_P (x) && !TEST_HARD_REG_BIT (reg_is_output_reload, REGNO (x))))
	    {
	      reg_reloaded_contents[i] = NULL_RTX;
	      reg_reloaded_insn[i] = 0;
	    }
...
/*
 This next check says:
 if we've got an output reload of a pseudo, then we've got to be careful that
 any registers equivalent to that pseudo register on input (except that
 output reload reg) are marked as no longer equivalent!
*/
    if (n_reloads !=0 &&
	REG_P (x) &&
	REGNO (x) <= FIRST_PSEUDO_REGISTER &&
	TEST_HARD_REG_BIT (reg_is_output_reload, REGNO(x)))
      {
	for(i=0;i<n_reloads;i++)
	  if(reload_reg_rtx[i] &&
	     rtx_equal_p (reload_reg_rtx[i], x))
	    break;
	if(i!=n_reloads)
	  {
	    int j;
	    for(j=0;j<n_spills;j++)
	      if (reg_reloaded_contents[j] &&
		  rtx_equal_p (reg_reloaded_contents[j], reload_out[i]) &&
		  REGNO(x)!=spill_regs[j])
		{
		  reg_reloaded_contents[j] = NULL_RTX;
		  reg_reloaded_insn[j] = 0;
		}

/*
   But wait, it gets even weirder... let's say that register A is equal
   to some pseudo, B.  Let's say that register C is equal to the address
   of the stack slot for B.  Let's say that D is an output reload of B, 
   but D is represented not as a stack slot, but as a memory reference
   using C.  We have to invalidate register A, because after the instruction,
   it will no longer represent B.  In our case, i is the reload index
   of the output reload, D.  j is the spill index of the address 
   register, C.  k is the spill index of the invalid register A.
   */
  
	    if (reload_out[i] &&
		GET_CODE(reload_out[i])==MEM &&
		REG_P(XEXP(reload_out[i],0)))
	      {
		for(j=0;j<n_spills;j++)
		  if(REGNO(XEXP(reload_out[i],0)) == spill_regs[j] &&
		     reg_reloaded_contents[j])
		    {
		      int k;
		      for(k=0;k<n_spills;k++)
			if(spill_regs[k] != REGNO (x) &&
			   reg_reloaded_contents[k] &&
			   GET_CODE (reg_reloaded_contents[k]) == MEM &&
			   rtx_equal_p(XEXP(reg_reloaded_contents[k],0),
				       reg_reloaded_contents[j]))
			  {
			    reg_reloaded_contents[k] = NULL_RTX;
			    reg_reloaded_insn[k] = 0;
			  }
		    }
	      }
	  }
  ...
  /* Now check whether we clobbered any memory mentioned in 
     reg_reloaded_contents: if x = MEM(REG) then REG might be spilled 
     and equivalent to y. Then if we also have spilled register which 
     equivalent ot MEM(y), it must be forgotten.
     (az 3/21/94).
    */
    if (GET_CODE(x) == MEM) {
	rtx addr = XEXP(x,0);
	if (GET_CODE(addr) == REG) {
	    ...
	    {
		rtx eq_addr = reg_reloaded_contents[spill_reg_order[regno]];
		for(i=0; i<n_spills; i++)  {
		    rtx y = reg_reloaded_contents[i];
		    if (y && 
			(GET_CODE(y) == MEM) &&
			rtx_equal_p(XEXP(y,0), eq_addr))
		    {
			reg_reloaded_contents[i] = NULL_RTX;
			reg_reloaded_insn[i] = 0;
		    }
  ...
  if (regno >= FIRST_PSEUDO_REGISTER)
	...
	if (spill_reg_order[regno + i] >= 0
	  ...
	  {
	    reg_reloaded_contents[spill_reg_order[regno + i]] = NULL_RTX;
...
#ifdef DSP21XX
/* If reg is in used as a secondary reload for output.  */
static HARD_REG_SET reload_reg_used_secondary_output;
/* If reg is in used as a secondary reload for input.  */
static HARD_REG_SET reload_reg_used_secondary_input;
#endif
     ...
     enum machine_mode mode;
{
  ...
  for (i = regno; i < nregs + regno; i++)
	...
	{
#ifdef DSP21XX
	case RELOAD_FOR_SECONDARY_OUTPUT:
	  SET_HARD_REG_BIT (reload_reg_used_secondary_output, i);
	  break;
	case RELOAD_FOR_SECONDARY_INPUT:
	  SET_HARD_REG_BIT (reload_reg_used_secondary_input, i);
	  break;
#endif
	case RELOAD_OTHER:
  ...
  switch (when_needed)
...
#ifdef DSP21XX
	      && ! TEST_HARD_REG_BIT (reload_reg_used_secondary_input, regno)
#endif
...
#ifdef DSP21XX
	      && ! TEST_HARD_REG_BIT (reload_reg_used_secondary_output,regno)
#endif
	      && ! TEST_HARD_REG_BIT (reload_reg_used_in_output, regno));
#ifdef DSP21XX
    case RELOAD_FOR_SECONDARY_INPUT:
    ...
    case RELOAD_FOR_SECONDARY_OUTPUT:
      return (! TEST_HARD_REG_BIT (reload_reg_used_in_op_addr, regno)
	      && ! TEST_HARD_REG_BIT (reload_reg_used_in_output_addr, regno)
	      && ! TEST_HARD_REG_BIT (reload_reg_used_in_output, regno));
    case RELOAD_FOR_INPUT_MODIFY:
      /* This may be overly conservative ... we'll ease this up later */
      return (! TEST_HARD_REG_BIT (reload_reg_used_at_all, regno));
#endif
    }
  abort ();
...
#ifdef DSP21XX
  HARD_REG_SET save_reload_reg_used_secondary_input;
  HARD_REG_SET save_reload_reg_used_secondary_output;
#endif
...
#ifdef DSP21XX
  CLEAR_HARD_REG_SET (reload_reg_used_secondary_input);
  CLEAR_HARD_REG_SET (reload_reg_used_secondary_output);
#endif
...
#ifdef DSP21XX	  
	  if(reload_when_needed[j]==RELOAD_FOR_OUTPUT)
	    reload_when_needed[reload_secondary_reload[j]]=RELOAD_FOR_SECONDARY_OUTPUT;
	  else if(reload_when_needed[j]==RELOAD_FOR_INPUT)
	    reload_when_needed[reload_secondary_reload[j]]=RELOAD_FOR_SECONDARY_INPUT;
	  else

#endif
	    reload_when_needed[reload_secondary_reload[j]]=reload_when_needed[j];
      }
...
#ifdef DSP21XX
  COPY_HARD_REG_SET (save_reload_reg_used_secondary_input,
		     reload_reg_used_secondary_input);
  COPY_HARD_REG_SET (save_reload_reg_used_secondary_output,
		     reload_reg_used_secondary_output);
#endif
      ...
      for (j = 0; j < n_reloads; j++)
	{
	  ...
	  if (inheritance)
	      ...
	      if (regno >= 0 && reg_last_reload_reg[regno] != 0)
		{
		  ...
		  if (reg_reloaded_contents[i] &&
		      REG_P(reg_reloaded_contents[i]) &&
		      REGNO(reg_reloaded_contents[i]) == regno
			...
			= HARD_REGNO_NREGS (spill_regs[i], reload_mode[r]);
		      ...
		      for (k = 1; k < nr; k++)
			if (REGNO(reg_reloaded_contents[spill_reg_order[spill_regs[i] + k]])
			    != regno)
			  break;
		...
		if(reload_in[r])
		  {
		    for(i=0;i<n_spills;i++)
		      {
			if(reg_reloaded_contents[i] &&
			   rtx_equal_p(reg_reloaded_contents[i],reload_in[r]) &&
			   HARD_REGNO_MODE_OK (spill_regs[i], reload_mode[r]) &&
			   TEST_HARD_REG_BIT (reg_class_contents[(int) reload_reg_class[r]], 
					      spill_regs[i]) &&
			   (reload_nregs[r] == max_group_size
			   ...
			   reload_reg_free_before_p (spill_regs[i],
						     reload_when_needed[r]))
			  {
			    if(HARD_REGNO_NREGS (spill_regs[i], reload_mode[r])==1)
			      {
				/* Mark the register as in use for this part of
				   the insn.  */
				mark_reload_reg_in_use (spill_regs[i],
							reload_when_needed[r],
							reload_mode[r]);
				reload_reg_rtx[r] = spill_reg_rtx[i];
				reload_inherited[r] = 1;
				reload_inheritance_insn[r] = reg_reloaded_insn[i];
				reload_spill_index[r] = i;
			      }
		    ...
		    for(i=0;i<n_spills && !reload_reg_rtx[r];i++)
		      {
			if (try_special_reload_trick(insn, i,r))
			  {
			    mark_reload_reg_in_use (spill_regs[i],
						    reload_when_needed[r],
						    reload_mode[r]);
	      ...
	      /* 
		 There's another way we come hope to win.  Let's 
		 say we've got a reload into an i register (coming
		 from a stack slot), and that value already exists 
		 in a dregister.  There will be a secondary reload
		 for that i register reload.  Let's try here to
		 intercept that secondary reload so that we don't
		 have to load that secondary reload register from 
		 memory (we are likely to be able to transfer directly
		 from the inheritance register to the i register!
		 */
		else
		  if (reload_secondary_p[r] && !reload_needed_for[r])
		    {
		      int m=0;	/* m will be the primary reload */
		      while(m<n_reloads && reload_secondary_reload[m]!=r)
			m++;
		      if(m==n_reloads)
			abort(); /* if this is a seconday reload, it better
				    be secondary to some other reload */
		      if(reload_in[m] && GET_CODE(reload_in[m])==MEM)
			for(i=0;i<n_spills;i++)
			  if(reg_reloaded_contents[i] &&
			     GET_CODE (reg_reloaded_contents[i])==MEM &&
			     rtx_equal_p(XEXP(reg_reloaded_contents[i],0),XEXP(reload_in[m],0)) &&
			     HARD_REGNO_MODE_OK (spill_regs[i], reload_mode[r]) &&
			     TEST_HARD_REG_BIT (reg_class_contents[(int) reload_reg_class[r]], 
						spill_regs[i]) &&
			     (reload_nregs[r] == max_group_size
...
#ifdef DSP21XX
      COPY_HARD_REG_SET (reload_reg_used_secondary_input,
			 save_reload_reg_used_secondary_input);
      COPY_HARD_REG_SET (reload_reg_used_secondary_output,
			 save_reload_reg_used_secondary_output);
#endif
...
#ifdef ADI
		/*** for POST_MODIFY ***/
		&& reload_when_needed[r] != RELOAD_FOR_OPERAND_ADDRESS
		&& reload_when_needed[r] != RELOAD_FOR_INPUT_RELOAD_ADDRESS
		&& reload_when_needed[r] != RELOAD_FOR_OUTPUT_RELOAD_ADDRESS
#endif
     ...
     rtx insn;
{
  ...
  for (j = 0; j < n_reloads; j++)
	  ...
	  && reload_reg_rtx[j] != 0)
	{
...
#ifdef DSP21XX
	  /* If we have a reload which is generating the address
	     for another reload which happens to be inherited,
	     we don't even need to bother with the reload */
	  {
	    rtx need_for = reload_needed_for[j];
	    int m;
	    if(need_for)
	      {
		for (m=0;!special && m<n_reloads;m++)
		  if(need_for==reload_in[m] && reload_inherited[m])
		    special = 1;
		if(special)
		  reload_in[j]=NULL_RTX; /* be sure to wipe out any trace so that
					    we don't set up a bad inheritance */
	      }
	  }
#endif		  
	  ...
	  else if (GET_CODE (oldequiv) == PRE_MODIFY
		   || GET_CODE (oldequiv) == POST_MODIFY)
	    {
	      special = 1;
	      this_reload_insn
		= modify_for_reload (reloadreg, oldequiv, j, where);
	    }
		  ...
		  if (second_reload_reg)
		      ...
		      else
			{
			  ...
			  if (tertiary_icode != CODE_FOR_nothing)
			    ...
			    {
#ifdef DSP21XX
			      if (!(reload_secondary_reload[j] &&
				    reload_inherited[reload_secondary_reload[j]]))
#endif
				{
				  reload_insn
	  ...
	  && PRESERVE_DEATH_INFO_REGNO_P (true_regnum (oldequiv_reg)))
	{
	 ...
	 actually no need to store the old value in it.  */
	  ...
	  && (reload_when_needed[j] == RELOAD_OTHER
	      ...
	      || reload_when_needed[j] == RELOAD_FOR_INPUT_RELOAD_ADDRESS)
#ifdef DSP21XX
	  && (!reload_in[j] || GET_CODE (reload_in[j]) == REG)
#else
	  && GET_CODE (reload_in[j]) == REG
#endif
#if 0
	  ...
	  && spill_reg_store[reload_spill_index[j]] != 0
#ifdef DSP21XX
	  && (!reload_in[j] || dead_or_set_p (insn, reload_in[j]))
#else
  ...
  for (j = 0; j < n_reloads; j++)
      ...
      if (i >= 0 && reload_reg_rtx[r] != 0)
	{
	  ...
	  for (k = 0; k < nr; k++)
	    {
	      reg_reloaded_contents[spill_reg_order[spill_regs[i] + k]] = NULL_RTX;
	  ...
	  /* Maybe the spill reg contains a copy of reload_in.  */
					     ...
					     reload_when_needed[r]))
	    {
		 ...
		 reload_inherited[r] &&
		 GET_CODE (reload_in[r]) == PLUS &&
		 GET_CODE(XEXP(reload_in[r],0)) == REG &&
		 CONST_INT_P (XEXP (reload_in[r],1)))
		{
		  rtx before_insn;
		  for(k=0;k<n_reloads;k++)
		    if(!reload_in[k] && 
		       !reload_out[k] && 
		       reload_needed_for[k]==reload_in[r] &&
		       reload_when_needed[k]==RELOAD_FOR_INPUT_MODIFY)
		      break;
		  before_insn=insn;
		  if(k!=n_reloads)
		    {
		      rtx i;
		      i=emit_insn_before (gen_rtx (SET, 
						   GET_MODE(reload_in[r]), 
						   reload_reg_rtx[k], 
						   const0_rtx), 
					  before_insn);
		      REG_NOTES (i)= gen_rtx (EXPR_LIST, DUMMY_MREG, 
					      reload_reg_rtx[r], REG_NOTES(i));
		    }
		}
	      if ( GET_CODE (reload_in[r]) == REG)
		{
		  int nregno = REGNO (reload_in[r]);
		  reg_last_reload_reg[nregno] = reload_reg_rtx[r];
		}
	      for (k = 0; k < nr; k++)
		{
		  reg_reloaded_contents[spill_reg_order[spill_regs[i] + k]]
		    = copy_rtx(reload_in[r]);
...
#ifdef ADI
	  /* 
	    We need to try to preserve REG_INC notes if
	    we reload the base register of a post modify expression
	    */
	  
	     
	  if (reload_in[r]!=0 && reload_out[r]!=0)
	    if (reload_needed_for[r] !=0 && 
		GET_CODE (reload_needed_for[r]) == MEM &&
		GET_CODE (XEXP (reload_needed_for[r],0)) == POST_MODIFY &&
		REG_P (XEXP (XEXP (reload_needed_for[r], 0), 0)))
	      {
		rtx old= NULL_RTX;
		rtx base = XEXP (XEXP (reload_needed_for[r], 0), 0);
		if (REGNO (base) < FIRST_PSEUDO_REGISTER &&
		  ...
		  if (REGNO (base) >= FIRST_PSEUDO_REGISTER &&
		      GET_CODE (reload_in[r]) == MEM &&
		      (XEXP (reload_in[r], 0) == reg_equiv_address[REGNO(base)] ||
		       XEXP (reload_in[r], 0) == reg_equiv_mem[REGNO(base)]))
		      old=base;
		if (old != NULL_RTX)
		  {
		    rtx x;
...
{
    rtx tmp;
    rtx emit_21xx_frame_address();
    tmp = emit_21xx_frame_address(reloadreg, in, before_insn);
    if (tmp) 
	return tmp;
}
#endif
...
/* Output reload-insns to reload VALUE into RELOADREG.
   VALUE is a postmodify or premodify RTX whose operand
   is a register or memory location;
   so reloading involves incrementing that location.
...
static rtx
modify_for_reload (reloadreg, value, operandno, insn)
     rtx reloadreg;
     rtx value;
     int operandno;
     rtx insn;
{
  /* REG or MEM to be copied and incremented.  */
  rtx incloc = XEXP (value, 0);
  rtx modify = XEXP (value, 1);

  /* Nonzero if increment after copying.  */
  int post = (GET_CODE (value) == POST_MODIFY);
  rtx prev = PREV_INSN (insn);
  rtx add_insn;
  int code;
  rtx scratch = 0;
  ...
  if (reload_secondary_reload[operandno] >= 0) {
    scratch = reload_reg_rtx[reload_secondary_reload[operandno]];
    emit_insn_before (gen_move_insn (scratch, incloc), insn);
  }

  /* If this is post-modify, first copy the location to the reload reg.  */
  if (post)
    emit_insn_before (gen_move_insn (reloadreg, scratch?scratch:incloc), insn);
  ...
  if (! post)
    {
      emit_insn_before (gen_move_insn (reloadreg, scratch), insn);
      emit_insn_before (gen_add2_insn (reloadreg, modify), insn);
      if (scratch)
	emit_insn_before (gen_move_insn (scratch, reloadreg),  insn);
      emit_insn_before (gen_move_insn (incloc, scratch?scratch:reloadreg), insn);
    }
  else
    {
      /* Postincrement.
	 ...
	 the original value.  */
#ifdef ADI
      insn = NEXT_INSN (insn); /* SORRY BUT I HAVE TO BREAK THE RULE */
#endif
      emit_insn_before (gen_add2_insn (reloadreg, modify), insn);
      if (scratch)
	emit_insn_before (gen_move_insn (scratch, reloadreg),  insn);
...
static void mark_reg_inc_notes(rtx insn, rtx x)
{
  enum rtx_code code;
  char *fmt;
  int i,j;
  ...
  fmt = GET_RTX_FORMAT (code);
  if (code==POST_MODIFY)
    if (!find_reg_note (insn, REG_INC, XEXP (x,0)))
      REG_NOTES (insn) = gen_rtx (EXPR_LIST, REG_INC, XEXP (x,0), REG_NOTES(insn));
  
  for (i=GET_RTX_LENGTH (code) -1; i>=0; i--)
    if (fmt[i]=='e')
      mark_reg_inc_notes(insn, XEXP (x,i));
    else if (fmt[i]=='E')
      for (j=XVECLEN(x,i)-1; j>=0; j--)
	mark_reg_inc_notes(insn, XVECEXP (x,i,j));
}

#ifdef DSP21XX

/*
   We have a bug (I'll call this Craig's bug, since he found it)
   in which the compiler uses ar as a reload register to reload an
   insn which loads an i register for an indirect call.  Since ar
   is also a parameter register, we've got a problem.   So, what I
   do is this.  For every insn, I scan through the reloads to see
   if any of them ask for a register class which contains ar.  If
   they do, then I scan forward from that insn.  If I encounter 
   a use of ar before I see ar set, a call, or a label, then
   I change that reload so that it asks instead for the largest
   (last) subclass of reload register class requested.

   Potentially, we could find that this class only contained ar.
   If so, then we're hosed.  I suspect that if this happens then
   we're missing a condition for ending our search.

   Another potential problem is that ay1 is also used for a 
   parameter.  I'll cross that bridge when I come to it (I'll
   regret this, won't I !?!)

   -jak 7/13/93
*/

void fix_param_bug (rtx insn, FILE *dumpfile)
{
  int i,j;
  static char *reg_class_names[] = REG_CLASS_NAMES;
  for (i=0;i<n_reloads;i++)
    {
      if ((reload_out[i] || reload_in[i] || reload_secondary_p[i]) && 
	  reg_in_class (reload_reg_class[i], REG_AR))
	{
	  /* Now search forward to see if AR is "used" before
	     it is set.  Stop looking if either:
	     1.  ar is set
	     2.  a call is made
	     3.  a label is seen
	     */
	  rtx p;
	  int bingo=0;
	  for(p=NEXT_INSN(insn);p && !bingo ;p=NEXT_INSN(p))
	    {
	      if (GET_CODE (p)==INSN &&
		  GET_CODE (PATTERN (p)) == SET &&
		  REG_P ( SET_DEST (PATTERN (p))) &&
		  REGNO ( SET_DEST (PATTERN (p))) == REG_AR)
		break;
	      else 
		...
		if (GET_CODE (p)==INSN &&
		    ...
		    REG_P (XEXP (PATTERN (p), 0)) &&
		    REGNO (XEXP (PATTERN (p), 0)) == REG_AR)
		  bingo = 1;
	    }
	  if (bingo)
	    {
	      enum reg_class new=NO_REGS, *cl;
	      for (cl=&reg_class_subclasses[reload_reg_class[i]][0];
		   *cl != LIM_REG_CLASSES; 
		   cl++)
		if (!reg_in_class(*cl, REG_AR))
		  new=*cl;
	      if(dumpfile)
		fprintf(dumpfile, ";; fix_param_bug() at insn %d, replacing class %s with %s\n",
			INSN_UID (insn), reg_class_names[reload_reg_class[i]], 
			reg_class_names[new]);
	      if(new==NO_REGS)
		abort();
	      reload_reg_class[i]=new;
	    }
	}
    }
}
void change_reloads (rtx insn, int in_as_needed)
{
  int i,j;
  for (i=0;i<n_reloads;i++)
    {
	 ...
	 reg_class_subset_p(reload_reg_class[i], DM_IREGS) &&
	 GET_CODE (reload_in[i]) ==PLUS &&
	 XEXP (reload_in[i],0) == frame_pointer_rtx &&
	 CONST_INT_P (XEXP (reload_in[i],1)))
	{
	  enum reg_class class = (reload_reg_class[i]==DAG2_IREGS?DAG2_MREGS:DAG1_MREGS);
	  if(in_as_needed)
	    {
	      for(j=0;j<n_spills;j++)
		if(TEST_HARD_REG_BIT(reg_class_contents[class],spill_regs[j]))
		  break;
	      if(j==n_spills)
		break;
	    }
	  reload_in[n_reloads]=reload_out[n_reloads]=NULL_RTX;
	  reload_reg_class[n_reloads]=class;
	  reload_reg_rtx[n_reloads]=NULL_RTX;
	  reload_inmode[n_reloads]=reload_inmode[i];
	  reload_optional[n_reloads]=0;
	  reload_strict_low[n_reloads]=0;	  
	  reload_in[n_reloads]=0;
	  reload_secondary_reload[n_reloads]=0;
	  reload_secondary_p[n_reloads]=1;
	  reload_secondary_icode[n_reloads]=CODE_FOR_nothing;
	  reload_needed_for[n_reloads]=reload_in[i];
	  reload_when_needed[n_reloads]=RELOAD_FOR_INPUT_MODIFY;
...
#ifdef DSP21XX
rtx
emit_21xx_frame_address(reloadreg, in, before_insn)
    rtx reloadreg, in, before_insn;
{
    rtx base, offset, first_insn, i;
    int frame_ptr_p, good_offset_p, j;
    enum machine_mode mode=GET_MODE(in);

    if (GET_CODE(in) != PLUS)
	return 0;
    base   = XEXP(in, 0);
    frame_ptr_p = (GET_CODE(base) == REG) &&
	(REGNO(base) == FRAME_POINTER_REGNUM);
    if (! frame_ptr_p)
	return 0;
    if (mode != GET_MODE (stack_pointer_rtx))
      abort();
    offset = XEXP(in, 1);
    good_offset_p = (CONSTANT_P(offset) || (REG_P(offset)));
    if (! good_offset_p)
	return 0;
    first_insn = emit_insn_before (gen_rtx(SET, mode, reloadreg, offset), 
				   before_insn);
    if (recog_memoized(first_insn) < 0)
	abort();
    i = emit_insn_before (gen_rtx(SET, mode, reloadreg, 
				  gen_rtx(PLUS, mode, reloadreg, base)),
			  before_insn);
    if (recog_memoized(i) < 0)
	abort();
    REG_NOTES (i) = gen_rtx (EXPR_LIST, REG_EQUIV, in, REG_NOTES(i));

    for(j=0;j<n_reloads;j++)
      if(!reload_in[j] && 
	 !reload_out[j] && 
	 reload_needed_for[j]==in &&
	 reload_when_needed[j]==RELOAD_FOR_INPUT_MODIFY)
	break;
    if(j!=n_reloads)
      {
	i=emit_insn_before (gen_rtx (SET, mode, reload_reg_rtx[j], 
				     const0_rtx), before_insn);
	REG_NOTES (i)= gen_rtx (EXPR_LIST, DUMMY_MREG, reloadreg, 
				REG_NOTES(i));
      }
    return first_insn;
}
...
int try_special_reload_trick(rtx insn, int i, int r)
{
  if(GET_CODE (reload_in[r]) == PLUS &&
     XEXP (reload_in[r],0) == frame_pointer_rtx &&
     CONST_INT_P (XEXP (reload_in[r],1)) &&
     reg_reloaded_contents[i] &&
     GET_CODE (reg_reloaded_contents[i]) == PLUS &&
     XEXP (reg_reloaded_contents[i],0) == frame_pointer_rtx &&
     CONST_INT_P (XEXP (reg_reloaded_contents[i],1)) &&
     HARD_REGNO_MODE_OK (spill_regs[i], reload_mode[r]) &&
     TEST_HARD_REG_BIT (reg_class_contents[(int) reload_reg_class[r]],
			spill_regs[i]) &&
     reload_nregs[r] == 1 &&
     HARD_REGNO_NREGS (spill_regs[i], reload_mode[r])==1 &&
     reload_reg_free_p (spill_regs[i], reload_when_needed[r]) &&
     reload_reg_free_before_p (spill_regs[i],
			       reload_when_needed[r]))
    {
      rtx m_load=PREV_INSN(reg_reloaded_insn[i]);
      rtx note;
      while (m_load && 
	     (GET_CODE(m_load)!=INSN || !(note=find_reg_note (m_load, DUMMY_MREG, NULL))))
	 m_load=PREV_INSN(m_load);

      if(note && 
	 REG_P (XEXP (note,0)) && 
	 REGNO (XEXP (note, 0))== spill_regs[i] &&
	 GET_CODE (PATTERN (m_load)) == SET &&
	 CONST_INT_P(XEXP (PATTERN (m_load),1)) &&
	 INTVAL (XEXP (PATTERN (m_load),1)) == 0)
	{
	  rtx tmp,search_insn = PREV_INSN (insn);
	  rtx *loc=0;
	  rtx difference = GEN_INT (INTVAL (XEXP (reload_in[r],1)) - 
	    INTVAL (XEXP (reg_reloaded_contents[i],1)));

	  while (search_insn && 
		 GET_CODE(search_insn)!=CODE_LABEL && 
		 GET_CODE(search_insn)!=CALL_INSN && 
		 search_insn!=m_load && 
		 !loc)
	    {
	      if (GET_CODE (search_insn) == INSN)
		{
		  rtx body = PATTERN (search_insn);
		  
		  if(GET_CODE(body)==SET)
		    if(GET_CODE(SET_SRC(body))==MEM &&
		       REG_P (XEXP (SET_SRC (body),0)) &&
		       REGNO (XEXP (SET_SRC (body),0))==spill_regs[i])
		      {
			SET_SRC(body)=copy_rtx(SET_SRC(body));
			loc=&XEXP(SET_SRC(body),0);
		      }
		    else
		      if(GET_CODE(SET_DEST(body))==MEM &&
			 REG_P (XEXP (SET_DEST (body),0)) &&
			 REGNO (XEXP (SET_DEST (body),0))==spill_regs[i])
			{
			  SET_DEST(body)=copy_rtx(SET_DEST(body));
			  loc=&XEXP(SET_DEST(body),0);
			}
		}
	      if(!loc)
		search_insn=PREV_INSN(search_insn);
	    }
	  for(tmp=NEXT_INSN(m_load); tmp && tmp!=search_insn; tmp=NEXT_INSN(tmp))
	    {
	      rtx body = PATTERN (tmp);
		...
		return 0;
		  ...
		  GET_CODE (body) == SET &&
		  REG_P (SET_DEST (body)) &&
		  REGNO (SET_DEST (body)) == REGNO (SET_DEST (PATTERN (m_load))))
		return 0;
	    }
	  if(loc)
	    {
	      rtx reg = gen_rtx (REG, GET_MODE (*loc), spill_regs[i]);
	      SET_SRC (PATTERN (m_load)) = difference;
	  
	      *loc = gen_rtx (POST_MODIFY, 
			      GET_MODE (*loc), 
			      reg,
			      SET_DEST (PATTERN (m_load)));
	      REG_NOTES (search_insn) = gen_rtx (EXPR_LIST, REG_INC, reg, REG_NOTES (search_insn));
	      return 1;
	    }
