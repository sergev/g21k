...
init_expr_once ()
{
  ...
  start_sequence ();
  insn = emit_insn (gen_rtx (SET, VOIDmode, 0, 0));
  pat = PATTERN (insn);
     ...
     int len, align;
{
  ...
  if (!(data.autinc_from && data.autinc_to)
      ...
      if( !data.autinc_from)
	{
      ...
      if( !data.autinc_to)
	{
	  rtx temp=gen_reg_rtx(GET_MODE(to_addr));
	  if(mod_reg_equiv_size < REGNO(temp)+1)
	    {
	      mod_reg_equiv_const=(int *)realloc(mod_reg_equiv_const,(1+REGNO(temp))*sizeof(int));
	      if(!mod_reg_equiv_const)
	  ...
	  if(data.reverse)
	    {
	      emit_insn (gen_rtx (SET,GET_MODE(to_addr),temp,constm1_rtx));
	      mod_reg_equiv_const[REGNO(temp)]=-1;
	      data.to_addr=copy_addr_to_reg(plus_constant(to_addr,len-1));
	    }
	  else
	    {
	      emit_insn (gen_rtx (SET,GET_MODE(to_addr),temp,const1_rtx));
	      mod_reg_equiv_const[REGNO(temp)]=1;
	      data.to_addr=copy_addr_to_reg(to_addr);
	    }
	  data.to_addr = gen_rtx(POST_MODIFY, 
				 GET_MODE(to_addr), 
				 data.to_addr,
				 temp);
	  data.explicit_inc_to=0;
...
gen_push_operand ()
{
...
#ifdef DSP21XX
  {
    ...
    emit_insn (gen_rtx (SET,mode,temp,gen_rtx (CONST_INT, mode, -1)));
    if(mod_reg_equiv_size < REGNO(temp)+1)
      {
	mod_reg_equiv_const=(int *)realloc(mod_reg_equiv_const,(1+REGNO(temp))*sizeof(int));
	if(!mod_reg_equiv_const)
...
void
#ifdef __USE_STDARGS__
emit_library_call_value (rtx orgfun, ...)
#else
...
#else
      emit_library_call (memcpy_libfunc, 0,
			 VOIDmode, 3, XEXP (to_rtx, 0), Pmode1 (Extract_rtx_Pmode(XEXP (to_rtx, 0))),
			 XEXP (from_rtx, 0), Pmode1 (Extract_rtx_Pmode(XEXP (from_rtx, 0))),
			 size, Pmode1 (Extract_rtx_Pmode (size)));
#endif
#else
      emit_library_call (bcopy_libfunc, 0,
			 VOIDmode, 3, XEXP (from_rtx, 0), Pmode1 (Extract_rtx_Pmode(XEXP (from_rtx, 0))),
	...
	if (offset != 0)
	  {
	    ...
	    op0 = change_address (op0, VOIDmode,
				  gen_rtx (PLUS, Pmode1 (Extract_tree_Pmode (op0)), 
					   XEXP (op0, 0),
					   force_reg (Pmode1 (Extract_tree_Pmode (op0)), 
						      offset_rtx)));
	  }
...
/* Could determine the answer when only additive constants differ.
   Also, the addition of one can be handled by changing the condition.  */
	...
	return force_reg (Pmode1 (Extract_rtx_Pmode (op0)), op0);
      return op0;
