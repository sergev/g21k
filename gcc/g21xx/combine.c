...
#ifdef __WATCOMC__
static rtx gen_rtx_combine (enum rtx_code, ...);
#else
...
#ifdef DSP21XX
  /* On 21xx we don't want to merge in simple register moves because
     they break lifetimes and thus reduce conflicts and reloads.
     (az 2/16/94).
     */
  if (NOCOMBINE) {
      if (i2 && simple_reg_move_p(i2) && (i1 == 0))
	  return 0;
      if (i2 && simple_reg_move_p(i2) && i1 && simple_reg_move_p(i1))
	  return 0;
      if (i3 && simple_reg_move_p(i3))
	  return 0;
  }
#endif
...
#ifdef ADI
  /* I'n not sure whether this is the right init value, but I have */
  /* to satisfy the Purify (az 6/3/93)*/
