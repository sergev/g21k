...
#if defined CSCAPE /*EK*/
#include <cscape.h>
#include <scancode.h>
#endif

#if defined WIN31 /*EK*/
#include <windows.h>                            // prototype of OutputDebugString()
#endif
...
extern char *fgetssim( char *string, int n, char *stream );
extern int      MergeAchStr(void);
#endif
...
int ach_parse (char *filename) 
{
	...
	int state;
	short i;
	char *str_ptr = NULL;                   /* only used for SHARC EZKIT to simulate file_ptr */

/* *************************************************************************** */
/* StartOfDebug                                                                */
/* *************************************************************************** */
#ifdef DEBUGLEVEL1
		OutputDebugString("Enter ach_parse(..)\n");
#endif  
/* *************************************************************************** */
/* EndOfDebug                                                                  */
/* *************************************************************************** */
   ...
   if(MergeAchStr() <=0)
      return TRUE;
	str_ptr = szEzSharcAchStr;
	assert(str_ptr != NULL);
#endif

	/* save architecture file name */
	strncpy(ConfigInfo.szAchFile, filename, FILENAME_MAX);
	ConfigInfo.szAchFile[FILENAME_MAX] = 0;
...
#else
	while (fgetssim(line, BUFSIZ, str_ptr))
#endif
	{
	...
	/* END  while (fgets (line, BUFSIZ, file_ptr))  */
	}
...
/* *************************************************************************** */
#ifdef DEBUGLEVEL1
		OutputDebugString("Leave ach_parse(..)\n");
#endif  
/* *************************************************************************** */
/* EndOfDebug                                                                  */
/* *************************************************************************** */
...
Modified Date:  Nov.5, 1996
		export processor name to global structure ConfigInfo
********************************************************************/
...
static int ach_process_processor (void)
{
	...
	strncpy(ConfigInfo.szProcessorName, token, PROCESSORNAMELIMIT);
	ConfigInfo.szProcessorName[PROCESSORNAMELIMIT] = 0;
...
Modified:               Oct.30, 1996
				There are some bug related to 21061, some comments are added to help
				debug. 
...
/* *************************************************************************** */
#ifdef DEBUGLEVEL1
		char szDebugStr[LONGDEBUGSTR + 1];
		sprintf(szDebugStr, "Enter ach_check_segment(processor = %d, start =%x, end=%x, type=%u, width=%ld\n", processor, start,end,type,width);
...
/* ******************************************************************************************* *
 *  EndOf  adjust the memory space if in multiprocessor                                                                            *
 * ******************************************************************************************* */
...
/* ******************************************************************************************* *
 * ADSP210Z3(21060) related special stuff in achitecture                                                           *
 * ******************************************************************************************* */
...
/* ******************************************************************************************* *
 * ADSP210Z4(21062) related special stuff in achitecture                                                               *
 * ADSP210Z4 has 1/2 physical memory as ADSP210Z3, it need twice as much code to handle        *
 * virtual memory                                                                                                                                                          *    
 * ******************************************************************************************* */
...
/* ******************************************************************************************** *
 * ADSP21061 related special stuff in achitecture                                                                                   *
 * ADSP210Z4 has 1/4 physical memory as ADSP210Z3, it need four times as much code to handle    *
 * virtual memory                                                                               *
 * ******************************************************************************************** */
...
/* ****************************************************************************************** *
 * End of processor related stuff                                                             *
 * ****************************************************************************************** */
...
/* ******************************************************************************************* */
	...
	if (Processor_type == ACH_ADSP210Z4) 
	{ /* 21062, 2Mbit */
	    for (i=0;i<block_1_count;i++) 
		{
	...
	}       /*       EndOf if (Processor_type == ACH_ADSP210Z4) */
    ...
    }    /* EndOf else if (Processor_type == ACH_ADSP21061) */

/* ******************************************************************************************* */      
/* EndOf check for alias conflicts                                                               */
/* ******************************************************************************************* */
...
/* *************************************************************************** */
#ifdef DEBUGLEVEL1
		OutputDebugString("Leave ach_check_segment(..)\n");
#endif  
/* *************************************************************************** */
/* EndOfDebug                                                                  */
/* *************************************************************************** */
