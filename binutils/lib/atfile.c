...
static CONS *read_arg_list(char *file_name);
int atfile_get_length(CONS *list);
static CONS *nconc ( CONS *f, CONS *n);
char **atfile_expand_argv(int argc, char *argv[], int after, int gap);
void atfile_copy_list(char *argv[], CONS *arglist);
void free_argv(void);
