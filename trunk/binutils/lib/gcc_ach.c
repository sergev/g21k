...
#ifdef sun
extern char     *getenv(char *); /* the Sun stdlib.h and malloc.h conflict */
#endif
...
void process_architecture_file(int *pargc, char ***pargv)
{
	...
	else
	{
	    ...
	    char        *cp;
	    char        *xp;
	    char         term = FILENAME_SEPARATOR;

	    if((cp = get_adi_dsp())) /*EK*/
	      {
		while(*cp)
		  *buf++ = *cp++;
	...
	if(NULL != ach_file)
	  {
	    /* 
	     * if an architecture file is not 
	     * explicit add it to ARGV
	     *
	     */
