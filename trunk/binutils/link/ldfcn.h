...
#ifdef MSDOS
#define FREAD(p,s,n,ldptr)    fread(p,(size_t) s, (size_t) n,IOPTR(ldptr))
#else
#define FREAD(p,s,n,ldptr)    fread(p,s,n,IOPTR(ldptr))
#endif
#define FSEEK(ldptr,o,p)      fseek(IOPTR(ldptr),(p==BEGINNING)?(FILE_OFFSET(ldptr)+o):o,p)
#define FTELL(ldptr)          ftell(IOPTR(ldptr))
#ifdef MSDOS
#define FWRITE(p,s,n,ldptr)   fwrite(p,(size_t) s, (size_t) n,IOPTR(ldptr))
#else
