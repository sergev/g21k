...
#include <stdlib.h>
void huge              *D16HugeAlloc (long total_size);	/* Rational */
...
#if (defined (SIM) || defined (EMU)) && !defined WIN31
#include <cscape.h>
#include <scancode.h>
#include "gui_mgr.h"
#include "gui_tty.h"
...
long 
port_get_long (register unsigned char huge * buffer)
{
  register long           w = 0;
...
void 
port_put_long (register long w, register unsigned char huge * buffer)
{
  register int            i = BITSPERBYTE * sizeof (long);
...
short 
port_get_short (register unsigned char huge * buffer)
{
  register short          w = 0;
...
void 
port_put_short (register short w, register unsigned char huge * buffer)
{
  register int            i = BITSPERBYTE * sizeof (short);
