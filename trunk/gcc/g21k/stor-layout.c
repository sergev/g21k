     ...
     tree type;
{
  ...
  switch (TREE_CODE (type))
    ...
    case OFFSET_TYPE:
      TYPE_SIZE (type) = size_int (GET_MODE_BITSIZE (Pmode1 (Extract_tree_Pmode (type))));
      TYPE_MODE (type) = Pmode1 (Extract_tree_Pmode (type));

      break;

    case FUNCTION_TYPE:
    case METHOD_TYPE:
      TYPE_MODE (type) = mode_for_size (2 * GET_MODE_BITSIZE 
					(Pmode1 (Extract_tree_Pmode (type))),
					MODE_INT, 0);
      TYPE_SIZE (type) = size_int (GET_MODE_BITSIZE (TYPE_MODE (type)));
      break;
    ...
    case REFERENCE_TYPE:
      TYPE_MODE (type) = Pmode1 (Extract_tree_Pmode (type));
      TYPE_SIZE (type) = size_int (GET_MODE_BITSIZE (TYPE_MODE (type)));
#if (defined ADI) || (POINTERS_MAYBE_SIGNED) 
      TREE_UNSIGNED (type) = (SIGNED_POINTERS)?0:1;
#else
      TREE_UNSIGNED (type) = 1;
#endif
