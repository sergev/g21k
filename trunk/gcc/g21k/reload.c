/*
 * $Source: /home/gcc/CORETOOLS/gcc/reload.c,v $
 * $Revision: 1.61 $ $Date: 1997/09/15 15:10:46 $
 * $Author: vdelvecc $
*
...
#ifndef BASE_REG_CLASS1
#define BASE_REG_CLASS1(m,r)  BASE_REG_CLASS
#define INDEX_REG_CLASS1(m,r) INDEX_REG_CLASS
#endif
...
/* The variables set up by `find_reloads' are:
   ...
   reload_reuse           int, index of reload to reuse for this reload
			   this reload uses the same register as the reload index.
   reload_when_needed_obscure enum, classifies reload as needed either for 
			   ...
			   of the above).  This is used as a place holder for
			   obscure needs. i.e. POST_MODIFY spills.
...
int n_reloads;
rtx current_find_reloads_insn;  /* This is only for debugging.
				   it contains the current insn being
				   reloaded. mmh*/
...
int reload_reuse[MAX_RELOADS];
enum reload_when_needed reload_when_needed_obscure[MAX_RELOADS];
...
static void find_reloads_address_plus();
static int find_reloads_address_modify ();
...
#define MATCHES(x, y) \
 (x == y || (x != 0 && (GET_CODE (x) == REG                             \
  ...
  if (in != 0 && GET_CODE (in) == SUBREG
    ...
    {
#ifdef ADI
      /* I think that this section causes us problems.
	 For example, if we are reloading (subreg:PM (reg:SI x)) into
	 a dag2 register (all PM addresses like to be in dag2 registers),
	 then we want to reload the entire (subreg zz) expression, since
	 we can't transfer the (reg:SI x) into a dag2 register since
	 dag2 registers are illegal in SI mode.
	  
	 I don't know what other problems taking this code out might cause.
	 I know, however that the preferred_reload_class() and 
	 secondary_reload_class() functions do look through (subreg zzz) 
	 expressions to find out what the real reloaded register is.
  ...
  if (out != 0 && GET_CODE (out) == SUBREG
    ...
    {
#ifdef ADI
      /* I got the idea for this code from the above section for input
	 reloads.  I'm here because I'm having a problem when reloading
	 something like:
	 (strict_low (subreg:SI (reg:DF hard)))

	 At this point, we've got the strict_low stripped off.
	 Usually, gcc tries reloading the entire hard reg.
	 I only want it to reload the entire (subreg ..) expression.
	 ...
	 */
      ...
      if (GET_CODE (SUBREG_REG(out)) == MEM)
	abort();

      if (GET_MODE_SIZE (GET_MODE (out)) > GET_MODE_SIZE (outmode))
	abort ();
...
#if defined ( ADI) && 0 
  /* I think that this is taken care of by the following solution merged in at 2.3.1 */
  /* This is, hopefully, a bug fix -- right now
     the compiler does not deal well with (subreg:SI(reg:DI 60)), where
     r60 is a pseudo in memory */
  if (out != 0 && 
      GET_CODE(out) == SUBREG &&
      REG_P(SUBREG_REG(out)) &&
      REGNO(SUBREG_REG(out)) >= FIRST_PSEUDO_REGISTER &&
      reg_equiv_mem[REGNO(SUBREG_REG(out))] !=0 ) 
    {
      rtx addr=XEXP(reg_equiv_mem[REGNO(SUBREG_REG(out))],0);
      out=gen_rtx(MEM,outmode,plus_constant(addr,SUBREG_WORD(out)));
    }
...
combine_reloads ()
{
  ...
  for (i = 0; i < n_reloads; i++)
	...
	if (reload_secondary_reload[output_reload] != -1)
	  ...
	  {
	    reload_secondary_reload[i] = reload_secondary_reload[output_reload];
	    reload_secondary_icode[i] = reload_secondary_icode[output_reload];
	  }
#else
	  reload_secondary_reload[i] = reload_secondary_reload[output_reload];
#endif
     ...
     register rtx x, y;
{
  ...
  if (GET_CODE (y) == PRE_DEC || GET_CODE (y) == PRE_INC || code == PRE_MODIFY)
  ...
  if (GET_CODE (x) == MEM)
      ...
      if (GET_CODE (addr) == PRE_MODIFY || GET_CODE (addr) == POST_MODIFY)
	{
	  if (CONSTANT_P (XEXP (addr, 1)))
	    {
	      val.base  = XEXP (addr, 0);
	      val.start = - INTVAL (XEXP (addr, 1));
	      val.end   = INTVAL(XEXP (addr, 1));
	      val.safe  = REGNO (val.base) == STACK_POINTER_REGNUM;
	      return val;
#if 0
	      base = XEXP (addr, 0);
...
#ifdef ADI
/*
  Scan through all recog_operands and if any are inside
  strict_low_part's then change the recog_operand_loc
  to point to the strict_low_part instead.
*/
static void point_outside_strictlow (rtx *up, rtx *down, int j)
{
  if(down==recog_operand_loc[j])
    {
      recog_operand_loc[j]=up;
      if(GET_CODE(*up)!=STRICT_LOW_PART)
	recog_operand[j]=*up;
    }
  else
    {
      int i;
      char *fmt;
      fmt=GET_RTX_FORMAT(GET_CODE(*down));
      for(i=GET_RTX_LENGTH(GET_CODE(*down))-1; i>=0; i--)
	  if(fmt[i] == 'e')
	    point_outside_strictlow (down,&XEXP(*down,i),j);
    }
}
#endif

     ...
     short *reload_reg_p;
{
  ...
  current_find_reloads_insn = insn;
  this_insn = insn;
  ...
  for (i=0;i<MAX_RELOADS;i++)
    reload_when_needed_obscure[i] = -1;
  ...
  switch (GET_CODE (body))
	...
	if(insn_operand_strict_low[insn_code_number][i])
	  point_outside_strictlow(&body,&XEXP(body,0),i);
	  ...
	  while (*p && (c = *p++) != ',')
		...
		if ((swapped && (c != commutative || i != commutative + 1))
		...
		else
		  {
		    ...
		    if (value != 0 
#if defined (ADI) && defined (A21C0)
		/* This is a temporary fix. Consider insn ax1=i2+1. 
		 * If ax1 can be substituted with i2 and losers=0, 
		 * the substitution does not happen.
		 * See bug26.c. --lev 8/22/95
		 */
			&& losers > 1
#endif
			)
		...
		if(class_ok_for_mode (REG_CLASS_FROM_LETTER(c), operand_mode[i]))
#endif
		  this_alternative[i]
		...
		goto reg;
	      ...
	      case 'T':
	      case 'U':
#ifdef TARGET_EXTRA_CONSTRAINT_LETTERS
TARGET_EXTRA_CONSTRAINT_LETTERS
#endif
...
#ifdef DSP21K
	    case 'x':   /* DM M registers */
	    case 'z':   /* PM M registers */
		/* If the operand is used as a pointer, then the 21060 must
		   have the base in a I reg, not an M register, so we
		   ignore those preferences */
  
		if ((REG_P(operand) && REGNO_POINTER_FLAG (REGNO (operand)))
		   || (GET_CODE (operand) == SUBREG &&
		       REGNO_POINTER_FLAG (SUBREGNO (operand))))
		    break;
		/* FALL-THRU */
#endif
      default:
 ...
 finish:
		 ...
		 && GET_CODE (recog_operand[i]) == MEM)
	  {
	    operand_reloadnum[i]
	      = push_reload (XEXP (recog_operand[i], 0), NULL_RTX,
			     ...
			     BASE_REG_CLASS1 (GET_MODE (XEXP(recog_operand[i], 0)), REG_NONE), 
  ...
  for (i = 0; i < n_reloads; i++)
      ...
      if (reload_needed_for[i] != 0 && ! reload_needed_for_multiple[i])
	{
	  ...
	  if (reload_in[i] != NULL_RTX && reload_out[i] != NULL_RTX)
	    {
	      /* These input-output reloads which are needed for
		 addresses are generated in ADI code for POST_MODIFY.
		 This combination of an output reload needed for an
		 address was not used (nor did it have a useful meaning)
		 before POST_MODIFY was introduced, and it is still not
		 very well handled.  Previously, an "output" reload
		 meant an output from the main insn, and other code
		 seems to assume that output reloads go after the main
		 insn.

		 That is undesirable in this case, because if a reload
		 of this kind is needed for an input address, the
		 "output" reload can and should occur before the main
		 insn, immediately after the reload of the input which
		 needs this reload, thus freeing up the reload register
		 for any other output reloads:

		    input reload for input address into R
		    reload of input (load from input address)
		    output reload for input address out of R
		    main insn
		    [ R now free for output or output address reloads ]

		 But emit_reload_insns inserts the output reload after
		 the main insn, and other code seems to assume that
		 location as well.  Perhaps someone should go through
		 and isolate these assumptions so they can be changed.

		 Until then, the suboptimal solution used here is to
		 call these RELOAD_OTHERs which are "needed_for_multiple",
		 thus causing everyone to assume the worst about them. */
	      reload_needed_for_multiple[i] = 1;
	      continue;
	    }
	  ...
	  {
	    /* This has been factored out because of a 2.7.2 compiler bug. */
	    int needs = reload_when_needed_obscure[i];
	    if (needs >= 0) {
	      reload_when_needed[i] = (enum reload_when_needed)needs;
	    }
	  }
	}
    }
  ...
  if (code == SUBREG && GET_CODE (SUBREG_REG (x)) == REG)
...
#ifdef ADI
	/*
	  This code is designed to handle the case where
	  you might have (subreg:SI (reg:DM x) 0) where x is
	  a psuedo and where x is equivalent to
	  (const:DM (const_int y)).  If this is what we've got
	  then either change the mode of the const or get rid
	  of the const all together.
	  */
	{
	  if(GET_MODE(x)!=GET_MODE(tem))
	    switch(GET_CODE(tem))
	      {

	      case CONST:
		tem=gen_rtx(CONST,GET_MODE(x),XEXP(tem,0));
		break;

	      case SYMBOL_REF:
		tem=gen_rtx(CONST,GET_MODE(x),tem);
		break;
		/* (subreg:si (const_int:pm 111)) should become
		   (const_int 111) */

	      case CONST_INT:
		if (GET_MODE(x) == SImode)
		  tem = gen_rtx (CONST_INT, VOIDmode, INTVAL(tem));
		break;
	      }
	  return tem;
	}
	...
	abort ();
	 ...
	 force a reload in that case.  So we should not do anything here.  */
      ...
      /* 
	Here, we want to handle a (subreg:m1 (reg:m2 pseudo) x)
	Do this by just adding an offset to the address of the
	pseudo.  
	*/
	       ...
	       reg_equiv_mem[regno] != 0)
	{
	  ...
	  rtx tem;
	  rtx addr = plus_constant(XEXP(reg_equiv_mem[regno],0),offset);
...
#ifdef ADI
/* I'm a little worried about making this change at this late date,
   but I don't really have a choice -- it needs to work.
    
   Essentially, we've got a problem.  Previous passes can
   end up producing an operand that looks like
   (subreg:x (subreg:y (reg: pseudo) . ) . )

   We need to convert this to something like:
   (mem:x ..)
...
*/
    ...
    {
      rtx tem;
      tem = find_reloads_toplev(SUBREG_REG(x),ind_levels,is_set_dest);
      if(GET_CODE(tem)==MEM)
	{
	  int offset=SUBREG_WORD(x) * UNITS_PER_WORD;
	  rtx addr=plus_constant(XEXP(tem,0),offset);
	  x=gen_rtx(MEM,GET_MODE(x),addr);
...
/* this function will look a lot like find_reloads_toplev() 
 The big difference is that incoming psuedo-regs will be
 translated to their reg_equiv_mem.
*/
extern int somewhere_in_reload; /* this is defined in reload1.c to be true during
				   almost the entire reload pass */

rtx
reduce_addr_operand (x)
     rtx x;
{
  int regno;
  if(somewhere_in_reload)
	...
	if (reg_equiv_constant[regno] != 0)
	  ...
	  x = reg_equiv_address[regno];
	break;
      case SUBREG:
	x=reduce_addr_operand(SUBREG_REG(x));
	break;
      }
     ...
     int ind_levels;
{
  ...
  if (GET_CODE (ad) == REG)
      ...
      else if (reg_equiv_address[regno] != 0)
	{
	  ...
	  push_reload (tem, NULL_RTX, loc, NULL_PTR, BASE_REG_CLASS1 (GET_MODE (tem),REG_NONE),
	...
	return 0;
	 ...
	 subject of a CLOBBER in this insn.  */
	       ...
	       REGNO_OK_FOR_BASE_P (regno, GET_MODE(ad))
	...
	return 0;
      ...
      push_reload (ad, NULL_RTX, loc, NULL_PTR, BASE_REG_CLASS1 (GET_MODE (ad),
		   REG_NONE),
  ...
  if (GET_CODE (ad) == MEM)
		    ...
		    && GET_CODE (XEXP (XEXP (tem, 0), 1)) == CONST_INT)))
	{
	  ...
	  push_reload (tem, NULL_RTX, loc, NULL_PTR,
		       BASE_REG_CLASS1 (GET_MODE (tem),REG_NONE), GET_MODE (tem), VOIDmode, 0,
		       0, operand);
	  return 1;
	}
      else
...
#if defined(ADI) && defined(MEMSEG)
	     INDEX_REG_CLASS1(GET_MODE(ad), REGNO(XEXP (ad, 0))),
	     GET_MODE (ad), operand,
#else
	     INDEX_REG_CLASS1, GET_MODE (ad), operand,
#endif
	     ind_levels);
	}
      else
...
#if defined(ADI) && defined(MEMSEG)
	  enum reg_class class = BASE_REG_CLASS1 (GET_MODE (ad),
						REGNO(XEXP (ad, 0)));
#else
	  enum reg_class class = BASE_REG_CLASS1;
#endif

#ifdef DSP21XX
	if(GET_CODE(ad)==PLUS && REG_P(XEXP(ad,0)))
	  if(reg_in_class(PM_MREGS,REGNO(XEXP(ad,0))))
	    class=PM_IREGS;
	  else if(reg_in_class(DM_MREGS,REGNO(XEXP(ad,0))))
	    class=DM_IREGS;
#endif
	  find_reloads_address_part (ad, loc, class, 
				     Pmode1 (Extract_rtx_Pmode (ad)),
				     operand, ind_levels);
	}
      return 1;
    }
	   ...
	   && ! memory_address_p (mode, ad))
    {
      *loc = ad = gen_rtx (PLUS, GET_MODE (ad),
      ...
      find_reloads_address_part (XEXP (ad, 0), &XEXP (ad, 0),
				 BASE_REG_CLASS1(GET_MODE(ad), REG_NONE),
				 GET_MODE (ad), operand, ind_levels);
#else
      find_reloads_address_part (XEXP (ad, 0), &XEXP (ad, 0), BASE_REG_CLASS1,
				 GET_MODE (ad), operand, ind_levels);
#endif
...
#if defined(ADI) && defined(MEMSEG)
      find_reloads_address_part (ad, loc, BASE_REG_CLASS1(GET_MODE(ad),REG_NONE),
				 Pmode1 (Extract_rtx_Pmode (ad)), operand,
				 ind_levels);
#else
      find_reloads_address_part (ad, loc, BASE_REG_CLASS1, 
				 Pmode1 (Extract_rtx_Pmode (ad)), operand,
				 ind_levels);
#endif
      return 1;
     ...
     rtx x, y;
{
  ...
  if (CONSTANT_P (x) && CONSTANT_P (y))
	...
	y = XEXP (y, 0);
      ...
      return gen_rtx (CONST, VOIDmode, gen_rtx (PLUS, Pmode1 (Extract_rtx_Pmode (x)),
						x, y));
    }

  return gen_rtx (PLUS, Pmode1 (Extract_rtx_Pmode (x)), x, y);
}

...

/* 
   When an addresss contains a (plus x y) expression, 
   find_reloads_address_plus() is called.  We will
   always use base+(index/offset) addressing, so we will
   never need to reload the entire plus, we just may need
   to reload some of the inputs to the "plus"

*/
static void find_reloads_address_plus(x,operand, ind_levels)
rtx x;
rtx operand;
int ind_levels;
{
  ...
  if(code0==SUBREG)
    code0=GET_CODE(SUBREG_REG(op0));
  if(code1==SUBREG)
    code1=GET_CODE(SUBREG_REG(op1));
  ...
  if (code0 == REG && code1 == REG)
    {
      /*** Mode is that of x -- pointer arithmetic **/
      /** should not change DAG **/
      if (REG_OK_FOR_INDEX_P (rop0, mode)
	  && REG_OK_FOR_BASE_P (rop1, mode)
#if defined(ADI) && defined(A21C0)
		&& SAME_DAG_REGS (REGNO (rop1), REGNO (rop0))
#endif
	)
	return;
      else if (REG_OK_FOR_INDEX_P (rop1, mode)
	       && REG_OK_FOR_BASE_P (rop0, mode)
#if defined(ADI) && defined(A21C0)
		&& SAME_DAG_REGS (REGNO (rop0), REGNO (rop1))
#endif
	)
	return;

      /* If one of these registers is known to be a pointer then
	 just favour that one. */

      else if (RTX_ISPOINTER_P (rop1) && REG_OK_FOR_BASE_P (rop1, mode))
	find_reloads_address_1 (op0, 1, &XEXP (x, 0), operand, ind_levels);
      else if (RTX_ISPOINTER_P (rop0) && REG_OK_FOR_BASE_P (rop0, mode))
	find_reloads_address_1 (op1, 1, &XEXP (x, 1), operand, ind_levels);

      else if (REG_OK_FOR_BASE_P (rop1, mode))
	find_reloads_address_1 (op0, 1, &XEXP (x, 0), operand, ind_levels);
      else if (REG_OK_FOR_BASE_P (rop0, mode))
	find_reloads_address_1 (op1, 1, &XEXP (x, 1), operand, ind_levels);

      else if (REG_OK_FOR_INDEX_P (rop1, mode))
	find_reloads_address_1 (op0, 0, &XEXP (x, 0), operand, ind_levels);
      else if (REG_OK_FOR_INDEX_P (rop0, mode))
	find_reloads_address_1 (op1, 0, &XEXP (x, 1), operand, ind_levels);
      else
	{
    ...
    {
      find_reloads_address_1 (op0, 0, &XEXP (x, 0), operand, ind_levels);
#ifdef CONST_OK_FOR_AUTO_MODIFY
	      if (GET_CODE (op1) == CONST_INT
		       && ! CONST_OK_FOR_AUTO_MODIFY (op1))
		{
		  /*EK* 3rd arg was "&op1", which counts as a mortal sin */
		  push_reload (op1, 0, &XEXP (x, 1), 0, INDEX_REG_CLASS1(mode, REG_NONE),
...
/* The goal of this function is to find the reloads for a modify expression.*/

#if defined (DSP21XX) || defined (A21C0)
static int find_reloads_address_modify (x, loc, operand, ind_levels)
     rtx x;
     rtx *loc;
     rtx operand;
     int ind_levels;
{
  ...
  rtx op0 = XEXP(x,0);
  rtx op1 = 0;
  enum reg_class class1 = NO_REGS; /* class of mod-reg */
  enum reg_class class0 = NO_REGS; /* class of base-reg */
  RTX_CODE code0 = GET_CODE(op0);
  RTX_CODE code1 = UNKNOWN;
  int modifier_reloaded_p = 0;
  ...
  if (code0 == SUBREG)
    {
      op0 = SUBREG_REG (XEXP (x, 0));
      code0 = GET_CODE (op0);
    }
  if (code1 == SUBREG)
    {
      op1 = SUBREG_REG (XEXP (x, 1));
      code1 = GET_CODE (op0);
    }
  ...
  if (code0 == REG)
    {
      regno = REGNO (op0);
      /*
	First I put in the following code
	Then I commented out the following code.
	
	I put it in because I didn't think it was a good idea to be post-modifying
	a constant address.  But now I think it is.  Imagine if we're copying
	to/from some area in memory which has a label associated with it.  We'll
	have a pseudo which is equiv to that constant, but we'll be incrementing
	it
  ...
  /* Reload modifier register */
   
  if (op1)
    {
      code1  = GET_CODE (op1);
      class1 = MODIFY_REG_CLASS (mode, regno);
       
#if defined (DSP21XX)
      if (code0 == REG 
	  && REG_OK_FOR_BASE_P (op0, mode))
	class1 = IS_DAG1_IREG (REGNO (op0)) ? DAG1_MREGS : DAG2_MREGS;
#endif

      switch (code1)
	{
	case REG:
	  if (! REG_OK_FOR_MODIFY_P (op1, mode))
	    {
	      rtx temp = find_reloads_toplev (XEXP (x, 1), ind_levels, 0);
	      modifier_reloaded_p = 1;
	     
	      reloadnum = push_reload (temp, 0, &XEXP(x,1), 0,
				       class1, mode, VOIDmode, 0, 0, operand);
	      changed_p = 1;
	    }
	  break;

	case CONST_INT:
	  if (!CONST_OK_FOR_AUTO_MODIFY (XEXP (x, 1))) {
	    reloadnum = push_reload (op1, 0, &XEXP (x, 1), 0,
				     class1, mode, VOIDmode, 0, 0, operand);
	    modifier_reloaded_p = 1;
	    changed_p = 1;
	  }
  ...
  switch (code0)
	...
	if (op1 
	    ...
	    && ! modifier_reloaded_p)
	  {
	    ...
	    if (IS_DAG1_MREG (r1))
	      class0 = DAG1_IREGS;
	    else if (IS_DAG2_MREG (r1))
	      class0 = DAG2_IREGS;
	    else
	      abort();
	  }
	else
	  class0 = ((class1 == DAG1_MREGS)
		    ? DAG1_IREGS : DAG2_IREGS);
  
	if (IS_PSEUDO (r0)) 
	  {
	    rtx temp = find_reloads_toplev (XEXP (x, 0), ind_levels, 1);

	    /* This chunk of code is kind of confusing and they would
	       probably lynch me for doing this but for now.  This is
	       very simple we setup the vector reload_reuse to tell
	       choose_reload_regs that the input uses the same
	       register configuration as the output.  Assuming and I
	       think its always the case that push_reload will return
	       exactly the same sequence of moves for each the in and
	       the out.  Because of this we just have some simple for loop
	       that sets up the permutation into the reload_reg_rtx's. */
	        
	    int nr_before, nr_after, nr_end, nr, nrout;
	     
	    nr_before = n_reloads;
	     
	    /* output first */
	    reloadnum = push_reload (NULL_RTX, temp, NULL_RTX, &XEXP (x, 0),
				     class0, mode, mode, 0, 0, operand);
	     
	    nr_after = n_reloads;
	    nrout = nr_after - nr_before;

	    /* input second */
	    push_reload (temp, NULL_RTX, &XEXP (x, 0), NULL_RTX,
			 class0, mode, mode, 0, 0, operand);
	     
	    /* Walk all the reloads produced by the output case and
	       if the class is the same use reload_reuse to force
	       choose_reloads_reg to reuse the output as the input */
	    for (nr=0;nr<nrout;nr++) {
	      if (reload_reg_class[nr_before+nr] == class0)
		/* index through the input_address_reloads. */
		reload_reuse[nr_after+nr] = nr_before+nr;

	      /* Okay the following is used to ensure that the output side of this
		 reload is handled correctly. see pr:907 
		 nr_before+nr index through the output_address_reloads */
	      reload_when_needed_obscure[nr_before+nr] = RELOAD_FOR_OUTPUT_RELOAD_ADDRESS;
	    }

	    changed_p = 1;
	}
	else if (!TEST_HARD_REG_BIT (reg_class_contents[(int)class0], r0))
	  {
	    reloadnum = push_reload (op0, op0, &XEXP (x, 0), &XEXP (x, 0),
				     class0, mode, mode, 0, 0, operand);
	    changed_p = 1;
	  }
      }
    ...
    default:
      {
	rtx temp = find_reloads_toplev (XEXP (x, 0), ind_levels, 1);
	reloadnum = push_reload (temp, temp, &XEXP (x, 0), &XEXP(x, 0),
				 class0, mode, mode, 0, 0, operand);
	changed_p = 1;
      }
    }
     ...
     int ind_levels;
{
  ...
  if (!(code0 == REG 
	&& REG_OK_FOR_BASE_P (op0, mode)))
    push_reload (XEXP (x, 0), XEXP (x, 0), &XEXP (x, 0), 0, 
		 BASE_REG_CLASS1 (mode, REG_NONE), mode, mode, VOIDmode, 0, operand);
   
  switch (code1)
    {
    case REG:
      do_reload = !(REG_OK_FOR_INDEX_P (op1, mode));
      break;

    case CONST_INT:
      do_reload = CONST_OK_FOR_AUTO_MODIFY (op1);
      break;

    case CONST:
      if (GET_CODE (XEXP (op1, 0)) == CONST_INT)
	do_reload = CONST_OK_FOR_AUTO_MODIFY (XEXP (op1, 0));
      else
	do_reload = 1;
      break;
  ...
  if (do_reload)
    {
      rtx temp = find_reloads_toplev (XEXP (x, 1), ind_levels, 0);
      push_reload (temp, NULL_RTX, &XEXP(x,1), 0, INDEX_REG_CLASS1 (mode, REG_NONE),
		   mode, VOIDmode, 0, 0, operand);
    }
}
#endif

int set_other_regno (x, operand, other_operand, other_regno)
     rtx x, operand;
     rtx *other_operand;
     int *other_regno;
{
  if (GET_CODE (operand) == MEM 
	  ...
	  || GET_CODE (XEXP (operand, 0)) == POST_MODIFY))
    {
      *other_operand = (x == XEXP (XEXP (operand, 0), 0)) ? 
	XEXP (XEXP (operand, 0), 1) : 
	(x == XEXP (XEXP (operand, 0), 1)) ? XEXP (XEXP (operand, 0), 0) 
	: NULL;
    }
  if (*other_operand && GET_CODE(*other_operand) == SUBREG)
    SWITCH_SUBREG(*other_operand);
   
  if (*other_operand && GET_CODE (*other_operand) == REG)
    *other_regno = REGNO (*other_operand);
  else
    *other_regno = REG_NONE;
}
...
#define SET_OTHER_REGNO set_other_regno(x,operand,&other_operand,&other_regno)

#if defined(ADI) && defined(MEMSEG)
  /** Need mode to determing DAG for base and index registers **/
  enum machine_mode mode = GET_MODE(x);
#endif
  int other_regno = REG_NONE;
  rtx other_operand = NULL;

  switch(code)
    {
    case PLUS:
      find_reloads_address_plus(x,operand, ind_levels);
      return 0;
      break;
#ifdef ADI
    case POST_MODIFY:
	...
	return find_reloads_address_modify(x,loc,operand,ind_levels);
#else      
    case POST_INC:
    case POST_DEC:
#endif
...
#if defined(ADI) && defined(MEMSEG)
	  if ((regno >= FIRST_PSEUDO_REGISTER
	       || !(context ? REGNO_OK_FOR_INDEX_P (regno,mode)
		    : REGNO_OK_FOR_BASE_P (regno,mode))))
#else
	  if ((regno >= FIRST_PSEUDO_REGISTER
		...
		= push_reload (x, NULL_RTX, loc, NULL_PTR,
		   context ? INDEX_REG_CLASS1 (mode, other_regno) : 
			BASE_REG_CLASS1 (mode, other_regno),
	  ...
	  if (code == POST_MODIFY || code == PRE_MODIFY)
	    {
	      if (GET_CODE (XEXP (x, 1)) == REG)
		{
		  register int regno = REGNO (XEXP (x, 1));
		  int value = 0;
		  rtx x_orig = x;
		  int other_regno;
		  if (GET_CODE (XEXP (x, 0)) == REG)
		    other_regno = REGNO (XEXP (x, 0));
		   else
		    other_regno = REG_NONE;
		  ...
		  if (reg_equiv_address[regno] != 0)
		    {
		      rtx tem = make_memloc (XEXP (x, 1), regno);
		      /* First reload the memory location's address.  */
		      find_reloads_address (GET_MODE (tem), 0, XEXP (tem, 0),
					    &XEXP (tem, 0), operand, ind_levels);
		      /* Put this inside a new increment-expression.  */
		  ...
		  if (reg_renumber[regno] >= 0)
		    regno = reg_renumber[regno];
		  if ((regno >= FIRST_PSEUDO_REGISTER
		       || !(context ? REGNO_OK_FOR_BASE_P (regno, GET_MODE(x))
			    : REGNO_OK_FOR_INDEX_P (regno, GET_MODE(x)))))
		    {
		      register rtx link;

		      int reloadnum
			= push_reload (XEXP(x,1), 0, &XEXP(x,1), 0,
				       context ? 
				       BASE_REG_CLASS1(GET_MODE(x), other_regno) :
				       INDEX_REG_CLASS1(GET_MODE(x), other_regno),
				       GET_MODE (x), GET_MODE (x), VOIDmode, 0, operand);
		      reload_inc[reloadnum]
			= find_inc_amount (PATTERN (this_insn), XEXP (x_orig, 1 ));
...
#if 0 
	      /*def CONST_OK_FOR_AUTO_MODIFY
		this was to tell reload how to handle reloading a
		post_modify with a constant */
	      else if (GET_CODE (XEXP (x,1)) == CONST_INT
		       && ! CONST_OK_FOR_AUTO_MODIFY (XEXP(x,1)))
		{
		    push_reload (XEXP(x,1), 0, &XEXP(x,1), 0, INDEX_REG_CLASS1(GET_MODE(x), REG_NONE),
				 GET_MODE(x), VOIDmode, 0, 0, operand);
		}
#endif
	    }
      ...
      else if (GET_CODE (XEXP (x, 0)) == MEM)
	{
	  ...
	  reloadnum = push_reload (x, 0, loc, 0,
			       context ? BASE_REG_CLASS1(GET_MODE(x), REG_NONE) :
					 INDEX_REG_CLASS1(GET_MODE(x), REG_NONE),
			       GET_MODE (x), VOIDmode, 0, 0, operand);
	  ...
	  if (code == POST_MODIFY || code == PRE_MODIFY)
	    {
		 ...
		 more efficient to use the value of SPILL_INDIRECT_LEVELS from
		 reload1.c here.  */
	      find_reloads_address (GET_MODE (x), &XEXP (x, 1),
	      ...
	      reloadnum = push_reload (x, 0, loc, 0,
			       ...
			       GET_MODE (x), VOIDmode, 0, 0, operand);
	      reload_inc[reloadnum]
		= find_inc_amount (PATTERN (this_insn), XEXP (x, 1));
    ...
    case MEM:
	 ...
	 reload1.c here.  */
      ...
      push_reload (*loc, NULL_RTX, loc, NULL_PTR,
		   context ? INDEX_REG_CLASS1 (mode, other_regno) : 
		   BASE_REG_CLASS1 (mode, REG_NONE),
		   GET_MODE (x), VOIDmode, 0, 0, other_regno);
      return 1;
    }
      break;
    case REG:
      ...
      if (reg_equiv_mem[regno] != 0)
	{
	  push_reload (reg_equiv_mem[regno], NULL_RTX, loc, NULL_PTR,
		       context ? INDEX_REG_CLASS1 : BASE_REG_CLASS1,
		       GET_MODE (x), VOIDmode, 0, 0, operand);
	  return 1;
	}
#endif
	...
	if (reg_equiv_constant[regno] != 0)
	  {
	    push_reload (reg_equiv_constant[regno], 0, loc, 0,
			 context ?
			 INDEX_REG_CLASS1(mode, other_regno) :
			 BASE_REG_CLASS1(mode, other_regno),
			 GET_MODE (x), VOIDmode, 0, 0, operand);
	    return 1;
	  }
  ...
  for (i = 0; i < n_replacements; i++)
    {
      ...
      if (reloadreg)
	{
	  if (RTX_ISPOINTER_P (*r->where) && !RTX_ISPOINTER_P (reloadreg))
	    {
	      reloadreg = copy_rtx (reloadreg);
	      RTX_MARK_ISPOINTER (reloadreg);
	    }
	      ...
	      || INSN_UID (p) < reload_first_uid))
	{
	  ...
	  if (pat != 0
		  ...
		  || (goal_const && (tem = find_reg_note (p, REG_EQUIV,
		      ...
		      && (valueno = true_regnum (valtry = SET_DEST (pat))) >= 0)
#ifdef ADI
		  /* I (jak) took out this code on 1/15/93 because it produced
		     bad results.  Specifically, we store SF mode constants in
		     (const_double ) rtx's.  We store the double value in the
		     operands of the const_double and signify that it is an
		     SF by setting the mode.  When we output the constant into
		     the assembly stream, we convert it to what it's float
		     representation would be.  Therefore, it is no longer equivalent
		     to the double representation.  so if a double register comes
		     along and needs that value, we can't allow this reload to 
		     be inherited.
		     */
		  || (goal_const && (tem = find_reg_note (p, REG_EQUIV,
							  NULL_RTX))
      ...
      if (GET_RTX_CLASS (GET_CODE (p)) == 'i')
	{
	  ...
	  if (GET_CODE (pat) == SET || GET_CODE (pat) == CLOBBER)
	      ...
	      if (GET_CODE (dest) == REG)
		{
		  ...
		  /* If destination is a psedo register that is equiv to a memory location
		     and this memory location overlaps with the goal then we have not 
		     found a hard register that the compiler can use as an equiv. */
		  if (REGNO(dest) >= FIRST_PSEUDO_REGISTER
		      && reg_equiv_mem[REGNO(dest)]
		      && goal 
		      && GET_CODE (goal) == MEM
		      && true_dependence (goal, reg_equiv_mem[REGNO (dest)]))
		    return 0;
		  ...
		  if (GET_CODE (v1) == SET || GET_CODE (v1) == CLOBBER)
		      ...
		      if (GET_CODE (dest) == REG)
			{
			  ...
			  /* If destination is a psedo register that is equiv to a memory location
			     and this memory location overlaps with the goal then we have not 
			     found a hard register that the compiler can use as an equiv. */
			  /* Note that GCC 2.7.2 has a entirely different fix
			     which might be solving the same problem. */
			  if (REGNO(dest) >= FIRST_PSEUDO_REGISTER
			      && reg_equiv_mem[REGNO(dest)]
			      && goal
			      && GET_CODE (goal) == MEM
...
#ifdef DSP21XX
rtx find_subst(rtx patt)
{
  int j;
  for(j=0;j<n_replacements;j++)
    if(*(replacements[j].where)==patt)
      return reload_reg_rtx[replacements[j].what];
  return patt;
}
#endif
...
/* Say something interesting about the current reload insn. */
void debug_reloadsf (FILE *f) 
{
  static char *reg_class_names[] = REG_CLASS_NAMES;
  static char *reload_when_needed_names[] = {
    "RELOAD_FOR_-1(unset)",
  ...
  if (current_find_reloads_insn)
    debug_rtxf (f, current_find_reloads_insn);

  fprintf (f, "needs `n_reloads': %d\n", n_reloads);
  for (r=0; r<n_reloads; r++) {

    fprintf (f, "\n%d when_needed: %s class: %s strict_low: %d optional: %d inc: %d multiple: %d\n",
	     r,
	     reload_when_needed_names[reload_when_needed[r]+1],
	     reg_class_names[reload_reg_class[r]],
	     reload_strict_low[r],
	     reload_optional[r],
	     reload_inc[r],
	     reload_needed_for_multiple[r]);
    fprintf (f, "  nocombine: %d reuse: %d when_needed_obscure: %s\n",
	     reload_nocombine[r],
	     reload_reuse[r],
	     reload_when_needed_names[reload_when_needed_obscure[r]+1]);

    if (reload_secondary_p [r]) {
      fprintf (f, "  This reload requires secondary reloads, this is the secondary reload: %d icode: %d\n",
	       reload_secondary_reload[r],
	       reload_secondary_icode[r]);
    }
     
    if (reload_in[r] && reload_out[r]
	&& reload_in[r] == reload_out[r]) {
      fprintf (f, "  in=out: inmode: %s outmode: %s",
	       mode_name[reload_inmode[r]],
	       mode_name[reload_outmode[r]]);
      debug_rtxf (f, reload_in[r]);
    } else if (reload_in[r]) {
      fprintf (f, "  in: inmode: %s", mode_name[reload_inmode[r]]);
      debug_rtxf (f, reload_in[r]);
    } else if (reload_out[r]) {
      fprintf (f, "  out: outmode: %s", mode_name[reload_outmode[r]]);
      debug_rtxf (f, reload_in[r]);
    }

    if (reload_in[r] && reload_in_reg[r]) {
      fprintf (f, "  reload in is equivalent to in_reg:");
      debug_rtxf (f, reload_in_reg[r]);
    }

    if (reload_reg_rtx[r]) {
      fprintf (f, "  this is the reload reg for %d is: ", r);
      debug_rtxf (f, reload_reg_rtx[r]);
    }
     
    if (reload_needed_for[r]) {
      fprintf (f, "  this reload is needed for an address computation:");
      debug_rtxf (f, reload_needed_for[r]);
    }
     
  }
}

void debug_reloads (void) 
{
  debug_reloadsf (stderr);
}
