...
rtx reverse_list ( list, reversed)
     rtx list, reversed;
{
  if (list)
    return reverse_list (XEXP (list, 1), 
			 gen_rtx (INSN_LIST, VOIDmode, XEXP (list, 0), reversed));
  else
    return reversed;
}
...
try_merge_delay_insns (insn, thread)
     rtx insn, thread;
{
  rtx trial, next_trial, next_insn;
  ...
  rtx merged_insns = 0;
  rtx reversed_insns = 0;
  int found_one = 0;
  int i;

#ifdef ADI
/* 
  It is possible that the branch is the last instruction in the function.
  If this is the case, then the thread may be zero.  If this is the
  case, then 1. we would like to avoid a segmentation fault, and 2. we
  won't be able to merge delay insns anyways.  So, we might as well
  just give up now. jak 2-December-1992
*/
  if(!thread)
    return;
#endif
/* two cases when we can't delete the insn:
   1.  It uses something which is set in the fallthrough case 
       (so it would need to be redone).
   2.  It sets somethign which is set in the fallthrough case.
   (exclusive of itself).
...
*/
  ...
  if (INSN_UID (next_insn) == INSN_UID (thread) )
    {
      
      CLEAR_RESOURCE(&set);
      
      for(trial=insn; !stop_search_p(trial,1); trial = next_nonnote_insn(trial))
	{
	  mark_set_resources(trial, &set,1);
	}
      
      for(trial=next_nonnote_insn(PREV_INSN(thread)); 
	  !stop_search_p(trial,1); 
	  trial = next_trial)
	{
	  ...
	  if(GET_CODE(trial)==INSN &&
	    ...
	    continue;
	  if(rtx_equal_p(PATTERN(next_to_match), pat))
	    {
	      if(!sets_cc0_p(pat) &&
		 !insn_references_resource_p(pat,&set,1))
		{
		  /* 
		    Now check to see that we're not reseting anything 
		    that is set in the slots or above in this thread.
		    */
		  rtx trial2;
		  struct resources set2;
		  CLEAR_RESOURCE(&set2);
		  mark_set_resources(insn,&set2,1);
		  
		  for(trial2=next_nonnote_insn(PREV_INSN(thread));
		      !stop_search_p(trial2,1) && trial2!=trial; 
		      trial2 = next_nonnote_insn(trial2))
		    {
		      mark_set_resources(trial2, &set2,1);
		    }
		  
		  if(!insn_sets_resource_p(trial,&set2,1))
		    {
		      update_block (trial, thread);
		      delete_insn (trial);
		      INSN_FROM_TARGET_P (trial) = 0;
		      found_one = 1;
		    }
		}
...
/* 
  Another possibility is that these slots came from the top of thread.
   If they do, then we don't need them in the top of thread (that is,
   only if we fall through right into thread).
...
*/
  ...
  if (INSN_UID (next_insn) == INSN_UID (thread)
      && !annul_p)
    {
      found_one=0; /*   Now, found_one will indicate a mismatch. */
      
      for(slot_number=1,trial=next_nonnote_insn(PREV_INSN (thread));
	  slot_number<num_slots && !found_one;
	  trial=next_active_insn(trial))
      {
	if(!rtx_equal_p(PATTERN(trial),PATTERN(next_to_match)))
	  found_one=1;
	next_to_match = XVECEXP(PATTERN(insn),0,++slot_number);
      }
      if(!found_one)
	for(trial=next_nonnote_insn(PREV_INSN(thread)),slot_number=1;
	    slot_number<num_slots; 
	    slot_number++, trial = next_trial)
	  {
	    next_trial=next_nonnote_insn(trial);
	    update_block (trial, thread);
	    delete_insn (trial);
...
/* I don't think this part below works.  Sure, without it
   we get sub-optimal code, but with it, it doesn't work.
   I've been trying to put more "smarts" up above so it can
   do the job correctly.  I suppose I need to "dive in" to
   the code below and really fix it, though. -JK 12/8/92 */

  /*
   * Now we have a merge_list as in the annuling case previously.
   * we will analyize the entire thread to ensure that we can
   * delete insns..
   */
  ...
  for (reversed_insns = reverse_list (merged_insns,0), i = slot_number-1; 
       reversed_insns; reversed_insns = XEXP (reversed_insns, 1), --i)
    {
      rtx rtrial = XEXP (reversed_insns, 0);
	  ...
	  && eligible_for_delay (delay_insn, slot_number - 1, trial))
	{
	  ...
	  if (! annul_p)
	    mark_referenced_resources (slot, &needed, 1);
   ...
   *
   * mmh:
   * I think that this is wrong if we are going to delete the insn from the
   * delay_slot because there is a conflict we have to reemit the insn on its 
   * correct thread.
   */
  if (slot_number != num_slots
     ...
     rtx delay_list;
{
  ...
  if (trial == 0 
      || trial == insn) /* mmh Look if trial is this insn then its not redudant but
			   its nescary. */
    return 0;
