...
extern int reload_reuse[MAX_RELOADS];
...
#ifdef DSP21XX
  RELOAD_FOR_SECONDARY_INPUT, /* added by jk so that a spill reg could be used to satisfy 
				 multiple secondary reload register needs */
  RELOAD_FOR_SECONDARY_OUTPUT, /* same as *SECONDARY_INPUT */
#endif
  RELOAD_FOR_INPUT,
  RELOAD_FOR_OUTPUT,
  RELOAD_OTHER
...
extern enum reload_when_needed reload_when_needed_obscure[MAX_RELOADS];
...
extern rtx reduce_addr_operand PROTO((rtx));
