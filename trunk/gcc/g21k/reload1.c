     ...
     FILE *dumpfile;
{
  ...
  for (insn = first; insn; insn = NEXT_INSN (insn))
	 ...
	 */
      ...
      for(i=0; set && find_reg_note(insn,REG_INC,i); i++)
	{
	  rtx x= XEXP (find_reg_note(insn,REG_INC,i), 0);
	  if(GET_CODE(x)==REG)
	    reg_equiv_constant[REGNO(x)] = 0;
	}
  ...
  while (something_changed)
	  ...
	  && caller_save_spill_class  == NO_REGS)
	{
	  ...
	  caller_save_spill_class
		...
		INDEX_REG_CLASS1 (stack_pmode, REG_NONE) :
		    BASE_REG_CLASS1 (stack_pmode, REG_NONE);
#endif
     ...
     FILE *dumpfile;
{
  ...
  if (fixed_regs[regno] || TEST_HARD_REG_BIT (forbidden_regs, regno))
    fatal ("reload bug: fixed or forbidden register was spilled.\n\
This may be due to a compiler bug or to impossible asm statements.");
  ...
  if (dumpfile) {
    fprintf (dumpfile, ";; Spilling reg %d (%s).\n", spill_regs[n_spills], reg_names[spill_regs[n_spills]]);
    fflush (dumpfile);
  }
      ...
      else if (GET_RTX_CLASS (GET_CODE (insn)) == 'i')
	{
	  ...
	  if (n_reloads > 0)
		...
		if (basic_block_needs[class] != 0
		 ...
		 Maybe, if the operand in question is in a 
		 vaild stackslot then consider that this copy might be legal. 
		 (mmh: March 23, 1994) */
		...
		for (p = NEXT_INSN (prev); p != next; p = NEXT_INSN (p))
		  if (p != insn && GET_RTX_CLASS (GET_CODE (p)) == 'i'
		    ...
		    {
		      /* Ok lets walk over each operand and replace it with
			 the memory location if it is in a psedo register
			 and save the origional rtx. */
		      int insn_code_num = INSN_CODE (p);
		      int noperands = 0;
		      int i, changes = 0;
		      rtx save_operands[MAX_RECOG_OPERANDS];
		      rtx pattern = PATTERN (p);

		      if (insn_code_num > 0)
			noperands = insn_n_operands[insn_code_num];
		      ...
		      for (i = 0; i < noperands; i++)
			{
			  ...
			  if ((GET_CODE (recog_operand[i]) == SUBREG
			       && REG_P (XEXP (recog_operand[i], 0))
			       && (regno = REGNO (XEXP (recog_operand[i], 0))) >
			          FIRST_PSEUDO_REGISTER
			       && reg_equiv_mem [REGNO (XEXP (recog_operand[i], 0))])
			      || (REG_P (recog_operand[i])
				  && (regno = REGNO (recog_operand[i])) > FIRST_PSEUDO_REGISTER
				  && reg_equiv_mem [REGNO (recog_operand[i])]))
			    {
			      save_operands[i] = recog_operand[i];
			      XEXP (pattern, i) = reg_equiv_mem [regno];
			      changes++;
			    }
			  else
			    save_operands[i] = NULL_RTX;
			}

		      /* Here we validate if we made any changes,
		         otherwise we just issue an error an delete the insn. */
		      if (changes == 0
			  || (recog_memoized (p) < 0
			      || (insn_extract (p),
				  ! constrain_operands (INSN_CODE (p), 1))))
			...
			{
			  /* Now replace all the saved_operands, back in the insn.   */
			  for (i = 0; i < noperands; i++)
			    {
			      if (save_operands[i])
				XEXP (pattern, i) = save_operands[i];
			    }
			}
		    }
#ifdef AUTO_INC_DEC
	      /* pr:940  (query-pr "940 --full") <- C-xC-e
		 Scan each of the notes associated with 'insn' and if the note is a
		 REG_INC then scan each of the reloads and then check to see if there
		 is a POST_MODIFY and the argument is the same as the regnote then add
		 a note to the insn. 
		 
		 The following code ensures that REG_INC notes are maintained. */
		...
		if (REG_NOTE_KIND (x) == REG_INC)
		  {
		    ...
		    for (p = NEXT_INSN (prev); p != next; p = NEXT_INSN (p))
		      if (p != insn && GET_RTX_CLASS (GET_CODE (p)) == 'i') {
			rtx src, dst, note, what;
			src = SET_SRC (PATTERN (p));
			dst = SET_DEST (PATTERN (p));
			note = XEXP (x, 0);
			what = NULL_RTX;
			...
			if (what
			    && (GET_CODE (XEXP (what, 0)) == POST_MODIFY
				|| GET_CODE (XEXP (what, 0)) == PRE_MODIFY
				|| GET_CODE (XEXP (what, 0)) == POST_INC
				|| GET_CODE (XEXP (what, 0)) == POST_DEC
				|| GET_CODE (XEXP (what, 0)) == PRE_INC
				|| GET_CODE (XEXP (what, 0)) == PRE_DEC)
			    && REGNO (XEXP (XEXP (what, 0), 0)) == REGNO (note)) {
			  
			  REG_NOTES (p) = gen_rtx (EXPR_LIST, REG_INC,
						   note,
						   REG_NOTES (p));
			}
		      }
     ...
     rtx avoid_return_reg;
{
  ...
  for (inheritance = optimize > 0; inheritance >= 0; inheritance--)
      ...
      for (j = 0; j < n_reloads; j++)
	{
	  ...
	  /* If we are trying to allocate a register for a reload
             which was marked as reload_reuse then skipping the rest of the
	     algorithim because we are piggy backing on other reloads. */
	  if (reload_reuse[j] >= 0) {
	    continue;
	  }
      ...
      for (j = 0; j < n_reloads; j++)
	{
	  ...
	  /* If we are trying to allocate a register for a reload
             which was marked as reload_reuse then skipping the rest of the
	     algorithim because we are going to inherit. */
	  if (reload_reuse[r] >= 0)
	    continue;
  ...
  for (j = 0; j < n_reloads; j++)
    {
      ...
      /* If we are trying to allocate a register for a reload
	 which was marked as reload_reuse then copy the reload_reg_rtx
	 from the appropriate reload and continue skipping the rest of the
	 algorithim because we are done. */
      if (reload_reuse[r] >= 0) {
	reload_reg_rtx[r] = reload_reg_rtx[reload_reuse[r]];
      }
  ...
  for (j = 0; j < n_reloads; j++)
	  ...
	  && reload_reg_rtx[j] != 0)
	{
	  ...
	  if (! special)
	      ...
	      if (reload_secondary_reload[j] >= 0)
		{
		  ...
		  if (second_reload_reg)
		      ...
		      else
			{
			  ...
			  if (tertiary_icode != CODE_FOR_nothing)
			    ...
			    {
			      reload_insn
				= gen_input_reload (reloadreg, second_reload_reg,
						    oldequiv, where);
			      if (this_reload_insn == 0)
				this_reload_insn = reload_insn;
	      ...
	      if (! special && !second_reload_reg) /* This is to short circut above. */
		{
		  reload_insn = gen_input_reload (reloadreg, NULL_RTX, oldequiv, where);
		  if (this_reload_insn == 0)
		    this_reload_insn = reload_insn;
		}
	  ...
	  && reload_inherited[j])
	{
	  ...
	  /* Likewise for input reloads that were inherited.  */
	    ...
	    {
	      rtx prev;
	      /* If this is not a reg then we die. */
	      if (GET_CODE (reload_reg_rtx[j]) == REG)
		{
...
#ifdef SECONDARY_OUTPUT_RELOAD_CLASS
	  if (! special
	      && second_reloadreg  /* without this we die. */
...
rtx
gen_input_reload (reloadreg, secondary_reg, in, before_insn)
     rtx reloadreg, secondary_reg;
     rtx in;
     rtx before_insn;
{
...
#if defined (DSP21XX) || (1 && defined (A21C0))
{
    rtx tmp;
    rtx emit_21xx_frame_address();
    tmp = emit_21xx_frame_address(reloadreg, secondary_reg, in, before_insn);
    if (tmp) 
	return tmp;
}
#endif
  ...
  {
#if defined(ADI) && defined(DSP21K)
    /*
     * For MAC operations we may need a secondary reload register. PR 900.
     * 02/20/96 -- lev
     */
    if (IS_MAC_OPERATOR(before_insn) && secondary_reg)
      {
	emit_insn_before (gen_move_insn (secondary_reg, in), before_insn);
	emit_insn_before (gen_move_insn (reloadreg,secondary_reg), 
			  before_insn);
      }
    else
      emit_insn_before (gen_move_insn (reloadreg, in), before_insn);
#else
    /* Now if we have a secondary_reg shouldn't we use it?
       My understanding is that we should use the register 
       Maybe we need a piece of code here that ensures that the move is needed.
       */
    if (secondary_reg) {
      emit_insn_before (gen_move_insn (secondary_reg, in), before_insn);
      in = secondary_reg;
...
#if defined (DSP21XX) || defined (A21C0)
rtx
emit_21xx_frame_address (reloadreg, secondary_reg, in, before_insn)
    rtx reloadreg, secondary_reg, in, before_insn;
{
    rtx base, offset, first_insn, i;
    int frame_ptr_p, good_offset_p;
...
#ifdef A21C0
    first_insn = emit_insn_before (gen_rtx 
				   (SET, GET_MODE (base), reloadreg, base), 
				   before_insn);
#else
    first_insn = emit_insn_before (gen_rtx (SET, DMmode, reloadreg, offset),
				   before_insn);
#endif
    ...
    if (!(CONSTANT_8bitP(offset) || (REG_P(offset)))) {
      if (secondary_reg) {
	emit_insn_before (gen_rtx (SET, GET_MODE (base), secondary_reg, offset), before_insn);
	offset = secondary_reg;
      }
    }

    i = emit_insn_before (gen_rtx 
			  (SET, GET_MODE (base), reloadreg,
			   gen_rtx (PLUS, GET_MODE (base), reloadreg,
				    offset)),
			  before_insn);
#else
    i = emit_insn_before (gen_rtx (SET, DMmode, reloadreg,
				  gen_rtx (PLUS, DMmode, reloadreg, base)),
			  before_insn);
#endif
