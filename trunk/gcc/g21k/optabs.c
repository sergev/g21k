...
optab fr_float_fract_optab;
optab fr_fract_float_optab;
...
rtx fract_float_sf_libfunc;
rtx fract_float_df_libfunc;
rtx float_fract_sf_libfunc;
rtx float_fract_df_libfunc;
...
#ifdef ADI
  rtx start_attempt=get_last_insn();
#endif
	...
	case DIV:
	  ...
	  if (! imag1)
	      ...
	      else		/* ((a+ib)(c-id))/divider */
		{
	      ...
	      if (res == NULL_RTX) 
		{
		  emit_conflict_block = 0;
		  res = expand_divmod (0, 
				       class == MODE_COMPLEX_FLOAT ? RDIV_EXPR : TRUNC_DIV_EXPR, 
				       submode, real_t, divisor, realr, unsignedp);
		}
	      ...
	      if (res == NULL_RTX) 
		{
		  ...
		  res = expand_divmod (0, 
				       class == MODE_COMPLEX_FLOAT ? RDIV_EXPR : TRUNC_DIV_EXPR, 
				       submode, imag_t, divisor, imagr, unsignedp);
		}
#ifdef DSP21K
	      if (emit_conflict_block && KEEP_CODE_SMALL)
		emit_conflict_block = 0;
#endif
	      if (res != imagr)
		emit_move_insn (imagr, res);
...
#ifdef ADI
  delete_insns_since(start_attempt);
#endif
  ...
  if (mode == BLKmode)
...
#endif
	{
	  ...
	  emit_library_call (memcmp_libfunc, 0,
			     ...
			     XEXP (x, 0), Pmode1 (Extract_rtx_Pmode (XEXP (x, 0))), 
...
#ifdef ADI
      emit_insn (GEN_FCN (icode) (x, y,gen_rtx(comparison,mode,x,y)));
#else
     ...
     rtx loc;
{
  if (! ((*insn_operand_predicate[(int)CODE_FOR_indirect_jump][0])
	 (loc, Pmode1 (Extract_rtx_Pmode (loc)))))
    loc = copy_to_mode_reg (Pmode1 (Extract_rtx_Pmode (loc)), loc);
...
#endif
#ifdef HAVE_extenddmsi
  if (HAVE_extenddmsi)
    extendtab[(int) SImode][(int) DMmode][0] = CODE_FOR_extenddmsi;
#endif
#ifdef HAVE_extendpmdm
  if (HAVE_extendpmdm)
    extendtab[(int) SImode][(int) PMmode][0] = CODE_FOR_extendpmdm;
#endif
#ifdef HAVE_extendpmsi
  if (HAVE_extendpmsi)
    extendtab[(int) SImode][(int) PMmode][0] = CODE_FOR_extendpmsi;
#endif
...
#endif
#ifdef HAVE_zero_extendpmdm
  if (HAVE_zero_extendpmdm)
    extendtab[(int) DMmode][(int) PMmode][1] = CODE_FOR_zero_extendpmdm;
#endif
#ifdef HAVE_zero_extenddmsi
  if (HAVE_zero_extenddmsi)
    extendtab[(int) SImode][(int) DMmode][1] = CODE_FOR_zero_extenddmsi;
#endif
    ...
    register char suffix;
{
  ...
  for (mode = first_mode; (int) mode <= (int) last_mode;
	...
	*p++ = tolower (*q);
	...
	= gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), libfunc_name);
    }
}
...
static void
init_fract_libfuncs (optable, opname, suffix)
    register optab optable;
    register char *opname;
    register char suffix;
{
...
init_optabs ()
{
  ...
  fr_float_fract_optab = init_optab(UNKNOWN);
  fr_fract_float_optab = init_optab(UNKNOWN);

  fr_smul_optab = init_optab(MULT);
  fr_sdiv_optab = init_optab(DIV);
  ...
  smul_optab->handlers[(int) SImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), MULSI3_LIBCALL);
#endif
#ifdef MULDI3_LIBCALL
  smul_optab->handlers[(int) DImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), MULDI3_LIBCALL);
#endif
#ifdef MULTI3_LIBCALL
  smul_optab->handlers[(int) TImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), MULTI3_LIBCALL);
#endif

/* We need these for the 21k and dsp */
#ifdef HAVE_umulqi3
  if (HAVE_umulqi3)
...
#ifdef HAVE_umulpsi3
  if (HAVE_umulpsi3)
    umul_optab->handlers[(int) PSImode].insn_code = CODE_FOR_umulpsi3;
#endif
#ifdef HAVE_umulsi3
  if (HAVE_umulsi3)
...
#ifdef HAVE_umulti3
  if (HAVE_umulti3)
    umul_optab->handlers[(int) TImode].insn_code = CODE_FOR_umulti3;
#endif
#ifdef MULSI3_LIBCALL
  umul_optab->handlers[(int) SImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), UMULSI3_LIBCALL);
#endif
#ifdef MULDI3_LIBCALL
  umul_optab->handlers[(int) DImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), UMULDI3_LIBCALL);
#endif
#ifdef UMULTI3_LIBCALL
  umul_optab->handlers[(int) TImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), UMULTI3_LIBCALL);
#endif
/* end of unsigned multiplies */
  ...
  sdiv_optab->handlers[(int) SImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), DIVSI3_LIBCALL);
#endif
#ifdef DIVDI3_LIBCALL
  sdiv_optab->handlers[(int) DImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), DIVDI3_LIBCALL);
#endif
#ifdef DIVTI3_LIBCALL
  sdiv_optab->handlers[(int) TImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), DIVTI3_LIBCALL);
#endif
  ...
  udiv_optab->handlers[(int) SImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), UDIVSI3_LIBCALL);
#endif
#ifdef UDIVDI3_LIBCALL
  udiv_optab->handlers[(int) DImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), UDIVDI3_LIBCALL);
#endif
#ifdef UDIVTI3_LIBCALL
  udiv_optab->handlers[(int) TImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), UDIVTI3_LIBCALL);
#endif
  ...
  smod_optab->handlers[(int) SImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), MODSI3_LIBCALL);
#endif
#ifdef MODDI3_LIBCALL
  smod_optab->handlers[(int) DImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), MODDI3_LIBCALL);
#endif
#ifdef MODTI3_LIBCALL
  smod_optab->handlers[(int) TImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), MODTI3_LIBCALL);
#endif
  ...
  umod_optab->handlers[(int) SImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), UMODSI3_LIBCALL);
#endif
#ifdef UMODDI3_LIBCALL
  umod_optab->handlers[(int) DImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), UMODDI3_LIBCALL);
#endif
#ifdef UMODTI3_LIBCALL
  umod_optab->handlers[(int) TImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), UMODTI3_LIBCALL);
#endif
  ...
  init_complex_libfuncs (smin_optab, "min", '3');
  ...
  init_complex_libfuncs (smax_optab, "max", '3');
  ...
  abs_optab->handlers[(int) DCmode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "cabs");
  ...
  ffs_optab->handlers[(int) SImode].libfunc
    = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "ffs");
...
#ifdef HAVE_movcsi
  if (HAVE_movcsi)
    mov_optab->handlers[(int) CSImode].insn_code = CODE_FOR_movcsi;
#endif
#ifdef HAVE_movsc
  if (HAVE_movsc)
    mov_optab->handlers[(int) SCmode].insn_code = CODE_FOR_movsc;
#endif
#ifdef HAVE_movdc
  if (HAVE_movdc)
    mov_optab->handlers[(int) DCmode].insn_code = CODE_FOR_movdc;
#endif
...
#ifdef HAVE_cmpcsi
  if (HAVE_cmpcsi)
    cmp_optab->handlers[(int) CSImode].insn_code = CODE_FOR_cmpcsi;
#endif
#ifdef HAVE_cmpsc
  if (HAVE_cmpsc)
    cmp_optab->handlers[(int) SCmode].insn_code = CODE_FOR_cmpsc;
#endif
#ifdef HAVE_cmpdc
  if (HAVE_cmpdc)
    cmp_optab->handlers[(int) DCmode].insn_code = CODE_FOR_cmpdc;
#endif
  ...
  init_complex_libfuncs (cmp_optab, "cmp", '2');
...
#ifdef HAVE_tstdm
  if (HAVE_tstdm)
    tst_optab->handlers[(int) DMmode].insn_code = CODE_FOR_tstdm;
#endif
#ifdef HAVE_tstpm
  if (HAVE_tstpm)
    tst_optab->handlers[(int) PMmode].insn_code = CODE_FOR_tstpm;
#endif
#ifdef HAVE_tstdi
  if (HAVE_tstdi)
...
#ifdef HAVE_tstcsi
  if (HAVE_tstcsi)
    tst_optab->handlers[(int) CSImode].insn_code = CODE_FOR_tstcsi;
#endif
#ifdef HAVE_tstsc
  if (HAVE_tstsc)
    tst_optab->handlers[(int) SCmode].insn_code = CODE_FOR_tstsc;
#endif
#ifdef HAVE_tstdc
  if (HAVE_tstdc)
    tst_optab->handlers[(int) DCmode].insn_code = CODE_FOR_tstdc;
#endif
...
#ifdef HAVE_movdm
  if (HAVE_movdm)
    mov_optab->handlers[(int) DMmode].insn_code = CODE_FOR_movdm;
#endif

#ifdef HAVE_movpm
  if (HAVE_movpm)
    mov_optab->handlers[(int) PMmode].insn_code = CODE_FOR_movpm;
#endif

#ifdef HAVE_adddm3
  if (HAVE_adddm3)
    add_optab->handlers[(int) DMmode].insn_code = CODE_FOR_adddm3;
#endif

#ifdef HAVE_addpm3
  if (HAVE_addpm3)
    add_optab->handlers[(int) PMmode].insn_code = CODE_FOR_addpm3;
#endif

#ifdef HAVE_subdm3
  if (HAVE_subdm3)
    sub_optab->handlers[(int) DMmode].insn_code = CODE_FOR_subdm3;
#endif

#ifdef HAVE_subpm3
  if (HAVE_subpm3)
    sub_optab->handlers[(int) PMmode].insn_code = CODE_FOR_subpm3;
#endif

#ifdef HAVE_cmpdm
  if (HAVE_cmpdm)
    cmp_optab->handlers[(int) DMmode].insn_code = CODE_FOR_cmpdm;
#endif

#ifdef HAVE_cmppm
  if (HAVE_cmppm)
    cmp_optab->handlers[(int) PMmode].insn_code = CODE_FOR_cmppm;
#endif
#ifdef HAVE_negdm2
  if (HAVE_negdm2)
    neg_optab->handlers[(int) DMmode].insn_code = CODE_FOR_negdm2;
#endif
#ifdef HAVE_negpm2
  if (HAVE_negpm2)
    neg_optab->handlers[(int) PMmode].insn_code = CODE_FOR_negpm2;
#endif
#ifdef HAVE_absdm2
  if (HAVE_absdm2)
    abs_optab->handlers[(int) DMmode].insn_code = CODE_FOR_absdm2;
#endif
#ifdef HAVE_abspm2
  if (HAVE_abspm2)
    abs_optab->handlers[(int) PMmode].insn_code = CODE_FOR_abspm2;
#endif
#ifdef HAVE_iordm3
  if (HAVE_iordm3)
    ior_optab->handlers[(int) DMmode].insn_code = CODE_FOR_iordm3;
#endif
#ifdef HAVE_iorpm3
  if (HAVE_iorpm3)
    ior_optab->handlers[(int) PMmode].insn_code = CODE_FOR_iorpm3;
#endif
#ifdef HAVE_lshldm3
  if (HAVE_lshldm3)
    lshl_optab->handlers[(int) DMmode].insn_code = CODE_FOR_lshldm3;
#endif
#ifdef HAVE_lshlpm3
  if (HAVE_lshlpm3)
    lshl_optab->handlers[(int) PMmode].insn_code = CODE_FOR_lshlpm3;
#endif
#ifdef HAVE_lshrdm3
  if (HAVE_lshrdm3)
    lshr_optab->handlers[(int) DMmode].insn_code = CODE_FOR_lshrdm3;
#endif
#ifdef HAVE_lshrpm3
  if (HAVE_lshrpm3)
    lshr_optab->handlers[(int) PMmode].insn_code = CODE_FOR_lshrpm3;
#endif

#endif /* ADI and MEMSEG */


#ifdef HAVE_fr_smulqi3
  if(HAVE_fr_smulqi3)
      fr_smul_optab->handlers[(int) QImode].insn_code = CODE_FOR_fr_smulqi3;
#endif

#ifdef HAVE_fr_smulhi3
  if(HAVE_fr_smulhi3)
      fr_smul_optab->handlers[(int) HImode].insn_code = CODE_FOR_fr_smulhi3;
#endif

#ifdef HAVE_fr_smulsi3
  if(HAVE_fr_smulsi3)
      fr_smul_optab->handlers[(int) SImode].insn_code = CODE_FOR_fr_smulsi3;
#endif

#ifdef HAVE_fr_smuldi3
  if(HAVE_fr_smuldi3)
      fr_smul_optab->handlers[(int) DImode].insn_code = CODE_FOR_fr_smuldi3;
#endif

#ifdef HAVE_fr_smulti3
  if(HAVE_fr_smulti3)
      fr_smul_optab->handlers[(int) TImode].insn_code = CODE_FOR_fr_smulti3;
#endif

#ifdef HAVE_fr_smulxi3
  if(HAVE_fr_smulxi3)
      fr_smul_optab->handlers[(int) XImode].insn_code = CODE_FOR_fr_smulxi3;
#endif

  init_fract_libfuncs (fr_smul_optab, "fr_smul", '3');

/* signed fractional division */

#ifdef HAVE_fr_sdivqi3
  if(HAVE_fr_sdivqi3)
      fr_sdiv_optab->handlers[(int) QImode].insn_code = CODE_FOR_fr_sdivqi3;
#endif

#ifdef HAVE_fr_sdivhi3
  if(HAVE_fr_sdivhi3)
      fr_sdiv_optab->handlers[(int) HImode].insn_code = CODE_FOR_fr_sdivhi3;
#endif

#ifdef HAVE_fr_sdivsi3
  if(HAVE_fr_sdivsi3)
      fr_sdiv_optab->handlers[(int) SImode].insn_code = CODE_FOR_fr_sdivsi3;
#endif

#ifdef HAVE_fr_sdivdi3
  if(HAVE_fr_sdivdi3)
      fr_sdiv_optab->handlers[(int) DImode].insn_code = CODE_FOR_fr_sdivdi3;
#endif

#ifdef HAVE_fr_sdivti3
  if(HAVE_fr_sdivti3)
      fr_sdiv_optab->handlers[(int) TImode].insn_code = CODE_FOR_fr_sdivti3;
#endif

#ifdef HAVE_fr_sdivxi3
  if(HAVE_fr_sdivxi3)
      fr_sdiv_optab->handlers[(int) XImode].insn_code = CODE_FOR_fr_sdivxi3;
#endif

  init_fract_libfuncs (fr_sdiv_optab, "fr_sdiv", '3');

#ifdef HAVE_fract_float_sf
  if(HAVE_fract_float_sf)
      fr_fract_float_optab->handlers[(int) SFmode].insn_code = CODE_FOR_fract_float_sf;
#endif

#ifdef HAVE_fract_float_df
  if(HAVE_fract_float_df)
      fr_fract_float_optab->handlers[(int) DFmode].insn_code = CODE_FOR_fract_float_df;
#endif

#ifdef HAVE_float_fract_sf
  if(HAVE_float_fract_sf)
      fr_float_fract_optab->handlers[(int) SFmode].insn_code = CODE_FOR_float_fract_sf;
#endif

#ifdef HAVE_float_fract_df
  if(HAVE_float_fract_df)
      fr_float_fract_optab->handlers[(int) DFmode].insn_code = CODE_FOR_float_fract_df;
#endif

  fract_float_sf_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fract_float_sf");
  fract_float_df_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fract_float_df");

  float_fract_sf_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__float_fract_sf");
  float_fract_df_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__float_fract_df");


  extendsfdf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__extendsfdf2");
  extendsfxf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__extendsfxf2");
  extendsftf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__extendsftf2");
  extenddfxf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__extenddfxf2");
  extenddftf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__extenddftf2");

  truncdfsf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__truncdfsf2");
  truncxfsf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__truncxfsf2");
  trunctfsf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__trunctfsf2");
  truncxfdf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__truncxfdf2");
  trunctfdf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__trunctfdf2");

  memcpy_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "memcpy");
  bcopy_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "bcopy");
  memcmp_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "memcmp");
  bcmp_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__gcc_bcmp");
  memset_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "memset");
  bzero_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "bzero");

  eqsf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__eqsf2");
  nesf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__nesf2");
  gtsf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__gtsf2");
  gesf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__gesf2");
  ltsf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__ltsf2");
  lesf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__lesf2");

  eqdf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__eqdf2");
  nedf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__nedf2");
  gtdf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__gtdf2");
  gedf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__gedf2");
  ltdf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__ltdf2");
  ledf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__ledf2");

  eqxf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__eqxf2");
  nexf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__nexf2");
  gtxf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__gtxf2");
  gexf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__gexf2");
  ltxf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__ltxf2");
  lexf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__lexf2");

  eqtf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__eqtf2");
  netf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__netf2");
  gttf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__gttf2");
  getf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__getf2");
  lttf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__lttf2");
  letf2_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__letf2");

  floatsisf_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__floatsisf");
  floatdisf_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__floatdisf");
  floattisf_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__floattisf");

  floatsidf_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__floatsidf");
  floatdidf_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__floatdidf");
  floattidf_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__floattidf");

  floatsixf_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__floatsixf");
  floatdixf_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__floatdixf");
  floattixf_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__floattixf");

  floatsitf_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__floatsitf");
  floatditf_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__floatditf");
  floattitf_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__floattitf");

  fixsfsi_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixsfsi");
  fixsfdi_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixsfdi");
  fixsfti_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixsfti");

  fixdfsi_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixdfsi");
  fixdfdi_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixdfdi");
  fixdfti_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixdfti");

  fixxfsi_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixxfsi");
  fixxfdi_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixxfdi");
  fixxfti_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixxfti");

  fixtfsi_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixtfsi");
  fixtfdi_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixtfdi");
  fixtfti_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixtfti");

  fixunssfsi_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixunssfsi");
  fixunssfdi_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixunssfdi");
  fixunssfti_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixunssfti");

  fixunsdfsi_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixunsdfsi");
  fixunsdfdi_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixunsdfdi");
  fixunsdfti_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixunsdfti");

  fixunsxfsi_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixunsxfsi");
  fixunsxfdi_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixunsxfdi");
  fixunsxfti_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixunsxfti");

  fixunstfsi_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixunstfsi");
  fixunstfdi_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixunstfdi");
  fixunstfti_libfunc = gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__fixunstfti");
}

#ifdef BROKEN_LDEXP
