     ...
     int unsignedp;
{
     ...
     between signed and unsigned.  */
  {
    ...
    if (unsignedp) {
      opx = expand_binop (mode, umul_optab,
...

rtx
expand_fract_from_float (mode, op0, target, format)
     enum machine_mode mode;
     register rtx op0, target;
     tree format;
{
  enum insn_code icode 
    = fr_fract_float_optab->handlers[(int)GET_MODE (op0)].insn_code;
  
  if (icode != CODE_FOR_nothing)
    if (!insn_operand_predicate[(int)icode][1] 
	|| ((*insn_operand_predicate[(int)icode][1])(op0, GET_MODE (op0))))
      {
	rtx frmul = gen_reg_rtx (mode), x;
	if (x = 
	    insn_gen_function[(int)icode] (frmul, op0,
	       gen_rtx (CONST_INT, VOIDmode,
			TREE_INT_CST_LOW (TYPE_SIZE (format))
			- TYPE_BINARYPOINT (format))))
	  {
	    emit_insn (x);
	    
	    if (target) 
	      {
		emit_move_insn (target , frmul);
		frmul = target;
	      }
  ...
  fprintf (stderr, "expand_fract_from_float no conversion(mmh)\n");
  abort();
}

rtx
expand_float_from_fract (mode, op0, target, format )
     enum machine_mode mode;
     register rtx op0, target;
     tree format;
{
  enum insn_code icode = fr_float_fract_optab->handlers[(int)mode].insn_code;
  
  if (icode != CODE_FOR_nothing)
    if (!insn_operand_predicate[(int)icode][1] 
	|| ((*insn_operand_predicate[(int)icode][1])(op0, mode)))
      {
	rtx frmul = gen_reg_rtx (mode), x;
	if(x = insn_gen_function[(int)icode] (frmul, op0,
			   gen_rtx (CONST_INT, VOIDmode,
				    TREE_INT_CST_LOW (TYPE_SIZE (format))
				    - TYPE_BINARYPOINT (format))))
	  {
  ...
  fprintf (stderr, "expand_float_from_fract no conversion(mmh)\n");
  abort();
}
  ...
  switch (GET_CODE (x))
    ...
    case CONST_INT:
      t = build_int_2 (INTVAL (x),
		       TREE_UNSIGNED (type) && INTVAL (x) < 0 ? -1 : 0);
