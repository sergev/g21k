...
static char* __ident__ = "@(#) rtl.c 1.3@(#)";
     ...
     register rtx orig;
{
  ...
  copy->is_pointer = orig->is_pointer;
     ...
     FILE *infile;
{
  ...
  for (i = 0; i < GET_RTX_LENGTH (GET_CODE (return_rtx)); i++)
	  ...
	  while (1)
	      ...
	      if (c == '\\')
		{
		  ...
		  if (c == ';')
		    ...
		    continue;	/* Allow continued lines */
		  else if (c == EOF)
		    break;
		}
	      else if (c == '"')
...
#ifdef ADI
	/*
	  On ADI chips, it is nice to refer to registers by name
	  since there are so many kind. The following code should accept
	  both numbers and names, and give an error if the name
	  is not found
	  */
	tmp_int = -1;
	if (isdigit(*tmp_char))
	...
	if (tmp_code == REG)
	{
	    ...
	    int n_reg_names = sizeof(reg_names)/sizeof(char *);
	    if (tmp_int < 0)
		for(i=0; i<n_reg_names; i++)
		    if (!strcmp(reg_names[i], tmp_char)) {
			tmp_int = i;
			break;
		    }
	    if (tmp_int < 0) {
