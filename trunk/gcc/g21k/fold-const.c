	...
	case RDIV_EXPR:
	  {
	    register tree magsquared
	      = const_binop (PLUS_EXPR,
			     ...
			     notrunc);
	    t = build_complex (const_binop (code,
					    const_binop (PLUS_EXPR,
...
#ifndef ADI
  /** this code looks redundant -- rbitpos is not used **/
...
#ifdef ADI
  /* Unsigned things are much more expensive than signed things */
  return 0;
#endif
  ...
  switch (code)
      ...
      if (wins)
	{
	  if (TREE_CODE (arg0) == INTEGER_CST)
	    ...
	    t = build_complex (fold (build1 (NEGATE_EXPR, TREE_TYPE (TREE_REALPART (arg0)),
					     TREE_REALPART (arg0))),
			       fold (build1 (NEGATE_EXPR, TREE_TYPE (TREE_IMAGPART (arg0)),
					     TREE_IMAGPART (arg0))));
					     ...
					     arg1, TREE_OPERAND (arg0, 1)))
	{
	  ...
	  if (integer_zerop (TREE_OPERAND (arg0, 1))
		  ...
		  || (TREE_CODE (arg2) == MINUS_EXPR
		      && TREE_CODE (TREE_OPERAND (arg0, 0)) == MINUS_EXPR
		      && operand_equal_p (TREE_OPERAND (arg0, 0), 
					  fold (build1 (NEGATE_EXPR, type, arg2)), 0))))
	    switch (comp_code)
	      {
	      case EQ_EXPR:
