  ...
  switch (code)
      ...
      if (var == x)
	{
	  ...
	  *loc = replacement->new = gen_reg_rtx (promoted_mode);
	  ...
	  if (RTX_ISPOINTER_P (x)) 
	    RTX_ISPOINTER_P (*loc) = 1;
	  ...
	  if (GET_CODE (XEXP (x, 0)) == PLUS)
	      ...
	      else
		{
	      ...
	      instantiate_virtual_regs_1 (&XEXP (XEXP (x, 0), 1), object,
					  extra_insns);
	      new = gen_rtx (PLUS, Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)), 
			     new, XEXP (XEXP (x, 0), 1));
	    }
	  ...
	  if (!validate_change (object, &XEXP (x, 1), new, 0))
	      ...
	      if (! extra_insns)
		{
		 ...
		 constant with that register.  */
	      temp = gen_reg_rtx (Pmode1 (Extract_rtx_Pmode (XEXP (x,0))));
	      ...
	      else
		{
		  ...
		  new = gen_rtx (PLUS, Pmode1 (Extract_rtx_Pmode (x)), XEXP (x, 0), new);
		  XEXP (x, 0) = old;
      ...
      if (new)
	{
	  ...
	  if (!validate_change (object, loc, temp, 0))
		...
		return 0;
	      ...
	      emit_insns_before (seq, object);
	      if (! validate_change (object, loc, temp, 0)) { 
		  if (!validate_replace_rtx (x, temp, object)) {
			/*EK*/
			fatal ("Compiler Error! Cannot validate_replace_rtx");
			abort ();
		  }
	      }
	    }
     ...
     int second_time;
{
  ...
  for (parm = fnargs; parm; parm = TREE_CHAIN (parm))
					  ...
					  passed_type, ! last_named))
	{
	  ...
	  passed_pointer = 1;
	  passed_mode = nominal_mode = Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx));
	}
#endif
      ...
      if (! second_time)
	{
	  ...
	  if (offset_rtx == const0_rtx)
	    ...
	    stack_parm = gen_rtx (MEM, passed_mode,
				  gen_rtx (PLUS, 
					   Pmode1 (Extract_rtx_Pmode (internal_arg_pointer)),
	  ...
	  && stack_parm != 0)
	{
	  ...
	  if (offset_rtx == const0_rtx)
	    ...
	    stack_parm = gen_rtx (MEM, nominal_mode,
				  gen_rtx (PLUS, Pmode1 (Extract_rtx_Pmode (internal_arg_pointer)),
					   internal_arg_pointer, offset_rtx));
	       ...
	       || passed_pointer || parm == function_result_decl)
	{
	  ...
	  if (nominal_mode == passed_mode
	      ...
	      && reg_mentioned_p (virtual_incoming_args_rtx,
				  XEXP (entry_parm, 0)))
	    {
	      /* Only add register note when instruction was same size.  If
		 (for example) it was a DF expanded into TWO SI moves, then
		 we'd mark one half of the register pair as equivalent to the
		 memory.  This really screws things up later! */
	      rtx linsn = get_last_insn ();
	      rtx load = single_set (linsn);
	      if (load &&
		  GET_MODE_SIZE (GET_MODE (SET_DEST (load))) == GET_MODE_SIZE (GET_MODE (entry_parm)))
		REG_NOTES (linsn)
     ...
     tree var;
{
  ...
  if (basereg == fp->internal_arg_pointer)
	...
	fp->arg_pointer_save_area
	  = assign_outer_stack_local (Pmode1 (Extract_rtx_Pmode (fp)), 
				      GET_MODE_SIZE (Pmode1 (Extract_rtx_Pmode (fp))), 0, fp);

      addr = fix_lexical_addr (XEXP (fp->arg_pointer_save_area, 0), var);
      addr = memory_address (Pmode1 (Extract_rtx_Pmode (addr)), addr);

      base = copy_to_reg (gen_rtx (MEM, Pmode1 (Extract_rtx_Pmode (addr)), addr));
#else
     ...
     rtx tramp;
{
  ...
  rtx temp = gen_reg_rtx (Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)));
  temp = expand_binop (Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)), add_optab, tramp,
		       GEN_INT (TRAMPOLINE_ALIGNMENT - 1),
		       temp, 0, OPTAB_LIB_WIDEN);

  tramp = expand_binop (Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)), and_optab, temp,
...
expand_main_function ()
{
  ...
  emit_library_call (gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), "__main"), 0,
		     VOIDmode, 0);
     ...
     int parms_have_cleanups;
{
  ...
  if (current_function_needs_context)
    {
      last_ptr = assign_stack_local (Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)), 
				     GET_MODE_SIZE (Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx))), 0);
  ...
  if (aggregate_value_p (DECL_RESULT (subr)))
...
#endif
	{
	  ...
	  if (struct_value_incoming_rtx)
	    {
	      value_address = gen_reg_rtx (Pmode1 (Extract_rtx_Pmode (struct_value_incoming_rtx)));
  ...
  if (PROFILE)
  {
      extern void init_function_profiler();
      extern void emit_prologue_profiler(int);

      init_function_profiler();
      emit_prologue_profiler(1);
  }
#endif
	...
	break;
	 ...
	 is found at the place we always store it.  */
      ...
      last_ptr = plus_constant (last_ptr, - 
				GET_MODE_SIZE (Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx))));
#endif
      last_ptr = copy_to_reg (gen_rtx (MEM, Pmode1 (Extract_rtx_Pmode (stack_pointer_rtx)),
				       memory_address 
