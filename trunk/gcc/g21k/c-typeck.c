     ...
     tree type, like;
{
  ...
  if (TREE_MEMSEG(type) == UNKNOWN_MEM) seg = TREE_MEMSEG(like);
  if (TREE_MEMSEG(like) == UNKNOWN_MEM) seg = TREE_MEMSEG(type);
  return c_build_type_variant (type, constflag, volflag, seg);
#else
...
#ifdef ADI
	memory_segment seg = TREE_MEMSEG (t1);
	if (TREE_MEMSEG (t1) != TREE_MEMSEG (t2))
	  error ("The mixing of PM/DM pointers is undefined and hence I'm giving you an error");
#endif
...
#if defined(ADI) && defined(MEMSEG)
  /***** memory segments must be the same for *****/
  if (TREE_MEMSEG(t1) != TREE_MEMSEG(t2)) return 0;
#endif
...
#if defined(ADI) && defined(MEMSEG)
      /*** I am not 100% sure why, but the segment of restype ***/
      /*** seems to be set correctly, so we reuse it ****/
      if (TYPE_READONLY (type) || TYPE_VOLATILE (type)
	  || TREE_MEMSEG(restype) != default_memory_segment)
...
#if defined(ADI) && defined(MEMSEG)
	TREE_MEMSEG(ref) = TREE_MEMSEG(datum);
#endif
...
#if defined(ADI) && defined(MEMSEG)
	  mtype = c_build_type_variant(t,
				       TREE_READONLY(t),
				       TREE_THIS_VOLATILE(t),
				       TREE_MEMSEG(t));
#else
	  mtype = TYPE_MAIN_VARIANT (TREE_TYPE (type));
#endif
	  ref = build1 (INDIRECT_REF, mtype, pointer);
     ...
     tree array, index;
{
  ...
  if (TREE_CODE (TREE_TYPE (array)) == ARRAY_TYPE
      ...
      if (pedantic)
	{
      ...
      type = c_build_type_variant(TREE_TYPE (TREE_TYPE (array)),
				  TREE_READONLY(array),
...
#if defined(ADI) && defined(MEMSEG)
      /*** Propagate the memory segment of the array to the reference ***/
      TREE_MEMSEG(rval) = TREE_MEMSEG(array);
#endif
...
#if defined(ADI) && defined(MEMSEG)
      /*** functions are always in PROG_MEM ***/
  ...
  switch (code)
    ...
    case EXACT_DIV_EXPR:
      if ((code0 == INTEGER_TYPE || code0 == REAL_TYPE
	   || code0 == COMPLEX_TYPE || code0 == FRACT_TYPE)
	  && (code1 == INTEGER_TYPE || code1 == REAL_TYPE
	      || code1 == COMPLEX_TYPE || code1 == FRACT_TYPE))
	{
	  if (!((code0 == INTEGER_TYPE || code0 == FRACT_TYPE)
		&& (code1 == INTEGER_TYPE || code1 == FRACT_TYPE))
	      && !((code0 == COMPLEX_TYPE 
    ...
    case RSHIFT_EXPR:
      if ((code0 == INTEGER_TYPE && code1 == INTEGER_TYPE)
	  || (code0 == FRACT_TYPE && code1 == INTEGER_TYPE))
	{
	  if (TREE_CODE (op1) == INTEGER_CST)
	    {
    ...
    case LT_EXPR:
    case GT_EXPR:
      if ((code0 == INTEGER_TYPE || code0 == REAL_TYPE || code0 == FRACT_TYPE)
	  && (code1 == INTEGER_TYPE || code1 == REAL_TYPE || code1 == FRACT_TYPE))
	short_compare = 1;
      else if (code0 == POINTER_TYPE && code1 == POINTER_TYPE)
	{
	  if (! comp_target_types (type0, type1))
     ...
     int noconvert;
{
  ...
  switch (code)
    ...
    case CONVERT_EXPR:
	 ...
	 associativity, but won't generate any code.  */
      if (!(typecode == INTEGER_TYPE || typecode == REAL_TYPE
	    || typecode == COMPLEX_TYPE || typecode == FRACT_TYPE))
    ...
    case BIT_NOT_EXPR:
      if (typecode != INTEGER_TYPE && typecode != FRACT_TYPE)
	...
	error ("taking the address of an iterator is illegal '%s'", 
     ...
     char *string;
{
  ...
  if (TREE_CODE (arg) == VAR_DECL 
      && ITERATOR_P (arg))
    {
      ...
      error (buf, IDENTIFIER_POINTER (DECL_NAME (arg)));
    }
     ...
     int parmnum;
{
  ...
  if ((codel == INTEGER_TYPE || codel == REAL_TYPE || codel == ENUMERAL_TYPE
       || codel == COMPLEX_TYPE || codel == FRACT_TYPE)
       &&
      (coder == INTEGER_TYPE || coder == REAL_TYPE || coder == ENUMERAL_TYPE
       || codel == COMPLEX_TYPE || coder == FRACT_TYPE))
...
#if defined(ADI) && defined(MEMSEG)
      int polymorph =  funname?does_function_have_polymorphic_property (IDENTIFIER_POINTER (funname)):-1;
#endif
...
#if defined(ADI) && defined(MEMSEG)
	      if(!(polymorph >= parmnum)
		 && TREE_MEMSEG(ttl) != TREE_MEMSEG(ttr))
	      {
		  if (errtype)
		      error ("%s from segment-incompatible pointer types",
			     get_spelling (errtype));
		  else
		      error("Passing argument %d to segment-incompatible pointer type",
			    parmnum);
	      }

#if 0
		warn_for_assignment
		  ("%s from \"segment-incompatible\" pointer type",
		   get_spelling (errtype), funname, parmnum);
#endif

	      if( (polymorph >= parmnum)
		 && TREE_MEMSEG (TREE_TYPE (type)) != TREE_MEMSEG (TREE_TYPE (rhstype))) {
		  tree type_type = TREE_TYPE(type);
		  tree new_type_type =
		      c_build_type_variant (type_type,
					    TYPE_READONLY(type_type),
					    TYPE_VOLATILE(type_type),
					    TREE_MEMSEG(TREE_TYPE(rhstype)));
		  tree x = build_pointer_type(new_type_type);
		  type = x;
	      }
#endif
	    }
     ...
     tree endtype;
{
  switch (TREE_CODE (value))
	...
	return initializer_constant_valid_p (TREE_OPERAND (value, 0), endtype);
      ...
      if ((TREE_CODE (TREE_TYPE (value)) == INTEGER_TYPE
	   && TREE_CODE (TREE_TYPE (TREE_OPERAND (value, 0))) == INTEGER_TYPE)
	  || (TREE_CODE (TREE_TYPE (value)) == FRACT_TYPE
	      && TREE_CODE (TREE_TYPE (TREE_OPERAND (value, 0))) == FRACT_TYPE))
	{
     ...
     char *ofwhat;
{
  ...
  if (inside_init && TREE_TYPE (inside_init) != 0
	...
	inside_init = decl_constant_value (inside_init);
      ...
      if (require_constant && code == COMPLEX_TYPE)
	if (initializer_constant_valid_p (inside_init, TREE_TYPE (inside_init)) == 0) 
	  {
	    error_init ("initializer element%s is not a complex constant",
			" for `%s'", ofwhat);
	    inside_init = error_mark_node;
	  }
	else 
	  {
	    tree x = build_complex (TREE_REALPART (inside_init), 
				    TREE_IMAGPART (inside_init));
	    TREE_TYPE (x) = TREE_TYPE (inside_init);
	    return x;
	  }
  ...
  if (code == INTEGER_TYPE || code == REAL_TYPE || code == POINTER_TYPE
      || code == ENUMERAL_TYPE || code == COMPLEX_TYPE || code == FRACT_TYPE)
    {
      if (raw_constructor)
	{
	  if (element == 0)
