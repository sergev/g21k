...
/* Identification String */ static char *__ident__ = "@(#) unroll.c 2.2@(#)";
		  ...
		  if (! last_iteration
		      || (splittable_regs_updates[regno]-- != 1))
		    {
		      ...
		      map->reg_map[regno] = tem;
                      REGNO_POINTER_FLAG ( REGNO (tem)) = REGNO_POINTER_FLAG (regno);
		      RTX_ISPOINTER_P (tem) = RTX_ISPOINTER_P (giv_src_reg);
		    }
		  else
     ...
     enum machine_mode mode;
{
  ...
  /* The modes must all be the same.  This should always be true.  For now,
     check to make sure.  */
#if ! defined(DSP21K) || ! defined(MEMSEG)
...
#ifdef ADI
      /** See if the initial value is equivalent to **/
      /** an integer constant */
    {
	rtx iinsn = bl->init_insn;
	if (iinsn) {
	    rtx note = find_reg_note(iinsn, REG_EQUAL, 0);
	    if (note)
	    {
		rtx eqval = XEXP(note, 0);
		if (GET_CODE(eqval) == CONST_INT)
		    *initial_value = eqval;
	    }
	}
    }
     ...
     rtx loop_start, loop_end;
{
...
#ifdef ADI
  /***  In the future, we should be able to check for loop ***/
  /*** invariants here, not just constants. ***/
#endif
  ...
  if (GET_CODE (comparison_value) == REG && !invariant_p (comparison_value))
    {
      rtx val = reg_set_last (comparison_value, loop_end);
      if (val
	  && CONSTANT_P (val))
	{
...
#ifdef ADI
	      /** We want to get around a problem created by cse: **/
	      /** for some reason the comparison value get loaded from **/
	      /** another register with the same constant value. **/
	      /** In such case we switch comparison value to the source **/
	      /** of the assignment and continue the search. (az,3/17/92) **/

	      else
	      {
		  rtx set_source = SET_SRC(set);
		  if (GET_CODE(set_source) == REG) {
		      comparison_value = set_source;
		      continue;	      /*** to get around break ***/
		  }
	      }
#endif
