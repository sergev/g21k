...
#ifndef USE_CPPCOMMENTS
#define USE_CPPCOMMENTS 0
#endif
...
/* This is used to maintain an interal values retrived via genlab */
static int labno = 0;
...
enum node_type {
 ...
 T_GENLAB,      /* used for assembler label generation just give me a new integer */
...
#endif
{
  ...
  no_output = 0;
  cplusplus = 0;
  cplusplus_comments = USE_CPPCOMMENTS;
  ...
  for (i = 1; i < argc; i++) {
	...
	if (! strcmp (argv[i], "-lang-c"))
	  cplusplus = 0, cplusplus_comments = USE_CPPCOMMENTS, objc = 0;
	if (! strcmp (argv[i], "-lang-c++"))
  ...
  while (1) {
	...
	*obp++ = '!';
	...
	while (ibp < limit) {
      ...
      /* Strip out { ... } style assembler commnets. */
    case '{':
      if (1 && !lang_asm)
	goto randomchar;
      if (ip->macro != 0)
     ...
     FILE_BUF *op;
{
  ...
  case T_BASE_FILE:
	...
	string = instack[0].nominal_fname;
      ...
      if (string)
	{
	  int len=3;    /* Two double-quotes + NUL */
#if DIR_SEPARATOR == '\\'
	  char *p, *q;
	  ...
	  while (*p)
	    if (*p++ == '\\')
	      len++;
#endif
	  buf = (char *) alloca (len + strlen (string));
#if DIR_SEPARATOR == '\\'
	  buf[0] = '"';
	  p=string;
	  q=&buf[1];
     ...
     char *prec;
{
  ...
  while (*prec) {
	...
	return 0;
      ...
      prec += 5;
      while (is_hor_space[(unsigned char)*prec])
	prec++;
      name = prec;
      while (is_idchar[(unsigned char)*prec])
	prec++;
      len = prec - name;
...
/*  install ("__GNUC__", -1, T_CONST, 2, 0, -1);  */
  ...
  if (lang_asm)
    install ("__GENLAB__", -1, T_GENLAB, 0, 0, -1);
