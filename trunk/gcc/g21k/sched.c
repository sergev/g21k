     ...
     rtx x, y;
{
  ...
  /* If we are comparing memory references and x is y then why bother? mmh (query-pr "1014 --full") */
  if (x == y)
    return 1;
...
#if 0 && defined(ADI) && defined(MEMSEG)
		/** FRAME_POINTER_REGNUM is not a constant any more **/
		/** because of possible "pmstack" switch. On the other **/
		/** hand, arg_pointer is always the same as  **/
		/** frame pointer, so there is no need for checking **/
#else
...
algo: 2
    while (no more changes) do
      changes = false
      x = head
      y = scan forward bound by tail searching for a dependent insn
      if found y to be a dependant insn then
         mark x through prev(y) as pipeline stage(i)
	 mark y through tail as pivotal(i)
	
         for each insn in pivotal(i) do
             for each sin in stage(i) do
                if insn depends on sin then
                   remove_dependency (insn, sin)
                end if
             end for
         end for

         copy stage(i) before looptop
	 copy y through tail after loopbot
	 move stage(i) after tail
	 increment i
         decrement loopcount
         changes = true
      end if
...
*/
#define MOTION_BEFORE 0
#define MOTION_AFTER 1

#define SEARCH_BACKWARDS 0
#define SEARCH_FORWARDS 1

static 
move_rtx_list (rtx start, rtx end, rtx pos, int whence, FILE *file)
{
  rtx pnxt, pprv;
  fprintf (file, "moving.... %d-%d %s %d\n", 
 	  INSN_UID (start), INSN_UID (end), whence ? "before" : "after", INSN_UID (pos));

  /* unlink the [ start ... end ] from the instruction stream */
  NEXT_INSN (PREV_INSN (start)) = NEXT_INSN (end);
  PREV_INSN (NEXT_INSN (end)) = PREV_INSN (start);

  if (whence == 0) { /* before */
    pprv = PREV_INSN (pos);
    NEXT_INSN (pprv) = start;
    PREV_INSN (start) = pprv;
    PREV_INSN (pos) = end;
    NEXT_INSN (end) = pos;
  }
  else {
...
static 
delete_rtx_list (rtx start, rtx end, FILE *file)
{
  rtx insn;
  fprintf (file, "removing .... %d-%d\n", 
	   INSN_UID (start), INSN_UID (end));
...
static
copy_rtx_list (rtx start, rtx end, rtx pos, int whence, rtx *new_start, rtx *new_end, FILE *file)
{
  rtx insn;
  int first = 1;
  rtx x;

  fprintf (file, "copying.... %d-%d %s %d\n", 
	  INSN_UID (start), INSN_UID (end), whence ? "before" : "after", INSN_UID (pos));
  ...
  for (insn = start; insn != NEXT_INSN (end); insn = NEXT_INSN (insn)) {
    ...
    if (GET_RTX_CLASS (GET_CODE (insn)) == 'i') {
      ...
      if (whence == MOTION_AFTER) 
	x = pos = emit_insn_after (copy_rtx (PATTERN (insn)), pos);
      else
	x = emit_insn_before (copy_rtx (PATTERN (insn)), pos);
...
static
rtx
find_a_note (rtx start, int note_kind, rtx bound, int whence)
{
  rtx x;

  if (whence == SEARCH_BACKWARDS)
    for (x = start; x && x != bound; x = PREV_INSN (x)) {
      if (GET_CODE (x) == NOTE
	  && INSN_CODE (x) == note_kind)
	return x;
    }
  else 
    for (x = start; x && x != bound; x = NEXT_INSN (x)) {
      if (GET_CODE (x) == NOTE
	  && INSN_CODE (x) == note_kind) 
	return x;
    }
...
/* Remove the dependence between insn and depend. */

void
maybe_delete_dependence (insn, depend)
     rtx insn;
     rtx depend;
{
  rtx elem,note;
  ...
  elem = SET_DEST (PATTERN (depend));

  if (REG_P (elem)) {
    note = find_regno_note (insn, REG_DEAD, REGNO (elem));
    if (note)
     ...
     rtx x, reg;
{
  ...
  for (i = GET_RTX_LENGTH (code) - 1; i >= 0; i--)
    {
      if (fmt[i] == 'e')
	consumed |= does_source_consume (XEXP (x, i), reg);
      else if (fmt[i] == 'E')
	for (j = 0; j < XVECLEN (x, i); j++)
	  consumed |= does_source_consume (XVECEXP (x, i, j), reg);
    }
  return consumed;
}

void
maybe_make_depend (insn, depend)
     rtx insn;
     rtx depend;
{
  rtx note, dst;
  ...
  dst = SET_DEST (PATTERN (depend));
  if (does_source_consume (SET_SRC (PATTERN (insn)), dst)) {
    add_dependence (insn, depend, REG_DEAD);
...
/* Walk the instructions in the loop and reconstruct the dependencies. */

static int
analyze_loop_pipeline_stage (rtx new_stage_start, rtx new_stage_end, 
			     rtx stage_start, rtx stage_end, rtx pivotal_start, rtx pivotal_end)
{
  rtx y, x;

  /* First we will remove all the dependencies elliminated by this swap. */
  for (y = stage_start; y && y != NEXT_INSN (stage_end); y = NEXT_INSN (y)) {

    if (GET_RTX_CLASS (GET_CODE (y)) == 'i') {
      
      /* Walk the rest of the loop body and remove all dependencies related to
	 stage. */
      for (x = pivotal_start; x && x != NEXT_INSN (pivotal_end); x = NEXT_INSN (x)) {
	
	if (GET_RTX_CLASS (GET_CODE (x)) == 'i')
	  maybe_delete_dependence (x, y);
      }
    }
  }
  
  /* Now we will add any new dependency created by this rotation. */
  for (y = pivotal_start; y && y != NEXT_INSN (pivotal_end); y = NEXT_INSN (y)) {

    if (GET_RTX_CLASS (GET_CODE (y)) == 'i') {
      
      /* Walk the rest of the loop body and remove all dependencies related to
	 stage. */
      for (x = new_stage_start; x && x != NEXT_INSN (new_stage_end); x = NEXT_INSN (x)) {
	
	if (GET_RTX_CLASS (GET_CODE (x)) == 'i')
	  
	  maybe_make_depend (x, y);
      }
    }
  }
...
static int
software_loop_pipeline (int n_insns, rtx head, rtx tail, FILE *file)
{
  int i;
  int changed = 1;
  int found;
  rtx insn,x,y;
  rtx doloop, loop_begin, loop_end, body_tail;
  rtx begin, end, cont;
  rtx fillp, drainp;

  rtx fa,fb,sa,sb,da,db;
  rtx next_tail;
  ...
  /* A loop looks like this...
     ...
     begin:   (note ... NOTE_INSN_LOOP_BEGIN)

              (code_label ... <top>)    ----- DOLOOP1_START_LABEL

     doloop:  (unspec doloop node)

     body_head:
              ...  <<<<< body
     body_tail:

     cont:    (note ... NOTE_INSN_LOOP_CONT)

              (note ... NOTE_INSN_LOOP_VTOP)
     
              (jump_insn conditional branch to <top>)
     
              (code_label ... <end>)    ----- DOLOOP1_END_LABEL

     end:     (note ... NOTE_INSN_LOOP_END)

     */

  doloop     = prev_nonnote_insn (head);

  loop_begin = XEXP (DOLOOP1_START_LABEL (PATTERN (doloop)), 0);

  loop_end   = XEXP (DOLOOP1_END_LABEL (PATTERN (doloop)), 0);

  cont       = find_a_note (tail, NOTE_INSN_LOOP_CONT, loop_begin, SEARCH_BACKWARDS);

  begin      = find_a_note (loop_begin, NOTE_INSN_LOOP_BEG, NULL_RTX, SEARCH_BACKWARDS);

  end        = find_a_note (loop_end,   NOTE_INSN_LOOP_END, NULL_RTX, SEARCH_FORWARDS);

  body_tail  = PREV_INSN (cont);
  fillp      = NEXT_INSN (begin);
  drainp     = PREV_INSN (end);

  if (cont == NULL_RTX || end == NULL_RTX || begin == NULL_RTX)

    return 0;

  next_tail = tail; /* exclude the jump back to loop_begin */
  ...
  while (changed) {
    ...
    if (found) {
      ...
      fprintf (file, ";;; swp: breaking dependency at %d by loop rotation rule\n", 
	       INSN_UID (PREV_INSN (y)));

      /* define stage(i) = [ x ... prev(y) ] */

      /* define pivotal(i) = [ y ... tail ] */

      debug_rtxf (file, y);

      fprintf (file, "\n\n ;; stage(%d)\n", i);
      drtxlf (file, x, PREV_INSN (y));

      fprintf (file, "\n ;; pivotal(%d)\n", i);
      drtxlf (file, y, body_tail);

      /*           
       * This is a simple example of the transformation performed here.
       * the operands letters are here to show the dependencies on the instructions.
       *
       *                                                 mov a
       *                                                 mov b
       *0               1               2              3 mul c,a,b
       *                                 mov a           add d,d,c
       *                 mov a           mov b           mov a
       *                 mov b           mul c,a,b       mov b
       * ----           -------         -------          ------
       *  mov a          mul c,a,b       add d,d,c       mul c,a,b
       *  mov b   -->    add d,d,c   --> mov a      -->  add d,d,c
       *  mul c,a,b      mov a           mov b           mov a
       *  add d,d,c      mov b           mul c,a,b       mov b
       * -----          -------         -------          -------
       *                 mul c,a,b       add d,d,c       mul c,a,b
       *                 add d,d,c       mov a           add d,d,c
       *                                 mov b           mov a
       *                                 mul c,a,b       mov b
       *                                 add d,d,c       mul c,a,b
       *                                                 add d,d,c
       *
       *
       * The expansion is key to the understanding of the algorithim.
       *
       */

      /* FILL THE PIPELINE

 	   Copy the stage(i) to before the top of the loop. 
           keep begin an inclusive marker */
      copy_rtx_list (x, PREV_INSN (y),    fillp, MOTION_BEFORE, &fa, &fb, file);

      /* COPY TO STEADY STATE OF THE PIPELINE

  	   Copy the stage (i) to before the loop end, reversing dependencies */

      copy_rtx_list (x, PREV_INSN (y), cont,   MOTION_BEFORE, &sa, &sb, file);
      
      insn = PREV_INSN (y); /* new body_tail */

      /* FILL DRAIN OF THE PIPELINE WITH TAIL OF LOOP 

           Move the pivotal(i) to after the loop. 
           keep end an inclusive marker */
      copy_rtx_list (y, body_tail,  drainp,   MOTION_AFTER, &da, &db, file);
      
      /* Analyize the new dependencies of the new transformed loop body */
      
      analyze_loop_pipeline_stage (sa, sb, x, PREV_INSN (y), y, body_tail);

      delete_rtx_list (x, PREV_INSN (y), file);

      body_tail = PREV_INSN (cont);  /* this is the new tail of the loop. */
      head = y;                      /* now the pivotal is the head of the loop */

      i++;
      changed = 1;

      fprintf (file, "\n\n\n\n\n;;---------------------\n;; The loop pipeline...at %d iterations.\n",i);
      drtxlf (file, begin, end);
  ...
  fprintf (file, "\n;;---------------------\n;; The final loop pipeline...n stages %d.\n",i);
  drtxlf (file, begin, end);
  fflush (file);
...
#define NEED_TAIL	2
...
#ifdef ADI
	/* 
	 * To facilitate DOLOOPS we need to tell the scheduler to ignore them.
	 * So what we do is treat the DOLOOP as if it were not part of the loop.
	 */
	if (doloop_start_p (head))
	  if (tail == head) 
	    return;
	  else {
	    in_loop_p = 1;
	    loop_depth = INTVAL (DOLOOP1_DEPTH (PATTERN (head)));
	    snglblock_loop_p = SNGL_BB_LOOP_P (head);
	    head = NEXT_INSN (head);
	  }
#endif
      }
  ...
  if (flag_software_loop_pipeline
      && snglblock_loop_p 
      && !reload_completed) {
    
    software_loop_pipeline (n_insns, head, tail, file);
    return ;
  }
