    ...
    int in_group;
{
  ...
  /* This change might be a natural thing to do in a general case,
     but we have only been bitten by the bug which appears with pm/dm
     code (az,1/13/93) */

  if (GET_CODE(new) == CONST_INT &&
      GET_MODE(old) != VOIDmode &&
      GET_MODE(old) != SImode)
      new = gen_rtx(CONST_INT, GET_MODE (old), INTVAL (new));
#endif

#if defined(ADI) && !defined (A21C0)

/* For now, we do not support well paradoxical SUBREG
 * The pointer info on the reg may be wrong but we do not
 * use it anyway. 03/07/97 -- lev
 */

  /* (view-pr "1298") 
     if this is a pointer then we need to propogate that information
     forward. */
  if (RTX_ISPOINTER_P (old))
    if (REG_P (new)) {
      if (!RTX_ISPOINTER_P (new)) {
	new = gen_rtx (SUBREG, GET_MODE (old), new, 0);
	mark_reg_pointer (new);
      }
    } else
      mark_reg_pointer (new);
#endif
...
#if 0
      /* The code below randomly substitutes the "to" without knowing if
	 the "from" actually matches the operand we're "investigating".
	 This led to things like hoisting an address expression like

	 m4=_L$27; i4=_L$27; modify(i4,m4);
        */
#ifdef ADI
      else if ((GET_MODE (x) == DMmode
		|| GET_MODE (x) == PMmode)
	       && CONSTANT_P (to))
	{
	  validate_change (object, loc, to, 1);
     ...
     enum machine_mode mode;
{
  ...
  /*
   * Check again for constant things
   */
  if (CONSTANT_P (op))
    return ((GET_MODE (op) == VOIDmode || GET_MODE (op) == mode)
#ifdef LEGITIMATE_PIC_OPERAND_P
  ...
  if (code == REG)
    /* Make sure that this is either an unbound register - OR - it is
       bound to a member of the GENERAL_REGS class. */
    return (REGNO (op) >= FIRST_PSEUDO_REGISTER
#if 0
	    || reg_classes_intersect_p( GENERAL_REGS, REGNO_REG_CLASS (REGNO (op)))
#else
  ...
  if (! reload_completed
	...
	return 0;
	 ...
	 operand.  */
      ...
      return ((offset == 0 && general_operand (XEXP (inner, 0), 
					       Pmode1 (Extract_rtx_Pmode (inner))))
	      || (GET_CODE (XEXP (inner, 0)) == PLUS
		  && GET_CODE (XEXP (XEXP (inner, 0), 1)) == CONST_INT
		  && INTVAL (XEXP (XEXP (inner, 0), 1)) == -offset
		  && general_operand (XEXP (XEXP (inner, 0), 0), 
				      Pmode1 (Extract_rtx_Pmode (inner)))));
    }

  return (GET_CODE (op) == MEM
	  && memory_operand (op, mode)
	  && general_operand (XEXP (op, 0), Pmode1 (Extract_rtx_Pmode (XEXP (op,0)))));
}
     ...
     register rtx y;
{
  ...
  if (ycode == PRE_DEC || ycode == PRE_INC
      ...
      || ycode == POST_MODIFY || ycode == PRE_MODIFY)
    return 0;
     ...
     int strict;
{
  ...
  while (which_alternative < nalternatives)
      ...
      for (opno = 0; opno < noperands; opno++)
	{
	  ...
	  /* Peel away CONST above obvious constants */
	  while (GET_CODE (op) == CONST && CONSTANT_P (XEXP (op, 0)))
	      op = XEXP (op, 0);
	  ...
	  while (*p && (c = *p++) != ',')
		...
		break;
            ...
            case 'z':
                ...
                if ( REG_P (op) && regno_pointer_flag &&
                         !REGNO_POINTER_FLAG (REGNO (op)))
                    goto usual_case;
                /*
                 * If was marked as a pointer, we DON'T want this to end up in
                 * an M register.
                 */
                break;
#endif /* DSP21K */
