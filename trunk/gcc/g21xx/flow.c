     ...
     FILE *file;
{
  ...
  /** But (says az on 8/12/92), with POST_MODIFY's we are likely to get **/
  /** more of them. */
  max_uid_for_flow += max_uid_for_flow;
#endif
...
#define FUDGE 1
  ...
  regset_size = (FUDGE * (max_regno + REGSET_ELT_BITS - 1) / REGSET_ELT_BITS);
  regset_bytes = FUDGE * regset_size * sizeof (*(regset)0);

  reg_n_refs = (int *) oballoc (FUDGE * max_regno * sizeof (int));
  bzero (reg_n_refs, FUDGE * max_regno * sizeof (int));

  reg_n_sets = (short *) oballoc (FUDGE * max_regno * sizeof (short));
  bzero (reg_n_sets, FUDGE * max_regno * sizeof (short));

  reg_n_deaths = (short *) oballoc (FUDGE * max_regno * sizeof (short));
  bzero (reg_n_deaths, FUDGE * max_regno * sizeof (short));

  reg_live_length = (int *) oballoc (FUDGE * max_regno * sizeof (int));
  bzero (reg_live_length, FUDGE * max_regno * sizeof (int));

  reg_n_calls_crossed = (int *) oballoc (FUDGE * max_regno * sizeof (int));
  bzero (reg_n_calls_crossed, FUDGE * max_regno * sizeof (int));

  reg_basic_block = (short *) oballoc (FUDGE * max_regno * sizeof (short));
  for (i = 0; i < FUDGE * max_regno; i++)
  ...
  if (final)
    {
      ...
      regs_sometimes_live
	= (struct sometimes *) alloca (FUDGE * max_regno * sizeof (struct sometimes));
...
#ifdef HAVE_POST_MODIFY

/*** Kind of transformation required to convert to POST_MODIFY: */

typedef enum {IMPOSSIBLE, SIMPLE, PRE_LOADED, POST_LOADED} post_mod_kind;
	  ...
	  if (win)
	    {
		 ...
		 that follows. */
	      XEXP (x, 0) = gen_rtx ((INTVAL (XEXP (y, 1)) == size
				     ...
				     Pmode1 (Extract_rtx_Pmode (addr)), addr);
		...
		= gen_rtx (EXPR_LIST, REG_INC, addr, REG_NOTES (insn));
	      ...
	      if (RE_RECOG_FLOW) {
		  INSN_CODE(insn) = -1;
		  if (recog_memoized(insn) < 0) abort();
	      }
#endif /* ADI */
...
#ifdef HAVE_POST_MODIFY
      /**** Depending on the situation, conversin to POST-MODIFY involves
	    one or two of the following actions:
	1. assignment of ADDR to INCR_VAL if the two are not
	   	the same;
	2. Substitution of MEM(POST_MODIFY(INCR_VAL, MODIFIER)) for
    		MEM(ADDR).
	3. Substitution of INCR_VAL for ADDR in INCR_INSN or, if INCR_VAL
   		and ADDR are the same, deletion of INCR_INSN.	***/

/**  Semantics of diferrent types of post-modify transformation:
...
*******************************************************************/
    ...
    {
	static post_mod_kind can_post_modify();
	static void change_to_move_or_delete();
	static void change_to_post_mod();
	static void preload_addr();

	rtx modifier, incr_insn, incr_val, temp;
	post_mod_kind mkind = can_post_modify(x, insn, &incr_insn,
					      &incr_val, &modifier);

	/* It is important to call change_to_move_or_delete */
	/* last in each sequence so it can move dead_notes to */
	/* new insns */

	switch (mkind) {

	  case SIMPLE:
	    change_to_post_mod(insn, x, addr, modifier);
	    change_to_move_or_delete(incr_insn, incr_val,modifier);
	    break;

	  case POST_LOADED:
	    change_to_post_mod(insn, x, addr, modifier);
	    change_to_move_or_delete(incr_insn, addr,modifier);
	    break;

	    /*** Pre-loading is more complex because we ***/
	    /*** create a new insn and extend (backwards) the life **/
	    /*** of INCR_VAL ***/

	  case PRE_LOADED:
	    preload_addr(incr_val, addr, insn);
	    change_to_post_mod(insn, x, incr_val, modifier);
	    change_to_move_or_delete(incr_insn, incr_val, modifier);
	    ...
	    /* REGNO is now used in INCR which is below INSN, but
	       ...
	       it as needed, we'll put a REG_DEAD note for it
	       on this insn, which is incorrect.  */
	    needed[REGNO(incr_val) / REGSET_ELT_BITS]
		|= 1 << (REGNO(incr_val) % REGSET_ELT_BITS);
	    /* If there are any calls between INSN and INCR, show
	       that REGNO now crosses them.  */
	    for (temp = insn; temp != incr_insn; temp = NEXT_INSN (temp))
		if (GET_CODE (temp) == CALL_INSN)
		    reg_n_calls_crossed[REGNO(incr_val)]++;
	    break;
	}

	/** Switch the next use of the modifier since we moved it. */
    }
#endif
  }
  /* 
    If 21xx, we might have generated a post modify in rtl-expansion.
    if that is the case, mark it here with a REG_INC note 
    */
  else if(GET_CODE(addr)==POST_MODIFY)
    {
      REG_NOTES (insn)
	= gen_rtx (EXPR_LIST, REG_INC, XEXP(addr,0), REG_NOTES (insn));
    }
}
#endif /* AUTO_INC_DEC */


#if defined(HAVE_POST_MODIFY)

rtx
find_previous_use(reg, insn)
    rtx reg, insn;
{
    rtx i;
...
/*** Below are support routines for the implementation of POST_MODIFY **/

static post_mod_kind
can_post_modify(x, insn, p_incr_insn, p_incr_val, p_modifier)
    rtx x, insn;
    rtx *p_incr_insn, *p_incr_val, *p_modifier;
{
    rtx addr = XEXP(x,0);
    rtx incr_insn, incr_pat, modifier, use;
    rtx incr_plus, incr_dest;
    rtx mreg;
    int regno, opnum;

    if (GET_CODE(addr) != REG) return IMPOSSIBLE;

    /*** Non-sequential insns can present serious problems to **/
    /** the reloader. We might, though, alloc CALL's later, as does **/
    /*** find_auto_inc **/

    if (GET_CODE(insn) != INSN) return IMPOSSIBLE;

    /*** Make sure that ADDR is used only once in INSN **/

    use = find_use_as_address(PATTERN(insn), addr, 0);
    if ((use == 0) || (use == (rtx)1)) return IMPOSSIBLE;

    /*** Fail if ADDR is not used afterwards ***/

    regno = REGNO(addr);
    incr_insn = reg_next_use[regno];
    if (! incr_insn) return IMPOSSIBLE;
    
    /*** The register must be incremented in the same basic block **/
    
    if (BLOCK_NUM (incr_insn) != BLOCK_NUM (insn)) return IMPOSSIBLE;

    /*** Make sure that incr is a PLUS with ADDR in it***/
    
    incr_pat = PATTERN(incr_insn);
    if ((GET_CODE(incr_pat) != SET) ||
	(GET_CODE(incr_plus = SET_SRC(incr_pat)) != PLUS)) return IMPOSSIBLE;
    
    modifier = 0;
    for(opnum=0; opnum<=1; opnum++)
	if (XEXP(incr_plus, opnum) == addr)
	    modifier = XEXP(incr_plus, 1-opnum);
    
    /*** Must have MODIFIER which is not set between INSN and INCR ***/
    /*** If MODIFIER is set between them, we could sometimes move ***/
    /*** the SET in front of INSN, but we leave this for future ***/
    /*** upgrades if it seems useful. ***/
    
    if (modifier == 0) return IMPOSSIBLE;

    /*** Process "paradoxical" (pm/si and dm/si) subregs */
    
    for (mreg = modifier;
	 GET_CODE(mreg) == SUBREG &&
	   (GET_MODE_SIZE(GET_MODE(mreg)) ==
	    GET_MODE_SIZE(GET_MODE(XEXP(mreg,0))));
	 mreg = XEXP(mreg,0));
    
    if (GET_CODE(mreg) == REG) {
	if (reg_set_between_p(mreg, insn, incr_insn))
	    return IMPOSSIBLE;
    }
    else if (! CONSTANT_P(mreg)) return IMPOSSIBLE;
    
    /*** In the future -- try to load modifying value in reg. **/
    /*** This is kludgy in the sense that this is very 21k/21xx **/
    /*** specific -- it is usually **/
    /*** profitable to keep modifiying value in a register because ***/
    /*** the resulting memory reference has a better chance to   ***/
    /*** parallelizable. Note that the modifier register must be ***/
    /*** set before the insn. I wonder if this will screw up the flow. ***/

    incr_dest = SET_DEST(incr_pat);
    
    /*** If ADDR itself is being incremented or ***/
    /*** if the incrementing insn is a lst use of ADDR, ***/
    /*** we are likely to succed **/

    *p_modifier  = modifier;
    *p_incr_insn = incr_insn;
    *p_incr_val  = incr_dest;

    if (incr_dest == addr) return SIMPLE;

    if (dead_or_set_p(incr_insn, addr)) /*** Actually, only DEAD is **/
	return POST_LOADED; 		/*** posslible here */
    
    if ((GET_CODE(incr_dest) == REG) &&
	! reg_used_between_p(incr_dest, insn, incr_insn))
	return PRE_LOADED;

    return IMPOSSIBLE;
}
/** Change INSN to a move from SOURCE. If SOURCE is the same as **/
/** the destination of the move, then simply delete INSN **/
/** In case of the move delete dead note for modifier **/

static void
change_to_move_or_delete(insn, source, modifier)
    rtx insn, source, modifier;
{
    rtx pat = PATTERN(insn), dest;
    int modreg;
    if(GET_CODE(pat) != SET) abort();
    
    dest = SET_DEST(pat);
    if(GET_CODE(modifier) == SUBREG ||
       GET_CODE(modifier) == REG)
    {
	rtx note;
	SWITCH_SUBREG(modifier);
	if (note=find_reg_note(insn,REG_DEAD,modifier))
	{
	    rtx prev_use = find_previous_use(modifier, insn);
	    if (prev_use == 0)
		abort();
	    remove_note(insn, note);
	    REG_NOTES(prev_use) =
		gen_rtx (EXPR_LIST, REG_DEAD, modifier, REG_NOTES(prev_use));
	    reg_next_use[REGNO(modifier)]=insn;
	}
    }
    if (dest == source) {
	PUT_CODE(insn, NOTE);
	...
	SET_SRC(pat) = source;
	/*** Force re-recognition of insn: */
	INSN_CODE(insn) = -1;
    }
}

/** Change MEMREF to POST_MODIFY of NEW_ADDR with MODIFIER **/
/** and set a note on INSN **/

static void
change_to_post_mod (insn, memref, new_addr, modifier)
    rtx insn, memref, new_addr, modifier;
{
    rtx old_addr = XEXP(memref,0);

#ifdef DSP21K
    /* Here we will replace const 1's with const1 registers you know those
       m-regs that are hardwired for performance. */
     
    if (CONSTANT_P (modifier))
      {
	if (GET_CODE (modifier) == CONST
	    && XEXP (modifier, 0) == const1_rtx)
	  modifier = gen_rtx (REG, GET_MODE (modifier), PLUS1_MREG (GET_MODE(modifier)));

	else if (GET_CODE (modifier) == CONST
		 && XEXP (modifier, 0) == constm1_rtx)
	  modifier = gen_rtx (REG, GET_MODE (modifier), MINUS1_MREG (GET_MODE (modifier)));
      }
#endif
    ...
    if (CONSTANT_P(modifier))
      {
	rtx reg = gen_reg_rtx (GET_MODE (old_addr));
	max_regno++;
	emit_insn_before (gen_rtx (SET, GET_MODE (old_addr), reg, modifier), insn);
	modifier = reg;
      }
#endif
	

    XEXP(memref,0) = gen_rtx(POST_MODIFY, GET_MODE(old_addr),
			     new_addr, modifier);
    /* Record that this insn has an implicit side effect.  */
    REG_NOTES (insn)
	= gen_rtx (EXPR_LIST, REG_INC, new_addr, REG_NOTES (insn));
}

/** Insert a move from OLD_ADDR to NEW_ADDR and place it before INSN. **/
/** Update relevant flow information ***/

static void
preload_addr(new_addr, old_addr, insn)
    rtx new_addr, old_addr, insn;
{
    rtx insns, temp;
    ...
    /* INCR will become a NOTE and INSN won't contain a
       use of OLD_ADDR.  If a use of OLD_ADDR was just placed in
       the insn before INSN, make that the next use. 
       Otherwise, invalidate it.  */
    if (GET_CODE (PREV_INSN (insn)) == INSN
	&& GET_CODE (PATTERN (PREV_INSN (insn))) == SET
	&& SET_SRC (PATTERN (PREV_INSN (insn))) == old_addr)
	reg_next_use[REGNO(old_addr)] = PREV_INSN (insn);
    else
	reg_next_use[REGNO(old_addr)] = 0;
}
#endif

     ...
     HOST_WIDE_INT amount;
{
  ...
  XEXP (use, 0) = gen_rtx (amount > 0
			   ...
			   Pmode1 (Extract_rtx_Pmode (reg)), reg);
...
#ifdef ADI
  /*** This is here because flow occasionaly changes insns in such ***/
  /*** way that they do not match the same pattern. At the moment ***/
  /*** (7/15/92) I (az) do not remember exact situation; it could ***/
  /***  be needed only for md with multiple "move" patterns. ***/
