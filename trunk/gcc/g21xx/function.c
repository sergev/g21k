...
/* Nonzero if the interrupt qualifier was used */
...
#ifdef DSP21XX
  p->interrupt = current_function_interrupt;
#endif
...
#ifdef DSP21XX
  current_function_interrupt = p->interrupt;
#endif
	  ...
	  if (!validate_change (object, &XEXP (x, 1), new, 0))
	      ...
	      if (! extra_insns)
		{
		 ...
		 constant with that register.  */
	      temp = gen_reg_rtx (Pmode1 (Extract_rtx_Pmode (new)));
	      ...
	      else
		{
		  ...
		  new = gen_rtx (PLUS, Pmode1 (Extract_rtx_Pmode (new)), XEXP (x, 0), new);
		  XEXP (x, 0) = old;
...
#ifdef DSP21XX
  clear_better_rtx(internal_arg_pointer);
#endif
...
#ifdef DSP21XX
	    emit_move_insn (parmreg, find_better_rtx (entry_parm));
#else
      ...
      else
	{
	  ...
	  if (entry_parm != stack_parm)
	      ...
	      if (stack_parm == 0)
		{
	      ...
	      {
		rtx temp= gen_reg_rtx (GET_MODE (entry_parm));
		emit_move_insn (temp, entry_parm);
		entry_parm = temp;
	      }
#endif
	      if (passed_mode != nominal_mode)
...
#ifdef DSP21XX
  current_function_interrupt = DECL_INTERRUPT (subr);
#endif
