...
#define BASE_REG_CLASS1(m)  BASE_REG_CLASS
#define INDEX_REG_CLASS1(m) INDEX_REG_CLASS
#endif
     ...
     short *reload_reg_p;
{
  ...
  switch (GET_CODE (body))
	...
	return;
#ifdef DSP21XX
      /* Why would we have an insn like this anyways? */
      if (rtx_equal_p (SET_DEST (body), SET_SRC (body)))
	{
/* Don't delete it, just replace it with a painless instruction */
	  SET_DEST (body) = SET_SRC (body) = gen_rtx (REG, SImode, REG_AX0);
/*	  delete_insn (insn);  */
	  return;
	}
#endif
...
#if 0 && (defined DSP21XX)
	      /* 
		Okay, so we gotta mark it clobbered.  But isn't clobbered
		a little severe? Clobbered implies we don't know what
		happened to the register.  But in most cases, this output
		register happens to be the destination of a "SET".  If
		it is, then we know what happened to it.  It isn't
		clobbered, it is set!  So, if that's the case, let's
		still mark that register as clobbered, but lets add
		a reg_note so that we know this is a fake clobber and
		we should look to the previous insn to see what it
		really contains.  
		jak (4/16/93)
		*/
	      {
		...
		if (constraints[i][0] == '='
		  ...
		  {
		    clob =
		      emit_insn_after (gen_rtx (CLOBBER, VOIDmode, recog_operand[i]), insn);
		    if(set && SET_DEST(set)==recog_operand[i])
		      REG_NOTES (clob) = 
			gen_rtx (EXPR_LIST, FAKE_CLOBBER, SET_SRC(set), REG_NOTES (clob));
		  }
	      }
#else

/* 
   This next ifndef is an experiment.  I don't think we really need a clobber there
   since we track equivalences by memory location, not by pseudo register now.
*/
#ifndef DSP21XX
	      /* If this is an output operand, we must output a CLOBBER
		 after INSN so find_equiv_reg knows REGNO is being written. */
...
#ifdef ADI
/* Here, for 21k we will allow whatever the constraint does...forget
   about the register preferencing, because it makes us pick a less
   desirable spill register */
	  ...
	  while (*p && (c = *p++) != ',')
	   ...
	   = (int) reg_class_subunion[this_alternative[i]][(int) GENERAL_REGS];
		...
		if(class_ok_for_mode (REG_CLASS_FROM_LETTER(c), operand_mode[i]))
		  ...
		  this_alternative[i]
#ifdef DSP21XX
		    = (int) REG_CLASS_SUBUNION(this_alternative[i],
		 ...
		 && GET_CODE (recog_operand[i]) == MEM)
	  {
	    operand_reloadnum[i]
	      = push_reload (XEXP (recog_operand[i], 0), NULL_RTX,
			     ...
			     BASE_REG_CLASS1 (GET_MODE (XEXP(recog_operand[i], 0))), 
...
#ifndef DSP21XX  
	  /* temporary kludge -- just checking  -- (az 3/29/94) */
     ...
     int ind_levels;
{
  ...
  if (GET_CODE (ad) == REG)
      ...
      else if (reg_equiv_address[regno] != 0)
	{
	  ...
	  push_reload (tem, NULL_RTX, loc, NULL_PTR, BASE_REG_CLASS1 (GET_MODE (tem)),
  ...
  if (GET_CODE (ad) == MEM)
		    ...
		    && GET_CODE (XEXP (XEXP (tem, 0), 1)) == CONST_INT)))
	{
	  ...
	  push_reload (tem, NULL_RTX, loc, NULL_PTR,
		       BASE_REG_CLASS1 (GET_MODE (tem)), GET_MODE (tem), VOIDmode, 0,
		       0, operand);
	  return 1;
	}
      else
...
#if defined(ADI) && defined(MEMSEG)
	  enum reg_class class=BASE_REG_CLASS(GET_MODE(ad));
#else
	  enum reg_class class=BASE_REG_CLASS;
#endif
...
#if defined(ADI) && defined(MEMSEG)
      find_reloads_address_part (ad, loc, BASE_REG_CLASS(GET_MODE(ad)),
  ...
  if (code0 == REG && code1 == REG)
    {
      ...
      if (REG_OK_FOR_INDEX_P (op0, mode)
	  && REG_OK_FOR_BASE_P (op1, mode))
	return;
      else if (REG_OK_FOR_INDEX_P (op1, mode)
	       && REG_OK_FOR_BASE_P (op0, mode))
	return;
      else if (REG_OK_FOR_BASE_P (op1, mode))
	find_reloads_address_1 (op0, 1, &XEXP (x, 0), operand, ind_levels);
      else if (REG_OK_FOR_BASE_P (op0, mode))
	find_reloads_address_1 (op1, 1, &XEXP (x, 1), operand, ind_levels);
      else if (REG_OK_FOR_INDEX_P (op1, mode))
	find_reloads_address_1 (op0, 0, &XEXP (x, 0), operand, ind_levels);
      else if (REG_OK_FOR_INDEX_P (op0, mode))
	find_reloads_address_1 (op1, 0, &XEXP (x, 1), operand, ind_levels);
      else
	{
     ...
     int ind_levels;
{
  ...
  rtx
      ...
      op1 = NULL_RTX;
  enum reg_class
      class1 = NO_REGS, /* class of mod-reg */
      class0 = NO_REGS;
  RTX_CODE 
      code0,
      code1 = UNKNOWN;
  ...
  /*
    On 21xx reloading op1 makes more sense -- usually there are fewer of them available.
    */
  
  if (op1)
  {
    class1 = MODIFY_REG_CLASS(mode);
    
    if(mode==DMmode && 
       REG_P(op1) && 
       !IS_PSEUDO(REGNO(op1)) && 
       reg_in_class (DAG2_MREGS, REGNO (op1)))
      class1 = DAG2_MREGS;
      
    if (code0 == REG && REG_OK_FOR_BASE_P(op0, mode))
      class1 = IS_DAG1_IREG(REGNO(op0)) ? DAG1_MREGS : DAG2_MREGS;

/* 
  One hope here, that we've got a hard reg for our modify register
  and he's in the right class 
  */
    if(!REG_P(op1) ||
       IS_PSEUDO(REGNO(op1)) ||
       !reg_in_class (class1, REGNO(op1)))
      {
	rtx temp=find_reloads_toplev(XEXP(x,1), ind_levels,0);
	push_reload (temp, 0, &XEXP(x,1), 0, class1, mode, VOIDmode, 0, 0, operand);
	changed_p = modifier_reloaded_p = 1;
      }
  }
     ...
     int ind_levels;
{
  ...
  if(!(code0==REG && REG_OK_FOR_BASE_P(op0,mode)))
      push_reload (XEXP(x,0), XEXP(x,0), &XEXP(x,0), 0, BASE_REG_CLASS(mode), 
		       mode, mode, VOIDmode, 0, operand);
  ...
  switch(code1)
    ...
    case REG:
      ...
      break;
    case CONST_INT:
      do_reload=(INTVAL(op1) > 31 || INTVAL(op1) < -32);
      break;

    case CONST:
      if (GET_CODE (XEXP (op1, 0)) == CONST_INT)
	do_reload = (INTVAL (XEXP (op1, 0)) > 31 
		     || INTVAL (XEXP (op1, 0)) < -32);
      else
	do_reload = 1;
      break;
  ...
  if(do_reload)
    {
      rtx temp=find_reloads_toplev(XEXP(x,1),ind_levels,0);
      push_reload (temp, 0, &XEXP(x,1), 0, INDEX_REG_CLASS(mode),
  ...
  switch(code)
      ...
      if (GET_CODE (XEXP (x, 0)) == REG)
	{
	  ...
	  if ((regno >= FIRST_PSEUDO_REGISTER
		...
		= push_reload (x, NULL_RTX, loc, NULL_PTR,
			       context ? INDEX_REG_CLASS1 (mode) : BASE_REG_CLASS1 (mode),
		  ...
		  if ((regno >= FIRST_PSEUDO_REGISTER
			...
			= find_inc_amount (PATTERN (this_insn), XEXP (x_orig, 1 ));
		      ...
		      value = 1;
		       ...
		       && ! CONST_OK_FOR_AUTO_MODIFY (INTVAL (XEXP(x,1)))) {
 		push_reload (XEXP(x,1), 0, &XEXP(x,1), 0, INDEX_REG_CLASS(GET_MODE(x)),
      ...
      else if (GET_CODE (XEXP (x, 0)) == MEM)
	{
	  ...
	  reloadnum = push_reload (x, 0, loc, 0,
				   context ? BASE_REG_CLASS(GET_MODE(x)) :
				             INDEX_REG_CLASS(GET_MODE(x)),
				   GET_MODE (x), VOIDmode, 0, 0, operand);
      ...
      if (reg_equiv_constant[regno] != 0)
	{
	  find_reloads_address_part (reg_equiv_constant[regno], loc, 
				     (context ? INDEX_REG_CLASS1 (mode)
				      : BASE_REG_CLASS1 (mode)),
		...
		: REGNO_OK_FOR_BASE_P (regno,mode))))
	{
	  push_reload (x, 0, loc, 0,
		       context ? INDEX_REG_CLASS(mode) : BASE_REG_CLASS(mode),
		       GET_MODE (x), VOIDmode, 0, 0, operand);
	  return 1;
	}
#else
     ...
     enum machine_mode mode;
{
...
#ifdef DSP21XX
  /* 
    This is my experiment to see if I can get it to inherit
    reload addresses.  I do this because it is expensive on
    the 21xx to generate a reload address. -jak (4/16/93)
    */
  else if(GET_CODE(goal) == PLUS &&
	      ...
	      || INSN_UID (p) < reload_first_uid))
	{
	  ...
	  if (pat != 0
		  ...
		  || (goal_const && (tem = find_reg_note (p, REG_EQUIV,
		      ...
		      && (valueno = true_regnum (valtry = SET_DEST (pat))) >= 0)
#if 0
		  /* (mmh) turned it back on to see if we have the same problem
		     2.4 merge changed it.
  ...
  while (1)
      ...
      if (GET_RTX_CLASS (GET_CODE (p)) == 'i')
	{
	  ...
	  /* 
	    For an explanation, see the code in find_reloads() that creates
	    a FAKE_CLOBBER note.  A fake clobber note means that this register
	    really isn't clobbered, it is written with the result of a set
	    */
	  if (GET_CODE (pat) == SET || 
	      (GET_CODE (pat) == CLOBBER &&
	       !find_reg_note (p, FAKE_CLOBBER, NULL_RTX)))
#else
