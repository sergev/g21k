...
typedef struct stream {
        /* keep track of an input file (Either the source file
	   or an included file) */
  struct stream *next;         /* static chain */
  struct stream *deeper_nest;  /* dynamic chain of open streams */
  char          *filename;
  int            max_line;          /* last line in file */
  int            old_max_line;
  char          *line_note_exists;  /* array[max_line] with valid line numbers marked */
  long          *line_loc_in_file;  /* array[max_line] of lseek position for all line notes */
  FILE          *file;
  long           last_access;       /* when was this last accessed */
} STREAM;
...
static STREAM the_stream;    /* used to make the only stream fit into 
	  ...
	  input streams (ie the various nested include files) */
static void open_source(STREAM *);
static struct stream *get_source_stream(char *);
static void get_fseek_locs(STREAM *);
static void get_path_from_name(char *);
static long access_timer = 1;  /* keep track of when things are open */
#endif
...
/* Flag for keeping first line number from being output by output_source_line.
   sdb doesn't like this */

int dont_put_line_number=FALSE;

#if defined(ADI_MERGED_LISTING)

/* head of the chain of alternative input streams (ie include files) */

static struct stream *first_stream;

/* areas for constructing file names */
...
/* isolate the path to the original input file (relative to current dir
   if not absolute).  This is used when reopening include files for merged
   listing output. */
...
get_path_from_name(char *name)
{
  ...
  last_slash = (char *)strrchr(name, DIR_SEPARATOR);  /* locate right-most slash */
  if (last_slash == (char *)NULL)
    {
      input_file_path = (char *) NULL;
    }
  else
    {
      int path_len;

      path_len = (int)(last_slash - name)+1;
      input_file_path = (char *) xmalloc (path_len+1);
      strncpy(input_file_path,name,path_len);
      input_file_path[path_len] = (char)NULL;
    }
}
#endif
     ...
     char *filename;
{
  ...
  /* Isolate the path from the cwd to the main input file.  This will be
     needed for reopening the include files in a merged listing.  The path
     is pointed to by the global var input_file_path. */
#ifdef ADI_MERGED_LISTING
  ...
  if (profile_block_flag)
    {
	 ...
	 6:  extra word added by GNU: address of address table
      ...
      assemble_integer (gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), name), UNITS_PER_WORD, 1);
      ...
      for (i = 0; i < count_basic_blocks; i++)
	{
	  ...
	  assemble_integer (gen_rtx (SYMBOL_REF, Pmode1 (DEFAULT_CODE_Pmode), name),
			    UNITS_PER_WORD, 1);
	}
	 ...
	 so we can find it easily, as the last word in the file's text.  */
      ...
      /* Close input files and free assorted stuff used in final */
  if( input_file_path )
  ...
  for( stream = first_stream; stream; stream=next_stream)
    {
      ...
      if( stream->line_note_exists ) free ( stream->line_note_exists );
      if( stream->line_loc_in_file ) free ( stream->line_loc_in_file );
      if( stream->filename )         free ( stream->filename );
      next_stream = stream->next;
      free( stream );
    }
#else
  if( the_stream.file ) fclose( the_stream.file );
#endif
...
#ifdef ADI_MERGED_LISTING
	     || LISTING_WITH_SOURCE  /* need linenums if merging source into output */
#endif
	        ) {
     ...
     int prescan;
{
  ...
  if (write_symbols == SDB_DEBUG
#ifdef ADI_MERGED_LISTING
         || LISTING_WITH_SOURCE   /* merged listing need insn's too */
#endif
)
    {
      rtx last = 0;

#ifndef ADI_MERGED_LISTING
      stream = &the_stream;     /* not merged listing, only one input file */
      stream->max_line = 0;
#endif
      for (insn = first; insn; insn = NEXT_INSN (insn))
	...
	if (GET_CODE (insn) == NOTE && NOTE_LINE_NUMBER (insn) > 0)
	  {
	    ...
	    last = insn;
	   /* ----- calculate max_line(s) from valid notes ---- */
#if defined(ADI_MERGED_LISTING)
	    stream = get_source_stream(NOTE_SOURCE_FILE (insn) );  /* get correct stream */
#endif
	  ...
	  if( stream->line_note_exists )
	    {
	      ...
	      stream->line_note_exists = (char *)xrealloc (stream->line_note_exists,
							   stream->max_line + 1);
	      stream->line_loc_in_file = (long *)xrealloc (stream->line_loc_in_file,
							    sizeof(long)*(stream->max_line + 1));
	    } else {
	      stream->line_note_exists = (char *)xmalloc (stream->max_line + 1);
	      stream->line_loc_in_file = (long *)xmalloc (sizeof(long)*(stream->max_line + 1));
	      stream->old_max_line = -1;
	    }
	  bzero ((char *)((int)stream->line_note_exists+stream->old_max_line+1), 
		           stream->max_line - stream->old_max_line);
	  bzero ((char *)((int)stream->line_loc_in_file+(stream->old_max_line+1)*sizeof(long)),
		           (stream->max_line - stream->old_max_line)*sizeof(long));
	}

          /* ---------- fill line_notes_exists -------- */
  for (insn = first; insn; insn = NEXT_INSN (insn))
    if (GET_CODE (insn) == NOTE && NOTE_LINE_NUMBER (insn) > 0)
      {
#if defined(ADI_MERGED_LISTING) 
  ...
  switch (GET_CODE (insn))
	...
	/* This note is a line-number.  */
	{
	  ...
	  for (note = NEXT_INSN (insn); note; note = NEXT_INSN (note))
	      ...
	      else if (GET_CODE (note) == NOTE && NOTE_LINE_NUMBER (note) > 0)
		{
		  ...
		  for (num = NOTE_LINE_NUMBER (insn) + 1;
		       ...
		       num++)
		    if (stream->line_note_exists[num])
		      break;
	  ...
	  if( note_after )
	    dont_put_line_number = TRUE;
	  output_source_line (file, insn);
	  dont_put_line_number = FALSE;	  /* reset to default */
	}
      break;
...
static void print_from(start_line, instream, out_file)
STREAM *instream;
int start_line;
FILE *out_file;

{
  int i,end_print,number_to_print;
  char c;

  /* Find the line to print until. This is the line before the next occupied
     line after the line in the note insn.  This insures that a multi-line
     stmt gets all printed out at once */
  ...
  number_to_print = end_print - start_line;
    ...
    /* print out the source lines into the merged file */
  fseek (instream->file,instream->line_loc_in_file[start_line],0);

  i = start_line;
  for (; number_to_print; number_to_print-- )
    {
      fprintf( out_file,"%s %-5i    ",ASM_COMMENT_START,i++);
      {
	while( (c = getc (instream->file)) != '\n' )
	  putc( c, out_file );
	putc( '\n', out_file );
      }
    }
     ...
     rtx insn;
{
  ...
  if (write_symbols != NO_DEBUG || LISTING_WITH_SOURCE )
	...
	FILE *in_file;
	    ...
	    /* Position at line to print, within appropriate input file,
	       ie. be in the source or the correct include file. */

 	instream = get_source_stream (filename);

	if( !instream->file )
	  {
	    /* protect against file that got closed because too many open */
	    open_source(instream);   /* try to open again */
	    if( !instream->file )
	      return;            /* run away */
	  }
	instream->last_access = access_timer++;  /* note access for close alg */

	if( !old_filename || strcmp(instream->filename,old_filename)!=0 )
	  fprintf(file,"%s# '%s'\n",ASM_COMMENT_START,instream->filename);

	if( instream->max_line != instream->old_max_line ) {

	      /* we've seen a new procedure at end of file, recalc line locs */
	  get_fseek_locs( instream );
	  instream->old_max_line = instream->old_max_line<1 ? 1: instream->old_max_line;
	  print_from(instream->old_max_line,instream,file);
	}

	instream->old_max_line = instream->max_line;
...
#ifdef ADI
#ifdef ATTR_type_NAMES
static char *insn_type_names [] = ATTR_type_NAMES;
#endif
#endif

#if defined(ADI) && defined(PEEP_PARALLEL)
/*** 1 if output_asm_insn should not do indentation, newlines or comments **/
/*** This feauture is used by peepholer-parallelizer ***/
extern int short_hand_p;
#endif
     ...
     rtx *operands;
{
...
#if defined(ADI) && defined(PEEP_PARALLEL)
  if (flag_print_asm_name && ! short_hand_p)
#else
  if (flag_print_asm_name)
      ...
      if (debug_insn)
	{
	  ...
	  /*** Also print type attribute and function unit-- we want **/
	  /** to see whether they are assigned correctly ***/

	  fprintf (asm_out_file, "\t(%s)",
		   insn_type_names[(int) get_attr_type(debug_insn)]);
#endif
...

          ...
          /* The following "stream" functions manage the various source files
...
void
get_fseek_locs(STREAM *instream)
{
  int i;
  char c;
  ...
  for(i=1;i<instream->max_line+1;i++)
    {
      if( instream->line_note_exists[i] )
	instream->line_loc_in_file[i]=ftell(instream->file);
      while( (c=getc (instream->file) != '\n') && (c !=EOF) )
	;
    }
}
        static void
open_source ( STREAM *instream )

        /* open a new source stream (for a main or included file).
	   This source stream goes at the bottommost level */
{
    FILE *file;
    char *temp_filename;

    if ( input_file_path == (char *)NULL )
      {
	temp_filename = instream->filename;
      }
    else
      {
	temp_filename = (char *)xmalloc(strlen(instream->filename) + strlen(input_file_path) + 1);
...
*get_source_stream (filename)

/* find stream for a filename.  This ignores the
   open stream stack and just uses the static connections. */
...
char *filename;
{
  ...
  if (!first_stream) {
    stream = (STREAM *)xmalloc(sizeof(struct stream));
    first_stream = stream;
  } else {
    for(stream = first_stream; stream; stream=stream->next)
      {
	if( strcmp(filename, stream->filename) == 0 )
	  {
	    return stream;
	  }
	if( !stream->next )
    ...
    stream->next = (struct stream *)xmalloc(sizeof (struct stream));
    stream = stream->next;
  }
     /* initialize stream structure */
  bzero( stream, sizeof(struct stream) );
  stream->filename = (char *) xmalloc (strlen(filename)+1);
  stream->max_line = 0;
...
close_a_file()
 {
  ...
  for( stream=first_stream->next; stream; stream=stream->next )
    {
      if( stream->file )
	{
	  if( stream->last_access && (stream->last_access < earliest_time ))
	    {
	      earliest_time = stream->last_access;
	      close_stream = stream;
	    }
	}
    }
  if( close_stream )
    {
      fclose(close_stream->file);
      close_stream->file = (FILE *)NULL;
    }
}
#endif
