...
#ifndef DEFAULT_STRENGTH_REDUCTION_BENEFIT
#define DEFAULT_STRENGTH_REDUCTION_BENEFIT 0
#endif
...
#ifdef ADI
static void compute_loop_depths ();
#endif

     ...
     FILE *dumpfile;
{
  ...
  max_uid_for_loop = 5*(get_max_uid () + 1 + max_loop_num * 32);
  ...
  /**** Recompute LUIDs for each loop *******/
  {
    ...
    { 
      int i;	/*** localize i to distinguish it from the loop number **/
     ...
     int nregs;
{
  ...
  for (m = movables; m; m = m->next)
    {
#ifdef DSP21XX_________
	/* Reserving a register for a constant will end up costing
	   more on a machine with small register set.
	   We could make more intelligent decisions in the future, i.e.
	   gues whether there is enough registers, or rerun loops after
	   reloader. (az 1/22/94)
	 */
	/* This does not seem to by us much on most tests. az 1/28/94 */
	if (CONSTANT_P(m->set_src))
	    continue;
#endif
     ...
     rtx loop_end;
{
  ...
  loop_iv_list = 0;
  addr_placeholder = gen_reg_rtx (Pmode1 (DEFAULT_DATA_Pmode));
...
#ifdef ADI
      /* I (az) believe that we can ignore back jumps. To be on the safe
       side, I restrict this bet to conditional jumps only. */
	  && ! (GET_CODE (p) == JUMP_INSN &&
		condjump_p(p) &&
		INSN_LUID(JUMP_LABEL (p)) < INSN_LUID(p))
#endif
	  /* If we enter the loop in the middle, and scan around to the
  ...
  bzero ((char *) reg_map, max_reg_before_loop * sizeof (rtx));
    ...
    {
	void prepare_doloop();
	if (flag_strength_reduce && ! flag_unroll_loops)
#ifdef DSP21XX
	    if (!current_function_interrupt &&
		! NODOLOOPS)
#endif
	    prepare_doloop(loop_start, end);
    }
#endif
  ...
  for (bl = loop_iv_list; bl; bl = bl->next)
      ...
      for (v = bl->giv; v; v = v->next_iv)
	{
	  ...
	  /* ??? The new final_[bg]iv_value code does a much better job
	     ...
	     be necessary.  */
#if defined(ADI) && ! defined(DSP21XX)

      /*     In  DSP  we  generally  want to   err  on   the side  of  more */
      /*     strength     reduction    because    we     can have  multiple */
      /*     benefits:   a)   using DOLOOP     thus  eliminating  BIV,   b) */
      /*     using post-modify's which,   in  turn,   might   allow    more */
      /*     parallelization.        					    */

	     /* but on 21xx we want to discourage strength reduction because */
	  /* it tends to create more variables with long lives. */
     ...
     rtx loop_start, loop_end;
{
  ...
  if (GET_CODE (src_reg)==SUBREG)
    bl = reg_biv_class [REGNO (SUBREG_REG (src_reg))];
  else
  ...
  if (loop_dump_stream)
	...
	fprintf (loop_dump_stream, " src reg %d benefit %d",
		 REGNO (SUBREG_REG(src_reg)), v->benefit);
      else
	fprintf (loop_dump_stream, " src reg %d benefit %d",
...
#ifdef ADI
  if (GET_CODE (v->src_reg) == SUBREG)
      v->src_reg = SUBREG_REG (v->src_reg);
		  ...
		  if (reg_mentioned_p (v->dest_reg, PATTERN (p)))
		    {
		      ...
		      break;
			   ...
			   (GET_CODE(v->src_reg)==SUBREG &&
			    SET_DEST(PATTERN(p)) == SUBREG_REG (v->src_reg))))
#else
		       && SET_DEST (PATTERN (p)) == v->src_reg)
#endif
...
#ifdef DSP21XX
      if (GET_MODE (dest_reg)==SImode && CONST_INT_P(arg))
	*inc_val=arg;
      else
...
{
  rtx orig_x = x;
  int benefit = DEFAULT_STRENGTH_REDUCTION_BENEFIT;
  char *storage;

#ifdef ADI

  /* We generally want to err on the side of strength reduction */
  /* because our chip updates base address instead of an index, */
  /* hence the possibility of eliminating the index (modifier) register */

  {
    extern int strength_reduction_benefit;
    benefit += strength_reduction_benefit;
  }
#endif
     ...
     int *benefit;
{
  ...
  switch (GET_CODE (x))
...
#ifdef ADI
      /** I (az) am not sure that this is a correct way -- just want **/
      /** to try and see. **/
...
#ifdef DSP21XX
      else if (GET_CODE(arg0) == SUBREG && REG_P (XEXP (arg0,0)))
	arg0 = gen_rtx (MULT, mode, arg0, const1_rtx);
#endif
      if (GET_CODE (arg1) == REG)
	arg1 = gen_rtx (MULT, mode, arg1, const1_rtx);
#ifdef DSP21XX
      else if (GET_CODE(arg1) == SUBREG && REG_P (XEXP (arg1,0)))
	arg1 = gen_rtx (MULT, mode, arg1, const1_rtx);
#endif
    ...
    case SUBREG:
      if (PMODE_P (GET_MODE (x))
	  && REG_P (XEXP (x,0))
	  && GET_MODE_CLASS (GET_MODE (XEXP (x,0))) == MODE_INT) {

	rtx reg = XEXP (x,0);
	if (REGNO (reg) >= max_reg_before_loop)
	  return 0;

	/* Check for biv or giv.  */
	switch (reg_iv_type[REGNO (reg)]) {
	  case BASIC_INDUCT:
	    return x;
	  case GENERAL_INDUCT:
	    {
	      struct induction *v = reg_iv_info[REGNO (reg)];
     ...
     struct induction *g1, *g2;
{
  ...
  /* We want to prevent combining PMmode and DMmode givs 
     because we end up with mixed mode operations which we do
     not recognize. In the future, we might want to allow this 
     with SUBREG inserted to make the resulting expressions legal */
