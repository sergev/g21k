     ...
     tree type, expr;
{
  ...
  if (warn_float_convert && 
      form == REAL_TYPE && 
      TYPE_PRECISION(TREE_TYPE(expr)) != TYPE_PRECISION(type))
    warning ("conversion of floating point might be undesirable");
#endif
  ...
  if (form == INTEGER_TYPE || form == ENUMERAL_TYPE || form == FRACT_TYPE)
...
tree 
convert_to_fract (type, expr)
     tree type, expr;
{
  register tree intype         = TREE_TYPE (expr);
  register enum tree_code form = TREE_CODE (intype);
 
  if (form == REAL_TYPE || form == INTEGER_TYPE || form == FRACT_TYPE)
      return build1 (CONVERT_EXPR, type, expr);
  else
    error ("can't convert to a fract.");

  abort();
/*  return build_fract(type, 0,0,0,0);*/
}     
