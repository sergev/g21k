...
init_caller_save ()
{
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
    ...
    if (TEST_HARD_REG_BIT (reg_class_contents[(int) ALL_BASE_REG_CLASS], i))
#else
  ...
  addr_reg = gen_rtx (REG, Pmode1 (DEFAULT_DATA_Pmode), i);

  for (offset = 1 << (HOST_BITS_PER_INT / 2); offset; offset >>= 1)
    {
      address = gen_rtx (PLUS, Pmode1 (DEFAULT_DATA_Pmode), addr_reg, GEN_INT (offset));
      ...
      for (insn = basic_block_head[b]; ; insn = NEXT_INSN (insn))
	{
	  ...
	  if (GET_RTX_CLASS (code) == 'i')
		...
		if (REG_NOTE_KIND (link) == REG_DEAD)
	      ...
	      /* On the 21xx we specially prereloaded calls, 
		 We need to skip these insns when considering
		 which registers need to be saved AND
		 we need to do the saves BEFORE the first
		 insn in the call sequence and the restores
		 AFTER the call itself 
		 */
	      if (find_reg_note (marker=insn, INSN_START_CALL, 0) 
		  || code==CALL_INSN)
		{
...
#ifdef DSP21XX
		      regno += insert_save_restore (marker, 1, regno, 
#else
...
#ifdef ADI
	  /* 
	    Why the code asked for the MIN() here is beyond me.
	    This gets us into problems if we are trying to restore
	    a DImode register, cause MOVE_MAX and UNITS_PER_WORD 
	    are both 1, but we know that we'll need to move two
	    registers.  
	    */
...
#ifdef ADI
	      /* I don't know how things ever worked without this line.
		 Basically, if we don't clear this bit, we think we
		 don't need to save this register ever again, since he
		 was already saved.  If he was modified, though, then
		 we won't be using the right value for this register.
		 A smarter scheme might be to clear hard_regs_need_restore
		 at this time and then, if an insn changes the value
		 of the register, THEN clear hard_regs_saved and decrement
		 n_regs_saved.  It seems very important, though, that
		 n_regs_saved and hard_regs_saved be in sync.
		 -Josh Kablotsky 23-June-1993 */
