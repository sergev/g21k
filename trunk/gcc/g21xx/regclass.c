	...
	if (TEST_HARD_REG_BIT (reg_class_contents[i], j))
	  {
	      ...
	      if (HARD_REGNO_MODE_OK (j, m))
		{
		  ...
		  if (0
		      ...
		      || (SECONDARY_INPUT_RELOAD_CLASS (BASE_REG_CLASS1 (m), m, r)
			  != NO_REGS)
#endif
#ifdef SECONDARY_OUTPUT_RELOAD_CLASS
		      || (SECONDARY_OUTPUT_RELOAD_CLASS (BASE_REG_CLASS1 (m), m, r)
			  != NO_REGS)
#endif
		      )
		  ...
		  if (set != 0 && GET_CODE (SET_DEST (set)) == REG
			...
			-= (MEMORY_MOVE_COST (GET_MODE (SET_DEST (set)))
		      ...
		      record_address_regs (XEXP (SET_SRC (set), 0),
					   BASE_REG_CLASS1 (Extract_rtx_Pmode (SET_SRC (set))),
					   loop_cost * 2);
		      continue;
		    }
	      ...
	      for (i = 0; i < noperands; i++)
		{
		  ...
		  if (GET_CODE (recog_operand[i]) == MEM)
		    record_address_regs (XEXP (recog_operand[i], 0),
					 BASE_REG_CLASS1 
					 (Extract_rtx_Pmode (XEXP (recog_operand[i], 0))) , 
					 loop_cost * 2);
		  else if (constraints[i][0] == 'p')
		    record_address_regs (recog_operand[i],
					 BASE_REG_CLASS1 (Extract_rtx_Pmode (recog_operand[i])),
					 loop_cost * 2);
		}
...

/* I made the following variables to allow their modification in a debugger */
int post_modify_base_benefit  = 8;
int post_modify_index_benefit = 8;
     ...
     int scale;
{
  ...
  switch (code)
...
#if 0 && defined( ADI )
	/* Josh's idea: if one reg in  MEM(REG,REG) is already 
	   biased toward a base class, bias another one toward
	   corresponding index class. */
	{
	  int r0 = REGNO(arg0),
	      r1 = REGNO(arg1),
	      r0_i_bias =
		  savings[r0].savings[DM_IREGS] - 
	          savings[r0].savings[DM_MREGS],
	      r1_i_bias =  
		  savings[r1].savings[DM_IREGS] - 
		  savings[r1].savings[DM_MREGS];

	  if      (r0_i_bias != 0) 
	    savings[r1].savings[DM_IREGS] -= r0_i_bias;
	  else if (r1_i_bias != 0) 
	    savings[r0].savings[DM_IREGS] -= r1_i_bias;
	}
#endif
	...
	else if (INDEX_REG_CLASS1 (Extract_rtx_Pmode (arg0)) 
		 == BASE_REG_CLASS1 (Extract_rtx_Pmode (arg0)))
	  {
	    record_address_regs (arg0, class, scale);
	    if (! CONSTANT_P (arg1))
	...
	/* If the second operand is a symbolic constant, the first operand
	  ...
	  record_address_regs (arg0, INDEX_REG_CLASS1 (Extract_rtx_Pmode (arg0)), scale);
	...
	/* If this the sum of two registers where the first is known to be a 
	  ...
	  {
	    record_address_regs (arg0, BASE_REG_CLASS1 (Extract_rtx_Pmode (arg0)), scale);
	    record_address_regs (arg1, INDEX_REG_CLASS1 (Extract_rtx_Pmode (arg0)), scale);
	  }
	...
	/* If this is the sum of two registers and neither is known to
	  ...
	  {
	    record_address_regs (arg0, BASE_REG_CLASS1 (Extract_rtx_Pmode (arg0)), scale / 2);
	    record_address_regs (arg0, INDEX_REG_CLASS1 (Extract_rtx_Pmode (arg0)), scale / 2);
	    record_address_regs (arg1, BASE_REG_CLASS1 (Extract_rtx_Pmode (arg1)), scale / 2);
	    record_address_regs (arg1, INDEX_REG_CLASS1 (Extract_rtx_Pmode (arg1)), scale / 2);
	  }
	...
	/* In all other cases, the first operand is an index and the
	  ...
	  {
	    record_address_regs (arg0, INDEX_REG_CLASS1 (Extract_rtx_Pmode (arg0)), scale);
	    record_address_regs (arg1, BASE_REG_CLASS1 (Extract_rtx_Pmode (arg1)), scale);
	  }
      }
      break;
    ...
    case PRE_MODIFY:
    case POST_MODIFY:
      /* Quadruple the importance of a pseudo register that is incremented
	 or decremented, since it would take two extra insns
	 if it ends up in the wrong place.  */
      record_post_modify_regs (x, class, 4 * scale);
      break;

    case SUBREG:
      addr_mode = GET_MODE(x);
      x = XEXP(x,0);
      if (GET_CODE(x) == REG) goto reg_or_subreg;
      else abort();
#endif
...
#if defined DSP21XX
	class = BASE_REG_CLASS (addr_mode);
#else
#if defined DSP21K
	class = BASE_REG_CLASS1 (addr_mode);
#else
	class = INDEX_REG_CLASS1 (addr_mode);
#endif
#endif
	pp->mem_cost += 
	  (MEMORY_MOVE_COST (Pmode1 (addr_mode)) * scale) / 2;
	for (i = 0; i < N_REG_CLASSES; i++)
...
#ifdef ADI
record_post_modify_regs(x, class,  scale)
     rtx x;
     enum reg_class class;
     int scale;
{
    ...
    int base_regnum, mod_regnum,
	bcost = scale * post_modify_base_benefit,
	mcost = scale * post_modify_index_benefit;

    struct costs *bsp, *msp;
    enum reg_class base_class, mod_class;
    ...
    if (GET_CODE (modifier) == SUBREG)
      modifier = SUBREG_REG (modifier);

    base_regnum = REGNO (base);
    mod_regnum = REGNO (modifier);

    bsp = REG_P (base) ? &costs[base_regnum] : 0;
    msp = REG_P (modifier) ? &costs[mod_regnum] : 0;

#ifdef DSP21K
    base_class = BASE_REG_CLASS1 (mode);
    mod_class  = INDEX_REG_CLASS1 (mode);
#endif
#ifdef DSP21XX
    mod_class  = MODIFY_REG_CLASS (mode);
#endif

/*
  Even though on 21xx DM can be accessed by both DAG1 and DAG2 registers,
  we want one of them to be preferred over another so that we can also bias 
  the corresponding modifier register in the same direction.
*/

#ifdef DSP21XX
    if (mode == DMmode) 
      {
	if (IS_DAG1_IREG (REGNO (base))
	    || IS_DAG1_MREG (REGNO (modifier)))
	  {
	    base_class = DAG1_IREGS;
	    mod_class  = DAG1_MREGS;
	  }
	else if (IS_DAG2_IREG (REGNO (base)) 
		 || IS_DAG2_MREG (REGNO (modifier)))
	  {
	    base_class = DAG2_IREGS;
	    mod_class  = DAG2_MREGS;
	  }
	else if (bsp->cost[DAG1_IREGS] < bsp->cost[DAG2_IREGS])
	  {
	    base_class = DAG1_IREGS;
	    mod_class  = DAG1_MREGS;
	  }
    ...
    if (mode == DMmode) 
      {
	if (IS_DM_IREG (REGNO (base))
	    || IS_DM_MREG (REGNO (modifier)))
	  {
	    base_class = DM_IREGS;
	    mod_class  = DM_MREGS;
	  }
	else if (IS_PM_IREG (REGNO (base)) 
		 || IS_PM_MREG (REGNO (modifier)))
	  {
	    base_class = PM_IREGS;
	    mod_class  = PM_MREGS;
	  }
	else if (bsp->cost [DM_IREGS] < bsp->cost[PM_IREGS])
	  {
	    base_class = DM_IREGS;
	    mod_class  = DM_MREGS;
	  }
	...
	&& base_class != ALL_REGS) 
      {
	bsp->mem_cost += (MEMORY_MOVE_COST (mode) *bcost * (loop_depth+1)) /2;
	for (i=0; i < N_REG_CLASSES; i++)
	  bsp->cost[i] += (may_move_cost[i][(int) base_class] * bcost *(loop_depth+1)) /2;
      }
    if (msp 
	&& mod_class  != NO_REGS 
	&& mod_class != ALL_REGS) 
	...
	msp->mem_cost += (MEMORY_MOVE_COST (mode) * mcost * (loop_depth+1)) /2;
	for (i=0; i < N_REG_CLASSES; i++)
	  msp->cost[i] += (may_move_cost[i][(int) mod_class] * mcost * (loop_depth+1)) /2;
      }
}
#endif	/* ifdef ADI */
