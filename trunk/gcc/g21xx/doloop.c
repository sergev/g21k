...
static int  find_loop_comp_and_jump();
static int  unjumped_loop_p(rtx, rtx, rtx *);
static int  get_loop_depth();
static int  can_compute_loop_counter ();

static void fix_label();
static rtx  label_after_insn();
static void mark_doloop_jump();
static void convert_comparison_to_doloop_end();
static rtx  emit_doloop_start();
static rtx  emit_counter_init_expr ();

static void get_strength_reduction_benefit();
static int  insn_in_range();
static rtx  handle_posttest_loops();
#define boolean int
...
typedef struct {
    ...
    boolean unsigned_p;	/* True if unsigned comparison in the end of loop */
    rtx loop_start;
} doloop_info;
...
void
prepare_doloop (loop_start, loop_end)
...
/*   Collect data and check possibility of using doloops */
	...
	return;
    if (! unjumped_loop_p(loop_start, loop_end, &continued_p))
	return;
    start_label = NEXT_INSN(loop_start);
...
static int
find_loop_comp_and_jump(loop_end, pcomp, pjump)
    rtx loop_end;
    rtx *pcomp, *pjump;
{
...
static void
convert_comparison_to_doloop_end(comp_insn, jump_insn, contp)
    rtx comp_insn, jump_insn;
    int contp;
{
...
int
can_compute_loop_counter (loop_start, loop_end, p_dl_info)
    rtx loop_start, loop_end;
    doloop_info *p_dl_info;
{
    ...
    if (loop_count > 0) {
#ifdef DSP21XX
	/* 2100's have only 14 bits in LCNTR: */
	if (loop_count >= 0x4000)
	    return 0;
#endif
	return 1;
    }
...
static rtx
emit_counter_init_expr(p_dl_info)
    doloop_info *p_dl_info;
{
    rtx result;
     ...
     rtx start_label, end_label;
{
  ...
  insn = start_label;
  while( insn && (GET_CODE(insn)==NOTE || GET_CODE(insn)==CODE_LABEL) ) 
    insn = PREV_INSN(insn);   /* find previous non-junk insn */
  if( insn && GET_CODE(insn) == JUMP_INSN ) {
    jump_target = JUMP_LABEL(insn);
    if( jump_target == end_label ) return 1;
...
static rtx
emit_doloop_start(counter_init, start_label, end_label, depth)
    rtx counter_init, start_label, end_label;
    int depth;
{
...
void get_strength_reduction_benefit()
{
    int srb;
...
int
get_loop_depth(loop_start, loop_end)
    rtx loop_start, loop_end;
{
    int loop_num;
...
int
unjumped_loop_p (loop_start, loop_end, padded_labelp)
    rtx  loop_start, loop_end;
    rtx *padded_labelp;
{
    rtx curi,
