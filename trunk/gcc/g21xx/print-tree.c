     ...
     int indent;
{
...
#if defined(ADI) && defined(MEMSEG) && defined(MEMSEG_NAME)

  fputs(" segment:", file);
  fputs(MEMSEG_NAME(TREE_MEMSEG(node)), file);
  ...
  switch (TREE_CODE_CLASS (TREE_CODE (node)))
	...
	fputs (" type_6", file);
      ...
      fprintf (file, " format %d.%d", 
	       TYPE_BINARYPOINT (node), 
	       TYPE_PRECISION (node)-1);
