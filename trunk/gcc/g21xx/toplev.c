...
/* Path to go between CWD (or root) and directory with original source
   file.  This path is used to reopen included files when reopening them 
   for merged listing output */
...
/* Warn if converting between different floatingpoint modes..
   i.e. double to float float to double.
   */

int warn_float_convert;
...
struct { char *string; int *variable; int on_value;} W_options[] =
{
  ...
  {"float-convert", &warn_float_convert, 1},
};

...
{
  if (INSN_CODE (insn) < 0)
    error ("internal error--unrecognizable insn:", 0,0);
  else
    error ("internal error--insn does not satisfy its constraints:", 0,0);
  debug_rtx (insn);
  if (asm_out_file)
    fflush (asm_out_file);
  if (aux_info_file)
     ...
     HOST_WIDE_INT v2;		/* Also used as pointer */
{
  ...
  warning_with_file_and_line (filename, line, s, v, v2, 0);
}

     ...
     HOST_WIDE_INT v2;
{
  if (flag_pedantic_errors)
    ...
    warning_with_file_and_line (file, line, s, v, v2, 0);
}
     ...
     char *name;
{
...
#ifdef __WATCOMC__
  dump_base_name = strdup(dump_base_name);
  {
    char *p=strchr(dump_base_name,'.');
    if(p && p!=dump_base_name)
      *p='\0';
  }
#endif
...
#ifdef DSP21XX
  mod_reg_equiv_const=(int *)malloc(sizeof(int)*FIRST_PSEUDO_REGISTER);
  if(!mod_reg_equiv_const)
      abort();
  mod_reg_equiv_size=FIRST_PSEUDO_REGISTER;
  bzero(mod_reg_equiv_const,sizeof(int)*FIRST_PSEUDO_REGISTER);
#endif
...

  {
	...
	if ((warn_unused
	    ...
	    && DECL_EXTERNAL (decl)
	    && ! TREE_PUBLIC (decl))
	  pedwarn_with_decl (decl, "`%s' declared `static' but never defined", 0);
     ...
     int at_end;
{
  ...
  if (TREE_STATIC (decl) || DECL_EXTERNAL (decl)
	...
	error ("invalid register name `%s' for register variable", (HOST_WIDE_INT) asmspec,0);
    }
#if defined (DBX_DEBUGGING_INFO) || defined (XCOFF_DEBUGGING_INFO)
  ...
  if (optimize > 0)
    {
      TIMEVAR (loop_time,
	       {
/* run it twice on the 21xx -- takes longer, so what ?  */
...
#ifdef DSP21XX
	if (KILL_REGS)
	    kill_regs(insns, flow_dump_file);
#endif		 
...
#ifdef DSP21XX
      /* On 21xx scheduling before register allocation 
	 can increase register life times which creates extra 
	 conflicts. Also, this scheduling is of marginal value since 
	 reloader creates so many new instructions.
	 */
  ...
  for (i = 1; i < argc; i++)
      ...
      else if (argv[i][0] == '-' && argv[i][1] != 0)
	{
	  ...
	  if (str[0] == 'm')
		...
		error ("Invalid option `%s'", (HOST_WIDE_INT) argv[i],0);
	    }
	  else if (str[0] == 'O')
	    {
	      register char *p = str+1;
	  ...
	  else if (!strcmp (str, "a"))
	    {
	      ...
	      warning ("`-a' option (basic block profile) not supported", 0, 0, 0);
#else
	      ...
	      if (*q || level > 3)
		{
		  warning ("invalid debug level specification in option: `-%s'", 
			   (HOST_WIDE_INT) str, 0, 0);
		  warning ("no debugging information will be generated", 0, 0, 0);
		  level = 0;
		}
...
You Lose!  You must define PREFERRED_DEBUGGING_TYPE!
...
#endif	      
	      if (write_symbols == NO_DEBUG)
		warning ("`-%s' option not supported on this version of GCC", 
			 (HOST_WIDE_INT) str, 0, 0);
	      else if (level == 0)
  ...
  if (optimize == 0)
	...
	warning ("-Wuninitialized is not supported without -O", 0, 0, 0);
    }
  ...
  if (flag_schedule_insns || flag_schedule_insns_after_reload)
    warning ("instruction scheduling not supported on this target machine", 0, 0, 0);
#endif
#ifndef DELAY_SLOTS
  if (flag_delayed_branch)
    warning ("this target machine does not have delayed branches", 0, 0, 0);
#endif
     ...
     char *name;
{
  ...
  if (!valid)
    error ("Invalid option `%s'", (HOST_WIDE_INT) name,0);
}

