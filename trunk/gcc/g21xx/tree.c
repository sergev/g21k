...
/*
  ...
  That compiler does not do right things for "(double)(unsigned)x" when
  x is negative.
  We might go back to old code once djgcc is fixed.
  AZ. 2/3/93
*/

static double long2unsigned2double(x)
     long x;
{
  double dd;
  if (x >= 0)
...
#ifndef BROKEN_DJGCC_DOUBLE_UNSIGNED
      d += (double) (unsigned HOST_WIDE_INT) (~ TREE_INT_CST_LOW (i));
#else
      d += long2unsigned2double (~ TREE_INT_CST_LOW (i));
#endif
      d = (- d - 1.0);
    }
  else
...
#if defined(ADI) || defined(MEMSEG)
  TREE_MEMSEG(result) = TREE_MEMSEG(ref);
#endif
...
tree
#if defined(ADI) && defined(MEMSEG)
build_type_variant (type, constp, volatilep, segment)
...
#if defined(ADI) && defined(MEMSEG)
	/*** We also keep types for each of the segment values ***/
      if (constp    == TYPE_READONLY (t) &&
	  volatilep == TYPE_VOLATILE (t) &&
	  segment   == TREE_MEMSEG(t))
#else
...
#if defined(ADI) && defined(MEMSEG)
  /*** Promote the element's memory segment to that of the array ***/
  TREE_MEMSEG(t) = TREE_MEMSEG(elt_type);
#endif
...
#ifdef ADI 
  /* 
    I put this in becuase if op can't be made into a narrower value,
    we still need to return whether it is unsigned or not. -jak(4/14/93)
    */
  register int uns = TREE_UNSIGNED (TREE_TYPE (op));
#else
  register int uns = 0;
#endif
  int first = 1;
