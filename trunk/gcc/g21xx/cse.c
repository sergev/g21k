...
#define COST(X)						\
  (GET_CODE (X) == REG					\
   ? (REGNO (X) >= FIRST_PSEUDO_REGISTER ? register_cost \
...
#ifdef DSP21XX
	/* On 21xx (but not on C0) we want to optionaly disable */
	/* cse for constants: it is often easier to reload a constant */
	/* into a register than it is to spill that register */
	if (NOCSE && qty_const [reg_qty[REGNO (x)]])
	    return x;
#endif
  ...
  switch (code)
      ...
      if (folded_arg0 != SUBREG_REG (x))
	{
	  ...
	  if (GET_CODE (new)==SYMBOL_REF &&
	      GET_MODE (new)!=GET_MODE (x))
	    {
	      new = gen_rtx (CONST, GET_MODE (x), new);
	    }
#endif
